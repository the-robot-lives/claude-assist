defmodule Styleguide.Catalog do
  @moduledoc """
  ...
  **Themes are not hardcoded.** `themes/0` discovers `themes/theme-*/` directories
  ...
  defp load_theme_from_dir(dir, folder_name) do
    ...
    name =
      cond do
        is_binary(title) and title != "" and String.contains?(title, " ") ->
          # e.g. "noizu.ink — NEON PROTOCOL Style Guide" → use short name if present
          raw_name || title
