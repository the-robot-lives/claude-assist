defmodule StarterWeb.AuthControllerTest do
  ...
  alias Starter.Schema.Organizations.InviteTokenRedemption
  ...
  describe "POST /api/v1/auth/register" do
    test "creates complete password registrations as pending without invite", %{conn: conn} do
      ...
      user = Starter.Repo.get_by!(User, email: email)
    ...
    test "valid invite activates the user and records redemption", %{conn: conn} do
      ...
      user = Starter.Repo.get_by!(User, email: email)
      invite = Starter.Repo.get!(InviteToken, invite.id)
  ...
  defp create_invite(email, attrs) do
    ...
    Starter.Organizations.create_invite_token(Map.merge(defaults, Map.new(attrs)))
  end
end
