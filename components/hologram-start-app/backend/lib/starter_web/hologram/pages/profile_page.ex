defmodule StarterWeb.Hologram.Pages.ProfilePage do
  ...
  defp verify_and_update_password(user, current_password, new_password) do
    import Ecto.Query
    alias Starter.Schema.Users.Credentials.UserCredential, as: CredSchema

    context = Noizu.Context.system()

    with {:ok, auth_provider} <- Starter.Auth.Providers.login(),
         {:ok, auth_provider_id} <- Starter.Auth.Providers.Provider.id(auth_provider) do
      credential =
      ...
      cond do
        ...
        true ->
          case Starter.Users.Credentials.update_password(user, new_password, context) do
            {:ok, _} -> :ok
            other -> other
          end
      end
