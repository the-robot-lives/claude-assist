defmodule StarterWeb.Hologram.Pages.TailwindPlusPage do
  @moduledoc """
  Browse Tailwind Plus demos (iframe → static HTML under priv/static/twp/demos).

  Catalog is JSON + HTML only — no TypeScript. Prefers `Starter.StyleGuide.TwpCatalog`
  ...
  defp catalog_sections do
    if catalog_available?() do
      Starter.StyleGuide.TwpCatalog.sections()
    else
  ...
  defp catalog_stats do
    if catalog_available?() and function_exported?(Starter.StyleGuide.TwpCatalog, :stats, 0) do
      Starter.StyleGuide.TwpCatalog.stats()
    else
  ...
  defp catalog_first_entry do
    if catalog_available?() and function_exported?(Starter.StyleGuide.TwpCatalog, :first_entry, 0) do
      Starter.StyleGuide.TwpCatalog.first_entry()
    else
      nil
    end
  end
  ...
  defp catalog_available? do
    Code.ensure_loaded?(Starter.StyleGuide.TwpCatalog) and
      function_exported?(Starter.StyleGuide.TwpCatalog, :sections, 0)
  end
