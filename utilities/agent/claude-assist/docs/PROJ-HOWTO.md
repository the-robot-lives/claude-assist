...
1. Install workspace deps and symlink the launcher onto your PATH:
   ```bash
   cd utilities/agent/claude-assist
   make install
   ```
2. Launch it:
   ```bash
   claude-assist
   ```
   With `zellij` installed and not already inside a session, this opens a split pane (web 80% / api 20%). Otherwise it falls back to the same thing without panes (`claude-assist --no-zellij`).
...
- Ports are fixed unless overridden: `CLAUDE_ASSIST_API_PORT` (default 3100), `CLAUDE_ASSIST_WEB_PORT` (default 5173).
...
**Gotchas:** `--interface`/`CLAUDE_ASSIST_DEFAULT_INTERFACE` are parsed by `packages/cli/src/interface-selection.ts` but that parser is never wired into `bin.ts` — passing `--interface tui` currently has no effect. Use the explicit `interactive` command instead.
...
**Gotchas:** the API also watches `CLAUDE_ASSIST_WATCH_PATHS` via chokidar by default — manual reindex is only needed if you set `CLAUDE_ASSIST_WATCH=false` or added a new index path since last boot.
