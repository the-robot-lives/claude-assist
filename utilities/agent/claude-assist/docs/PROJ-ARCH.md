...
It lives at `utilities/agent/claude-assist/` in the Noizu Infra monorepo but is a self-contained pnpm/TypeScript project — it does not use the shared `share/k8-lib` shell library or `.infra-config.yaml` build metadata. Its own `Makefile` (`make install`) installs pnpm deps and symlinks `bin/claude-assist` into `~/.local/bin`, matching the monorepo's utilities-on-PATH convention.
...
| bin/claude-assist | root | Launcher script — starts API + Web/TUI, zellij-aware pane layout, port health-check |
...
| Launcher | bash (`bin/claude-assist`, zellij-aware) |
