...
export function runRecentCommand(args: string[]): number {
  try {
    ...
    console.error(`claude-assist recent: ${error instanceof Error ? error.message : String(error)}`);
    return 1;
  }
}
...
export function queryRecentSessions(dbPath: string, since: Date, limit: number): RecentSession[] {
  ...
  try {
    ...
    throw new Error(`index not found at ${dbPath}; run \`claude-assist\` once to build it`);
  }
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes("no such table")) {
      throw new Error(`index at ${dbPath} is not initialized; run \`claude-assist\` once`);
    }
    throw error;
  } finally {
    db.close();
...
function recentHelp(): string {
  return `Usage: claude-assist recent [period] [options]
