//! Port of Sources/Queue/QueueManifest.swift — file list copied verbatim.
...
pub fn manifest_text() -> String {
    ...
    let mut sections: Vec<(&str, Vec<&QueueFile>)> = Vec::new();
    sections.push((
        "TOP-LEVEL",
        FILES.iter().filter(|f| !f.path.contains('/')).collect(),
    ));
    for (prefix, label) in groups {
        let group: Vec<&QueueFile> = FILES.iter().filter(|f| f.path.starts_with(prefix)).collect();
        if !group.is_empty() {
            sections.push((label, group));
        }
    }
    ...
    for (label, group) in sections {
        ...
        for file in group {
            lines.push(format!("- {} — {}", file.path, file.description));
        }
...
mod tests {
    ...
    #[test]
    fn valid_paths() {
        assert!(is_valid_path("binlog.jsonl"));
        assert!(is_valid_path("ideas/tools.jsonl"));
        assert!(!is_valid_path("ideas/nonexistent.jsonl"));
        assert!(!is_valid_path("../etc/passwd"));
    }

    #[test]
    fn manifest_text_sections() {
        let text = manifest_text();
        assert!(text.starts_with("TOP-LEVEL:"));
        assert!(text.contains("IDEAS:"));
        assert!(text.contains("- writing/subjects.jsonl — Subject matter for writing projects"));
    }
}
