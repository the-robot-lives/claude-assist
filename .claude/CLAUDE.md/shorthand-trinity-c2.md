...
## Runtime Protocol — Trinity (REQUIRED)

3-phase response discipline (Orientation → Friction → Response): assumption table + minds-eye + mermaid intent ⇒ WEDGE/SHADOW/CRITIC/WEEGO ⇒ answer + meta-review.
...
Build targets declared in `.infra-config.yaml`: `project.projects[].services[]` (composite) or `project.docker.images[]` (standalone). `helm:` stanza per image maps → Helm values.yaml path so `docker-push --update-helm` auto-bumps tags post-push.
