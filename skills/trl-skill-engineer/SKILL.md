...
|-----------|---------|----------|
| `INTRODUCTION.md` | Consumer contract read *before* SKILL.md: I/O declarations, conventions, reading order | Yes |
...
| `scripts/` | Reserved for future automation | Yes (create empty) |

> For the INTRODUCTION.md contract format, see [references/introduction-specification.md](references/introduction-specification.md).
...
|------|----------|---------|
| `INTRODUCTION.md` | root | Consumer I/O contract, read first |
| `agent-playbook.claude-code.md` | `references/` | Agent role definition + execution workflows |
| At least one `worked-example-*.md` | `references/` | End-to-end demonstration of the skill in action |
| `project-tracker.md` | `assets/` | Progress tracking template |
...
1. Check for `NPLLoad` or `NPLSpec` in available MCP tools (primary — this is the real signal)
2. Check whether the NPL source is present at `projects/NoizuPromptLingo/` (secondary)
3. If neither: NPL is not installed — offer to explain benefits only if the skill design would significantly benefit

**Do not use `$NPL_PROJECT` / `$NPL_ORG` as detection signals** — those name the current
project/organization for the npl MCP session services (tobor), and are set in repos that
have no NPL prompt-lingua tooling at all.
...
**Adjacent meta-skills (know the boundary — see NOT-clauses below):**
- **trl-skill-evaluator** — Runs task-flow, dialogue, and exam-based *evals* against a finished skill. skill-engineer *builds and self-checks*; skill-evaluator *tests at depth across scenarios/models*. Hand off after scaffolding.
- **trl-agent-architect** — Designs standalone agent/subagent definitions (`.claude/agents/*.md`). Use it when the deliverable is an *agent*, not a *skill module*.
...
> **Trigger boundary:** if the user wants to *test/score/regress* a skill across scenarios or models → **trl-skill-evaluator**. If they want to *build or audit the skill module itself* → this skill.

## Bundled Resources

### Root

- [INTRODUCTION.md](INTRODUCTION.md) — Consumer I/O contract for this skill (read first)
...
- [prompt-engineering-patterns.md](references/patterns/prompt-engineering-patterns.md) — 31 prompt engineering techniques catalog (reasoning, verification, interaction, safety, optimization) with academic sources
