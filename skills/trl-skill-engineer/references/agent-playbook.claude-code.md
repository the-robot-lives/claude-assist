...
> Alternate agent-executable version of trl-skill-engineer operational workflows. Designed for Claude Code to run the full skill lifecycle: interactive discovery, architecture design, scaffold generation, quality auditing, and MCP tool selection. This complements the human-facing SKILL.md entry point — it's a parallel execution layer optimized for tool-assisted workflows.
...
steps:
  ...
  - id: design-file-tree
    ...
    format: |
      ...
      ├── INTRODUCTION.md                       # Consumer I/O contract (read first)
      ├── references/
  ...
  - id: draft-section-outline
    ...
    required_frontmatter: YAML frontmatter (name + trigger description) — required, but counted separately from the 11 sections
    required_sections:
      - H1 Title
      - One-line subtitle (after H1)
  ...
  - id: draft-trigger-language
    ...
    description: >
      ...
      Check that the description is 3-8 lines — shorter descriptions miss
      triggers; longer ones dilute precision.
    pattern: |
      name: {skill-name}
      description: >
...
steps:
  ...
  - id: generate-introduction
    action: write
    description: >
      Generate INTRODUCTION.md at the skill root following
      references/introduction-specification.md. Include: frontmatter
      (skill, version, compatible_with, last_updated), a one-paragraph
      summary, the input contract (what the skill expects from its
      consumer), the output contract (what it produces), conventions
      the consumer must follow, and the recommended reading order for
      the bundled files.
  ...
  - id: verify-scaffold
    ...
    checklist:
      ...
      - "INTRODUCTION.md present with valid contract blocks"
...
steps:
  ...
  - id: structural-compliance-check
    ...
    description: >
      ...
      Mark each as present, missing, or present-but-empty. Assign the
      structural compliance sub-score using the rubric's qualitative bands
      (9-10 fully compliant / 7-8 minor gaps / 5-6 significant gaps — see
      assets/skill-scoring-rubric.md).
    ...
    required_files:
      ...
      - "scripts/ directory"
    required_frontmatter: YAML frontmatter with name and description fields (required, counted separately from the 11 sections)
    required_sections:
      - H1 Title
      - One-line subtitle (after H1)
      - Overview section
  ...
  - id: trigger-language-evaluation
    ...
    description: >
      ...
      recall by running 10 test scenarios: 5 that should trigger the skill,
      5 that should not (matching the quality checklist's "5+ matching and
      5+ non-matching" requirement). For each false-positive scenario, check if any
    ...
    test_scenarios:
      ...
      - type: should_trigger
        description: "Secondary trigger from the 'Also trigger when' clause"
      - type: should_trigger
        description: "Edge-of-scope scenario that still belongs to this skill"
      ...
      - type: should_not_trigger
        description: "Request naming the domain but in a different context"
      - type: should_not_trigger
        description: "Anti-scope scenario from the discovery brief"
      - type: should_not_trigger
        description: "Generic request that no specialized skill should claim"
  ...
  - id: run-quality-checklist
    action: evaluate
    description: >
      Run references/quality-checklist.md against the skill and tally the
      result out of 65 items. Apply the gate bands: 59+ of 65 = PASS,
      54-58 = CONDITIONAL (ship only with documented follow-ups),
      below 54 = FAIL. Report the checklist tally alongside the weighted
      rubric score — the checklist gates structural compliance; the rubric
      scores quality depth. Both gates must pass to ship.
    gate: "59/65 PASS | 54-58 CONDITIONAL | <54 FAIL"
...
steps:
  ...
  - id: search-for-new-options
    ...
    queries:
      ...
      - "[domain] MCP Claude tool <current year> OR <previous year>"
