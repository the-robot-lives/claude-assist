...
| Playwright MCP | Local stdio | Browser automation, screenshots, JS execution, accessibility snapshots | Runs locally; official Microsoft MCP (`@playwright/mcp`) | Stable (official) |
| Puppeteer MCP | Local stdio | Browser automation, screenshots, JS execution | Reference server archived by Anthropic in 2025 | Archived — prefer Playwright MCP |
...
- **When to avoid**: When you need real-time search (not just crawl). When the target site blocks crawlers aggressively and you need full browser JS execution (use Playwright MCP instead).
...
**JS-heavy SPAs or need screenshots:** Use Playwright MCP (Microsoft, `@playwright/mcp`). Runs locally, official, full browser control. Puppeteer MCP is archived — prefer Playwright.
