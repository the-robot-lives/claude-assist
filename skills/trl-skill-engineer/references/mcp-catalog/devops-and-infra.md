...
- **Deployment**: Local stdio via the `terraform-mcp-server` binary or Docker image (HashiCorp ships it as a Go binary / Docker image, not an npm package); connects to local Terraform CLI and state backend (local files, S3, Terraform Cloud, HCP Terraform); optionally connects to HCP Terraform API for remote workspace management
...
- **Deployment**: Local stdio; `npx mcp-server-kubernetes` (Flux159) or `containers/kubernetes-mcp-server` — verify the current package name before install — or Docker; reads `~/.kube/config` or `KUBECONFIG` env var; cluster context is set before invoking the MCP server (agent should not switch contexts autonomously)
...
| `aws` | AWS installer / OS package manager (CLI v2) | AWS API via CLI | Resource inspection + automation |
...
| `wrangler` | `npm i -g wrangler` | Cloudflare Workers deploy + KV | Edge compute deployment |
