...
| Codex CLI | Local stdio (Rust) | Native OS sandbox, MCP client, minimal footprint | Sandboxed by default | Stable |
| Goose | Local stdio | 70+ MCP extensions, prompt injection detection | Active injection defense | Beta |
| Gemini CLI | Local stdio | 1M context, free tier, Google Search grounding | Google account required | Stable |
...
- **Key features**: `.cursorrules` project conventions; Background Agents for long-running tasks; Design Mode for Figma-to-code; multi-file edit with inline diff review; model-switchable (GPT-5, Claude 5, Gemini)
...
| `codex` | `npm i -g @openai/codex` / brew | Rust CLI, native sandbox, MCP client | Sandboxed automation |
