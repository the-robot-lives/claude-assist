...
```
skills/{skill-name}/
├── INTRODUCTION.md                       # Consumer I/O contract (read first) — see introduction-specification.md
...
- [ ] `INTRODUCTION.md` exists with I/O contract (see `introduction-specification.md`)
- [ ] SKILL.md has all 11 required sections
- [ ] YAML frontmatter has `name` and a **concise** `description` (3–8 lines) with trigger language
- [ ] `name` field matches directory name exactly (shared `trl-` prefix included on both)
