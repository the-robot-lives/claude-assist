...
└── scripts/          # Reserved for future automation (create empty)
```
...
## Skills in the Monorepo

Skills live under `skills/` in this monorepo. They are versioned with the repo (not a
git submodule — this repo uses subtrees, per the root `CLAUDE.md`). This layout enables:
- A single, predictable path (`skills/{name}/`) every agent and tool can rely on
- Shared assets under `skills/shared/` reused across skills
- Focused contribution and review per-skill

When adding a new skill, add it under `skills/` and register nothing else — Claude Code's
native skill detection reads the SKILL.md frontmatter directly.

## Cross-Harness Compatibility

Skills should be portable across agent harnesses, not just Claude Code. The `compatible_with`
field in INTRODUCTION.md declares which are supported; write SKILL.md so it degrades cleanly:

All three CLI harnesses natively route on SKILL.md folders — same anatomy, different edges:

| Harness | Invocation | Skills root | Notes |
|---------|-----------|-------------|-------|
| **Claude Code** (Anthropic) | `/{skill-name}` + description auto-trigger | `~/.claude/skills/` | Full support: file system, tools, sub-agents |
| **Codex** (OpenAI) | Description auto-trigger | `~/.codex/skills/` (`.system/` reserved) | Recommends `agents/openai.yaml` UI metadata; strict conciseness doctrine |
...
- Nothing load-bearing in frontmatter beyond `name` + `description` (only fields all routers read)
- Adopt Grok's name rule universally — it's the tightest, so a Grok-valid name works everywhere
- Harness-specific features (Claude Code tools, NPL, MCP wiring) live in `references/`, never in SKILL.md
- SKILL.md never breaks or errors if references are unavailable

> For full per-harness deltas, `agents/openai.yaml`, and the portable authoring checklist, see
