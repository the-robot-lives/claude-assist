...
\* The ~80-line target governs the *narrative* entry section (intent → trigger → quick-start →
decision tree). **Catalog** and **meta** archetypes (e.g. trl-skill-engineer itself) legitimately
run longer because their SKILL.md is mostly *index tables* pointing into references/ — that's
navigation, not inlined content. The rule to enforce is "no full methodology inlined," not a raw
line count. If SKILL.md prose (not tables) exceeds ~80 lines, move it to references/.
...
Skills should run across harnesses — Claude Code, Codex (OpenAI), Grok (xAI), and generic
LLMs — not just Claude Code. Declare support in INTRODUCTION.md's `compatible_with`.

| Feature | Claude Code | Claude Teams | Codex CLI | Grok / Generic LLM |
|---------|------------|-------------|-----------|--------------------|
| File system access | Yes | No | Yes | No |
| Tool calls (bash, edit) | Yes | Limited | Yes | No |
| Skill routing | Slash commands | Instructions | Manual (description-guided) | System prompt |
| Multi-agent spawning | Yes | No | Limited | No |
| Memory/context persistence | Session | None | Session | None |

The lowest common denominator is Grok/Generic: SKILL.md must be plain Markdown any model can
read, with no harness-specific syntax. Anything requiring tools or file access is marked
`> **Requires:** ...` and isolated in `references/` so a routing-only harness can skip it.
...
**Description**
- [ ] Concise — 3–8 lines. Lead with the capability, then triggers/keywords; trim filler
- [ ] Reads cleanly in a routing-only harness (Codex/Grok) that sees only the description

**SKILL.md**
- [ ] Narrative entry section under ~80 lines (index tables may extend it — see Layered Architecture)
