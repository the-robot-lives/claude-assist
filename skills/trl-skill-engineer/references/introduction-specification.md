...
skill: {skill-name}
version: "1.0"
compatible_with:          # list every harness the skill is portable to
  - claude-code
  - claude-teams          # if SKILL.md works standalone
  - codex                 # OpenAI Codex CLI — if no Claude-only syntax/tools in SKILL.md
  - grok                  # xAI Grok — if SKILL.md is plain, routing-agnostic Markdown
