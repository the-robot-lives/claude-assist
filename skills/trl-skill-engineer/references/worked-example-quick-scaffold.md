...
**Score: 5 / 8** (dimensions score whole points — complete counts, partial does not)

The unified rule: 6+ of 8 dimensions → fast-path scaffold; below 6 → run targeted discovery on the gaps. At 5/8 the gaps are small and answerable in a single exchange, so proceed with targeted gap-filling rather than the full discovery interview.
...
| Meta | Teaches a practice or generates other skills | Weak — produces changelogs, not skills or practice guidance |
...
Note: `name:` and `description:` are the only frontmatter keys. Trigger and anti-trigger vocabulary is folded into the description itself; anti-scope (version bumping, git tag management, semantic versioning decisions) lives in the Scope section below. The "git-and-github" reference is not a skill — it is the trl-skill-engineer's MCP catalog category file (references/mcp-catalog/git-and-github.md), consulted for tool selection during scaffolding rather than cross-referenced from frontmatter.
...
2. **Create INTRODUCTION.md** — write the consumer I/O contract per `references/introduction-specification.md`: frontmatter (skill, version, compatible_with, last_updated), summary, input contract, output contract, conventions, and reading order.

3. **Fill agent-playbook.claude-code.md** — write the actual agent instructions for each workflow phase. The stub has section headers; the user adds the procedural detail.

4. **Write commit-conventions.md** — document the exact conventional commit format supported (Commitizen? Angular? custom?), edge cases, and how to handle non-conventional commits in the repo.

5. **Write output-formats.md** — specify the exact schema for JSON output and the RSS feed structure. Markdown can reference keepachangelog.com directly.

6. **Create a realistic worked example** — the stub at `worked-example-react-library.md` needs a real scenario: a specific repo, a realistic git log excerpt, and the generated changelog output so users can see the skill in action.

7. **Run the quality audit** — use `references/quality-checklist.md` from the trl-skill-engineer skill to verify the scaffold is complete before publishing.
