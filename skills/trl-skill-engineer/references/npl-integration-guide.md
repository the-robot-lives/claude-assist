...
NPL (Noizu Prompt Lingua) is a modular, structured framework for advanced prompt engineering and agent simulation. It lives at `projects/NoizuPromptLingo/` in the repo.
...
**Secondary method**: check whether the NPL source is present at `projects/NoizuPromptLingo/`.

**Do NOT use `$NPL_PROJECT` or `$NPL_ORG` as detection signals.** Those variables name the active project/org for the npl MCP session services (tobor) and are set in repos that have no NPL prompt-lingua installation at all.

If neither method succeeds, NPL is not installed. Proceed without it.
...
- id: check-npl
  ...
  check: Is NPLLoad available as an MCP tool? If not, does projects/NoizuPromptLingo/ exist?
...
```
projects/NoizuPromptLingo/npl/npl-full.md
```
