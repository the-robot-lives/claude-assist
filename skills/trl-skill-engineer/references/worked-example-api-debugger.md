...
**Completeness score: 2/8** — only domain is recognizable. This is a full discovery situation. Proceed to architecture at 6+ of 8 with any remaining gaps explicitly approved by the user; 8/8 is the ideal.
...
**User:** Cross-references: the MCP catalog's testing-and-qa category (this skill's references/mcp-catalog/testing-and-qa.md, since we might recommend a test tool to reproduce the issue), and maybe trl-seo-guru since we're thinking about public API documentation. For quality — a good debug response should classify the error, give a probable root cause, suggest a concrete fix, and include diagnostic commands I can actually run.
...
| Cross-Refs | MCP catalog testing-and-qa category, trl-seo-guru |
...
**Strategy** — advisory decision framework. Ruled out. Debugging is operational execution, not strategic advice.

**Meta** — teaches a practice or generates other skills. Ruled out. This skill fixes APIs; it doesn't teach skill-building or generate artifacts about the practice itself.
...
- `INTRODUCTION.md` — required at the skill root; states the input/output contract so consumers know what to provide and expect before opening SKILL.md.
...
Length check: the final description is 7 lines — within the required 3–8 line range. Shorter descriptions miss triggers; longer ones dilute precision.
...
Note on headings: this excerpt uses shorthand headings. In the full SKILL.md they map onto the canonical 11-section layout — "Quality Baseline" is part of the Core Content sections, and scope/anti-scope material lands under "When to Use This Skill." The full file also includes the H1 + one-line subtitle, Core Philosophy, cross-reference blockquotes, Quick Start Guides, Reference Guide, Related Skills, and Bundled Resources.
...
Note: only `name:` and `description:` belong in the frontmatter. The other discovery outputs live in the document body instead: the workflow archetype and draft status go in the Overview, cross-references (trl-seo-guru; the MCP catalog's testing-and-qa category) become advisory blockquotes, tool usage (browser-tools MCP, GitHub MCP) lives in `references/tool-integration.md`, and audience, constraints, and anti-scope are documented in the When to Use / scope sections.
...
| Cross-Reference Accuracy | 10% | 8 | 0.80 | trl-seo-guru and the MCP catalog testing-and-qa category are correctly identified; no phantom refs |
