...
**Length rule:** Keep the description 3–8 lines (comfortably under the 1024-character frontmatter limit). Lead with capability, then triggers, then keywords.
...
description: >
  ...
  implementation across web, terminal, and specification formats. Use this
  skill when the user wants to design a web app, create a landing page, build
  UI components, wireframe an interface, run an accessibility audit, generate
  Next.js or HTML/CSS code, create SVG mockups or logos, or hand off specs to
  developers — even if they don't say "UX" or "design." Also trigger when users
  mention wireframes, mockups, design systems, WCAG, Figma specs, or logotypes.
```
...
- **WHEN (explicit):** Eight distinct scenarios, covering the full range from "design a web app" to "hand off specs to developers"
  - Longer list than most skills because the surface area is wide — but each item is a distinct user goal, and the whole description stays within the 3–8 line rule.
...
- **Also trigger when:** Six keywords spanning tools (Figma), standards (WCAG), deliverables (wireframes, mockups), and terminology (logotypes)
