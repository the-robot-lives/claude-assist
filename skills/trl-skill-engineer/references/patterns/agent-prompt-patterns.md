...
**Skill integration:** Map to Claude Code's memory system — Tier 1 is context, Tier 2 is conversation compression, Tier 3 is `~/.claude/projects/<project-slug>/memory/`, Tier 4 is `docs/` or external retrieval.
...
- Limit tool count to ~10 per agent (beyond that, use a router) — or use deferred tool loading (e.g. ToolSearch) and MCP tool filtering, which support large tool surfaces without a router; a router remains valid for cross-agent routing
