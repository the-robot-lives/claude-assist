...
# App secrets (/observability/oneuptime) -> observability-app-secrets.
# Keys: ONEUPTIME_SECRET, ONEUPTIME_ENCRYPTION_SECRET, ONEUPTIME_DB_USER,
#   ONEUPTIME_DB_PASSWORD, ONEUPTIME_CLICKHOUSE_PASSWORD.
resource "kubectl_manifest" "infisical_app_secrets" {
  yaml_body = yamlencode({
    apiVersion = "secrets.infisical.com/v1alpha1"
    kind       = "InfisicalSecret"
    metadata = {
      name      = "infisical-observability-secrets"
      namespace = local.ns
      labels    = local.common_labels
    }
    spec = {
