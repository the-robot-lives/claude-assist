defmodule Therobotplans.Media do
  alias Therobotplans.Media.Asset, as: Entity
  alias Therobotplans.Schema.Media.Asset, as: Schema
  use Noizu.Repo
  ...
  def get_cached_variant(media_id, canonical_params) do
    import Ecto.Query
    Therobotplans.Repo.one(
      from v in Therobotplans.Schema.Media.Variant,
        where: v.media_id == ^media_id and v.params == ^canonical_params
    )
  end

  def cache_variant(attrs) do
    %Therobotplans.Schema.Media.Variant{}
    |> Therobotplans.Schema.Media.Variant.changeset(attrs)
    |> Therobotplans.Repo.insert(on_conflict: :nothing)
  end

  def fetch_from_s3(key) do
    config = Application.get_env(:therobotplans, Therobotplans.Storage, [])
  ...
  def get_or_create_variant(media, transform_params) do
    canonical = Therobotplans.Media.Transform.canonical_params(transform_params)
      ...
      nil ->
        with {:ok, original_binary} <- fetch_from_s3(media.file),
             {:ok, transformed, content_type} <-
               Therobotplans.Media.Transform.transform(original_binary, transform_params) do
          variant_key = Therobotplans.Media.Transform.variant_s3_key(media.file, transform_params)
