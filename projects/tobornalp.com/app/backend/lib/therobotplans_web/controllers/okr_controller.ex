defmodule TherobotplansWeb.OkrController do
  ...
  def index(conn, %{"org_id" => org_id} = params) do
    ...
    with {:ok, _} <- Authz.authorize(user_id, "organization", org_id, "viewer") do
      opts =
        [owner_id: params["owner_id"], level: params["level"], status: params["status"],
  ...
  def create(conn, %{"org_id" => org_id, "objective" => params}) do
    ...
    with {:ok, _} <- Authz.authorize(user_id, "organization", org_id, "member") do
      ...
      case Goals.create_objective(attrs) do
        {:ok, o} -> conn |> put_status(:created) |> json(%{objective: objective_json(o)})
  ...
  def update(conn, %{"org_id" => _org_id, "id" => id, "objective" => attrs}) do
    case Goals.update_objective(id, attrs) do
      {:ok, o} -> json(conn, %{objective: objective_json(o)})
      {:error, :not_found} -> conn |> put_status(:not_found) |> json(%{error: "Objective not found"})
      {:error, cs} -> conn |> put_status(:unprocessable_entity) |> json(%{errors: format_errors(cs)})
    end
  end
  ...
  def create_key_result(conn, %{"id" => objective_id, "key_result" => params}) do
    ...
    case Goals.create_key_result(attrs) do
      {:ok, kr} -> conn |> put_status(:created) |> json(%{key_result: kr_json(kr)})
      {:error, cs} -> conn |> put_status(:unprocessable_entity) |> json(%{errors: format_errors(cs)})
    end
  end
  ...
  def create_checkin(conn, %{"id" => objective_id, "checkin" => params}) do
    user_id = get_user_id(conn)
    attrs = Map.merge(params, %{"objective_id" => objective_id, "author_id" => params["author_id"] || user_id})

    case Goals.create_checkin(attrs) do
      {:ok, c} -> conn |> put_status(:created) |> json(%{checkin: %{id: c.id, body: c.body, period: c.period}})
      {:error, cs} -> conn |> put_status(:unprocessable_entity) |> json(%{errors: format_errors(cs)})
    end
  end
  ...
  defp objective_json(o) do
    %{id: o.id, title: o.title, level: o.level, status: o.status, period: o.period,
      owner_id: o.owner_id, organization_id: o.organization_id, project_id: o.project_id}
  end
  ...
  defp objective_detail_json(o) do
    Map.merge(objective_json(o), %{
      ...
      checkins: Enum.map(o.checkins || [], &%{id: &1.id, body: &1.body, period: &1.period, inserted_at: &1.inserted_at})
    })
  end

  defp kr_json(kr) do
    %{id: kr.id, objective_id: kr.objective_id, title: kr.title, target_value: kr.target_value,
      current_value: kr.current_value, auto_progress: kr.auto_progress, status: kr.status, due_on: kr.due_on}
  end
