defmodule TherobotplansWeb.AuthController do
  ...
  def register(conn, %{"invite_token" => raw_token, "user" => user_params}) do
    with {:ok, invite} <- Organizations.find_active_invite_by_raw_token(raw_token),
      ...
      Therobotplans.Events.dispatch(:user_registered, %{user_id: user.id, email: user.email})
  ...
  def request_magic_link(conn, %{"email" => email}) do
    ...
    case Therobotplans.Auth.SmartTokenAuth.request_magic_link(email) do
      {:ok, %{encoded_key: encoded_key, user: _user}} ->
        magic_link = "#{frontend_url}/auth/verify?token=#{encoded_key}"
        Therobotplans.Auth.SmartTokenEmail.send_magic_link(email, magic_link)

        response = %{message: "If an account exists with that email, a magic link has been sent."}

        response =
          if Application.get_env(:therobotplans, :dev_routes) do
  ...
  def verify_magic_link(conn, %{"token" => token_key}) do
    case Therobotplans.Auth.SmartTokenAuth.verify_magic_link(token_key, conn) do
      {:ok, session} ->
        {:ok, access_token, _} =
          Guardian.encode_and_sign(session, %{}, token_type: "access", ttl: {1, :hour})
  ...
  def request_otp_login(conn, %{"email" => email}) do
    case Therobotplans.Auth.SmartTokenAuth.request_otp_login(email) do
      {:ok, %{otp_code: otp_code, user: _user}} ->
        Therobotplans.Auth.SmartTokenEmail.send_otp_login(email, otp_code)
  ...
  def verify_otp_login(conn, %{"email" => email, "code" => code}) do
    case Therobotplans.Auth.SmartTokenAuth.verify_otp_login(email, code, conn) do
      {:ok, session} ->
        {:ok, access_token, _} =
          Guardian.encode_and_sign(session, %{}, token_type: "access", ttl: {1, :hour})
  ...
  def request_password_reset(conn, %{"email" => email}) do
    case Therobotplans.Auth.SmartTokenAuth.request_password_reset(email) do
      {:ok, %{otp_code: otp_code, user: _user}} ->
        Therobotplans.Auth.SmartTokenEmail.send_password_reset(email, otp_code)
  ...
  def refresh(conn, %{"refresh_token" => refresh_token}) do
    alias Therobotplans.Auth.TokenStore
  ...
  def send_verification(conn, _params) do
    ...
    case Therobotplans.Auth.SmartTokenAuth.request_email_verification(user.email) do
      {:ok, %{encoded_key: encoded_key}} ->
        verification_link = "#{frontend_url}/auth/verify-email?token=#{encoded_key}"
        Therobotplans.Auth.SmartTokenEmail.send_verification_email(user.email, verification_link)
  ...
  def verify_email(conn, %{"token" => token_key}) do
    case Therobotplans.Auth.SmartTokenAuth.verify_email_token(token_key) do
      {:ok, user} ->
        Therobotplans.Events.dispatch(:user_verified, %{user_id: user.id})
