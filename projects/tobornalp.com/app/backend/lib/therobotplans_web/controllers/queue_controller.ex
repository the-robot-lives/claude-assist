defmodule TherobotplansWeb.QueueController do
  ...
  def index(conn, %{"org_id" => org_id} = params) do
    ...
    with {:ok, _} <- Authz.authorize(user_id, "organization", org_id, "viewer") do
      ...
      json(conn, %{queues: Enum.map(boards, &queue_summary/1), methodologies: ItemQueue.methodologies()})
    else
  ...
  def create(conn, %{"org_id" => org_id, "queue" => params}) do
    ...
    with {:ok, _} <- Authz.authorize(user_id, "organization", org_id, "member") do
      ...
      case Queues.create(attrs) do
        {:ok, board} -> conn |> put_status(:created) |> json(%{queue: queue_detail(board)})
  ...
  def update(conn, %{"org_id" => org_id, "id" => id, "queue" => params}) do
    ...
    with {:ok, _} <- Authz.authorize(user_id, "organization", org_id, "member") do
      case Queues.update_board(id, params) do
        {:ok, board} -> json(conn, %{queue: queue_detail(board)})
        {:error, :not_found} -> conn |> put_status(:not_found) |> json(%{error: "Queue not found"})
        {:error, changeset} -> conn |> put_status(:unprocessable_entity) |> json(%{errors: format_errors(changeset)})
      end
    else
      err -> handle_error(conn, err)
  ...
  defp queue_summary(q) do
    %{id: q.id, name: q.name, slug: q.slug, methodology: q.methodology,
      organization_id: q.organization_id, project_id: q.project_id}
  end
  ...
  defp queue_detail(q) do
    Map.merge(queue_summary(q), %{
      ...
      config: q.config,
      stages: Enum.map(q.stages || [], fn s ->
        %{id: s.id, slug: s.slug, name: s.name, kind: s.kind, position: s.position, wip_limit: s.wip_limit}
      end),
      iterations: Enum.map(q.iterations || [], fn i ->
        %{id: i.id, name: i.name, sequence: i.sequence, status: i.status, starts_on: i.starts_on, ends_on: i.ends_on}
      end)
    })
  end
