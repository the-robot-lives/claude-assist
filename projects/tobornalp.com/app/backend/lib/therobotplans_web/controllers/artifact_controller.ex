defmodule TherobotplansWeb.ArtifactController do
  ...
  def create(conn, %{"org_id" => org_id, "artifact" => artifact_params}) do
    ...
    with {:ok, _} <- Authz.authorize(user_id, "organization", org_id, "member"),
         {:ok, project_id} <- validate_project(artifact_params["project_id"], org_id) do
      attrs = %{
        organization_id: org_id,
  ...
  def show(conn, %{"org_id" => org_id, "id" => id} = params) do
    with_org_artifact(conn, org_id, id, "viewer", fn artifact ->
      revision =
        if params["revision_id"] do
          elem(Artifacts.get(id, params["revision_id"]), 1)
        else
          elem(Artifacts.get(id), 1)
        end
  ...
  def index_revisions(conn, %{"org_id" => org_id, "artifact_id" => artifact_id}) do
    with_org_artifact(conn, org_id, artifact_id, "viewer", fn artifact ->
  ...
  # `Artifacts.add_revision/3` auto-increments the revision number; we return the
  # artifact with its new current revision so the editor reconciles in one round-trip.
  # member role; org-scoped + artifact-belongs-to-org guard.
  def create_revision(conn, %{"org_id" => org_id, "artifact_id" => artifact_id} = params) do
    with_org_artifact(conn, org_id, artifact_id, "member", fn artifact ->
  ...
  # Resolve org, authorize, load the artifact, ensure it belongs to the org.
  defp with_org_artifact(conn, org_id, id, role, fun) do
    user_id = get_user_id(conn)

    with {:ok, _} <- Authz.authorize(user_id, "organization", org_id, role),
         {artifact, _revision} when not is_nil(artifact) <- Artifacts.get(id) || :missing,
         true <- artifact.organization_id == org_id do
      fun.(artifact)
    else
