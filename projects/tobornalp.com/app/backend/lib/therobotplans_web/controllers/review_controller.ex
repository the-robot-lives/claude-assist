defmodule TherobotplansWeb.ReviewController do
  use TherobotplansWeb, :controller

  alias Therobotplans.Domains.Reviews
  ...
  def create(conn, %{"org_id" => org_id, "review" => review_params}) do
    ...
    with {:ok, _} <- Authz.authorize(user_id, "organization", org_id, "member"),
         {:ok, project_id} <- validate_project(review_params["project_id"], org_id),
         :ok <- validate_artifact(review_params["artifact_id"], org_id) do
      attrs = %{
        organization_id: org_id,
  ...
  def show(conn, %{"org_id" => org_id, "id" => id}) do
    with_org_review(conn, org_id, id, "viewer", fn review ->
      {^review, comments, overlays} = Reviews.get(id)
  ...
  def update(conn, %{"org_id" => org_id, "id" => id, "review" => review_params}) do
    with_org_review(conn, org_id, id, "member", fn _review ->
  ...
  # Resolve org, authorize, load the review, ensure it belongs to the org.
  defp with_org_review(conn, org_id, id, role, fun) do
    user_id = get_user_id(conn)
    ...
    with {:ok, _} <- Authz.authorize(user_id, "organization", org_id, role),
         ...
         true <- review.organization_id == org_id do
      fun.(review)
    else
  ...
  defp validate_artifact(nil, _org_id), do: :ok
  defp validate_artifact("", _org_id), do: :ok

  defp validate_artifact(artifact_id, org_id) do
    case Therobotplans.Repo.get(Therobotplans.Schema.Artifact, artifact_id) do
