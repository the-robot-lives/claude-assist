defmodule TherobotplansWeb.Plugs.RequireAdmin do
  @behaviour Plug
  import Plug.Conn
  ...
  defp get_user(conn) do
    case Therobotplans.Guardian.Plug.current_resource(conn) do
      %Therobotplans.Users.Sessions.UserSession{user: {:ref, _, id}} ->
        case Therobotplans.Repo.get(Therobotplans.Schema.Users.User, id) do
          nil -> :error
          user -> {:ok, user}
        end

      %Therobotplans.Users.Sessions.UserSession{user: %{id: id}} ->
        case Therobotplans.Repo.get(Therobotplans.Schema.Users.User, id) do
          nil -> :error
          user -> {:ok, user}
        end
