defmodule Therobotplans.Authz.Policies do
  alias Therobotplans.Authz.Policies.Policy, as: Entity
  alias Therobotplans.Schema.Authz.Policy, as: Schema
  alias Therobotplans.Schema.Authz.UserPolicy, as: UserPolicySchema
  ...
  def list_active(opts \\ []) do
    ...
    Therobotplans.Repo.all(query)
  end

  def get_by_name(name) do
    Therobotplans.Repo.one(from p in Schema, where: p.name == ^name and p.is_active == true)
  end
    ...
    case Therobotplans.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          policy |> Schema.changeset(attrs) |> Therobotplans.Repo.update()
        end
    end
  end
    ...
    case Therobotplans.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          Therobotplans.Repo.delete(policy)
        end
    end
  end
  ...
  def detach_from_user(user_id, policy_id) do
    case Therobotplans.Repo.one(
      ...
      nil -> {:error, :not_found}
      user_policy -> Therobotplans.Repo.delete(user_policy)
    end
  end
end
