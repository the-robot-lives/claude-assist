defmodule Therobotplans.Domains.Notifications.Tools.Poll do
  use Noizu.MCP.Server.Tool,
    name: "Notifications.Poll",
    description:
      "Monitor for notifications: like Notifications.Get, but if nothing is deliverable it BLOCKS until a notification arrives (returning instantly) or wait_ms elapses, instead of returning empty. Run this in a Monitor loop so the agent is woken the moment something arrives rather than polling on a timer. Honors the same per-recipient rate-limit.",
    hidden: true,
  ...
  alias Therobotplans.Domains.Notifications.Tools.Format
