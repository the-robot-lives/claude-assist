defmodule Therobotplans.Domains.Notifications.Tools.Notify do
  use Noizu.MCP.Server.Tool,
    name: "Notify",
    description:
      "Send a short direct message (<=128 chars) to one or more recipients. Target a user (recipient) or a list of users (recipients). Set ping:true to send a ping (body optional); the recipient should reply with a pong via Notify pong_to:<notification_id>.",
    hidden: true,
  ...
  input do
    ...
    field :sender, :string, required: true, description: "Sender handle"
  ...
  def call(args, _ctx) do
    ...
    with {:scope, {:ok, org_id, project_id}} <- {:scope, Resolve.scope(org_ref, project_ref)},
      ...
      case Notifications.notify(attrs) do
        ...
        {:error, :no_recipients} ->
          {:error, "No recipients resolved — provide recipient or recipients"}
