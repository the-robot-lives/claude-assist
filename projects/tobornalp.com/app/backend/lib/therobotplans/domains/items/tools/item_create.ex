defmodule Therobotplans.Domains.Items.Tools.ItemCreate do
  ...
  input_schema(%{
    ...
    "properties" => %{
      ...
      "project" => %{
        "type" => "string",
        "description" => "Optional project slug or UUID to scope this item to"
      },
  ...
  def call(args, _ctx) do
    ...
    with {:org, org_id} when not is_nil(org_id) <- {:org, Resolve.organization_id(org_ref)},
      ...
      attrs =
        extract(
          args,
          ~w(title description item_type status priority assignee reporter queue_id parent_id custom_fields)
        )
