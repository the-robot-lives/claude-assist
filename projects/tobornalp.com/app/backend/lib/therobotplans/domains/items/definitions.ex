defmodule Therobotplans.Domains.Items.Definitions do
  @moduledoc """
  Item field and type definitions, scoped at three levels:
  ...
  alias Therobotplans.Schema.{ItemFieldDefinition, ItemTypeDefinition, ItemTypeField}
  ...
  def update_field(id, attrs) do
    case Repo.get(ItemFieldDefinition, id) do
      nil -> {:error, :not_found}
      field -> field |> ItemFieldDefinition.changeset(attrs) |> Repo.update()
    end
  end
  ...
  def upsert_field(attrs) do
    ...
    case get_field_in_scope(org_id, project_id, slug) do
      ...
      existing -> existing |> ItemFieldDefinition.changeset(attrs) |> Repo.update()
    end
  end
  ...
  def get_type(id) do
    ...
    |> preload(type_fields: :item_field_definition)
    |> Repo.one()
  end
  ...
  def update_type(id, attrs) do
    case get_type(id) do
      nil -> {:error, :not_found}
      type_def -> type_def |> ItemTypeDefinition.changeset(attrs) |> Repo.update()
    end
  end
  ...
  def upsert_type(attrs) do
    ...
    case get_type_in_scope(org_id, project_id, slug) do
      ...
      existing -> existing |> ItemTypeDefinition.changeset(attrs) |> Repo.update()
    end
  end
  ...
  def add_field_to_type(type_id, field_id, opts \\ []) do
    %ItemTypeField{}
    |> ItemTypeField.changeset(%{
      item_type_definition_id: type_id,
      item_field_definition_id: field_id,
