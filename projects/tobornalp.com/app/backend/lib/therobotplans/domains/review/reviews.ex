defmodule Therobotplans.Domains.Reviews do
  @moduledoc """
  Code/content reviews over an artifact revision. Org-required, project-optional.

  A review targets a specific artifact revision (immutable lineage: artifact_id +
  revision_id fixed at create). The reviewer leaves positional overlay annotations
  (review_overlays) and, on completion, a verdict. Review comments reuse the
  polymorphic `Therobotplans.Services.Comment` (entity_type = "review") — no
  dedicated review_comments table exists.
  """
  import Ecto.Query, except: [update: 2]
  alias Therobotplans.Repo
  alias Therobotplans.Schema.{Review, ReviewOverlay}
  ...
  def get(review_id) do
    case Repo.get(Review, review_id) do
      ...
      review ->
        comments = Therobotplans.Services.Comment.list("review", review_id)
