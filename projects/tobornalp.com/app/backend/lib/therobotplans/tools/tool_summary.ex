defmodule Therobotplans.Tools.ToolSummary do
  ...
  alias Therobotplans.Tools.Catalog

  @impl true
  def call(args, ctx) do
    filter = Map.get(args, :filter) || Map.get(args, "filter")
    server = (ctx && ctx.server) || Therobotplans.MCP
      ...
      result =
        if direct != [] do
          Map.put(
            result,
            :tools,
            Enum.map(direct, fn d -> %{name: d.name, description: d.description} end)
          )
        else
          result
        end
