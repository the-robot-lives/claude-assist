defmodule Therobotplans.Tools.Catalog do
  ...
  def build(server \\ Therobotplans.MCP, ctx \\ nil) do
  ...
  def specs(server \\ Therobotplans.MCP, ctx \\ nil) do
    cond do
      not is_atom(server) ->
        []

      function_exported?(server, :catalog_specs, 1) ->
        apply(server, :catalog_specs, [ctx])

      true ->
        # The server macro generates __mcp__/1, but Kernel.function_exported?/3
        # can report false for macro-generated functions depending on the beam
        # load state — so probe defensively and fall back to the empty catalog
        # only when the call genuinely fails.
        case safe_mcp_tools(server) do
          nil -> []
          tools -> Noizu.MCP.Server.Features.Tools.expand(tools)
        end
    end
  end

  defp safe_mcp_tools(server) do
  ...
  def get_tool(name, server \\ Therobotplans.MCP, ctx \\ nil) do
    name = resolve_alias(name)
    Enum.find(build(server, ctx), &(&1.name == name))
  end

  def get_tools_by_category(category, server \\ Therobotplans.MCP, ctx \\ nil) do
    Enum.filter(build(server, ctx), &(&1.category == category))
  end

  def categories(server \\ Therobotplans.MCP, ctx \\ nil) do
    build(server, ctx)
  ...
  def call_hidden_tool(name, arguments, server \\ Therobotplans.MCP, ctx \\ nil) do
    name = resolve_alias(name)
    specs = specs(server, ctx)
