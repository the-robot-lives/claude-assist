defmodule Therobotplans.MCP.Resolve do
  @moduledoc """
  ...
  alias Therobotplans.Schema.Organizations.Organization, as: OrgSchema
  alias Therobotplans.Schema.Projects.Project, as: ProjectSchema
  ...
  def organization_id(ref) when is_binary(ref) do
    cond do
      match?({:ok, _}, Ecto.UUID.cast(ref)) ->
        Therobotplans.Repo.one(from(o in OrgSchema, where: o.id == ^ref, select: o.id))

      true ->
        Therobotplans.Repo.one(from(o in OrgSchema, where: o.slug == ^ref, select: o.id))
    end
  end
  ...
  def organization(ref) do
    case organization_id(ref) do
      nil -> nil
      id -> Therobotplans.Repo.get(OrgSchema, id)
    end
  end
  ...
  def project(ref) do
    case Ecto.UUID.cast(ref) do
      {:ok, uuid} ->
        Therobotplans.Repo.get(ProjectSchema, uuid) ||
          Therobotplans.Repo.get_by(ProjectSchema, slug: ref)
