...
if System.get_env("PHX_SERVER") do
  config :aifighter, AifighterWeb.Endpoint, server: true
end

config :aifighter, :redis,
  uri: System.get_env("REDIS_URL") || "redis://localhost:6379/0",
  key_prefix: System.get_env("REDIS_KEY_PREFIX", "aifighter:")
...
if otel_endpoint = System.get_env("OTEL_EXPORTER_OTLP_ENDPOINT") do
  ...
  config :opentelemetry,
    ...
    resource: %{
      "service.name" => System.get_env("OTEL_SERVICE_NAME") || "aifighter-backend",
...
if config_env() == :prod do
  ...
  config :aifighter, :dns_cluster_query, System.get_env("DNS_CLUSTER_QUERY")
  ...
  config :aifighter, :frontend_url, System.get_env("FRONTEND_URL")
  ...
  config :aifighter, :mail_from,
    {System.get_env("MAIL_FROM_NAME", "Aifighter"),
     System.get_env("MAIL_FROM_ADDRESS", "noreply@aifighter.local")}

  # ── Storage (S3/MinIO) ──────────────────────────────────────────
  if s3_bucket = System.get_env("S3_BUCKET") do
    config :aifighter, Aifighter.Storage,
      bucket: s3_bucket,
  ...
  if oidc_client_id = System.get_env("OIDC_CLIENT_ID") do
    ...
    config :aifighter, :oidc_enabled, true
  end
  ...
  if saml_metadata = System.get_env("SAML_IDP_METADATA_URL") do
    ...
    config :aifighter, :saml_enabled, true
  end

  # ── SSO: Social OAuth (each enabled when *_CLIENT_ID is set) ──
  config :aifighter, :sso_require_invite, System.get_env("SSO_REQUIRE_INVITE") == "true"
  ...
  oauth_providers =
    if google_id = System.get_env("GOOGLE_CLIENT_ID") do
      ...
      config :aifighter, :google_enabled, true
  ...
  oauth_providers =
    if fb_id = System.get_env("FACEBOOK_CLIENT_ID") do
      ...
      config :aifighter, :facebook_enabled, true
  ...
  oauth_providers =
    if gh_id = System.get_env("GITHUB_CLIENT_ID") do
      ...
      config :aifighter, :github_enabled, true
  ...
  #     config :aifighter, :linkedin_enabled, true
