defmodule AifighterWeb.MediaServeController do
  use AifighterWeb, :controller

  alias Aifighter.Media
  alias Aifighter.Media.Transform
  ...
  defp serve_original(conn, media) do
    if media.visibility == "public" do
      url = Aifighter.Storage.presigned_download_url(media.file)
  ...
  defp serve_transformed(conn, media, transform_params) do
    case Media.get_or_create_variant(media, transform_params) do
      {:ok, variant_key, content_type} ->
        if media.visibility == "public" do
          url = Aifighter.Storage.presigned_download_url(variant_key)
  ...
  defp get_user_id(conn) do
    try do
      case Plug.Conn.get_req_header(conn, "authorization") do
        ["Bearer " <> token] ->
          case Aifighter.Guardian.decode_and_verify(token) do
            {:ok, claims} ->
              case Aifighter.Guardian.resource_from_claims(claims) do
                {:ok, %Aifighter.Users.Sessions.UserSession{user: {:ref, _, id}}} -> id
                {:ok, %Aifighter.Users.Sessions.UserSession{user: %{id: id}}} -> id
                _ -> nil
              end
