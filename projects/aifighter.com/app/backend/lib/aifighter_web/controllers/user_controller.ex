defmodule AifighterWeb.UserController do
  use AifighterWeb, :controller

  alias Aifighter.Guardian
  alias Aifighter.Users
  ...
  defp validate_update_params(user, params) do
    if params["new_password"] && params["current_password"] do
      ...
      q =
        from c in Aifighter.Schema.Users.Credentials.UserCredential,
  ...
  defp apply_updates(user, params, _context) do
    ...
    if map_size(updates) > 0 do
      ...
      |> Aifighter.Repo.update_all(set: Enum.to_list(updates))
    end

    if params["new_password"] do
      Aifighter.Users.Credentials.update_password(user, params["new_password"], Noizu.Context.system())
    end
