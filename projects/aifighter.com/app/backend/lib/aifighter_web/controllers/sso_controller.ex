defmodule AifighterWeb.SSOController do
  ...
  def exchange(conn, %{"code" => code}) do
    with {:ok, session_id} <- Aifighter.Auth.SSOCode.exchange(code),
         {:ok, session} <- Aifighter.Users.Sessions.get(session_id, Noizu.Context.system()),
  ...
  defp handle_sso_callback(conn, provider_type, attrs) do
    ...
    case Aifighter.Auth.SSO.authenticate_sso(provider_type, attrs) do
      {:ok, session} ->
        {:ok, code} = Aifighter.Auth.SSOCode.create(session.id)
  ...
  defp maybe_add(list, flag, name) do
    if Application.get_env(:aifighter, flag), do: [name | list], else: list
  end
