defmodule AifighterWeb.Plugs.RequireAdmin do
  @behaviour Plug
  import Plug.Conn
  ...
  defp get_user(conn) do
    case Aifighter.Guardian.Plug.current_resource(conn) do
      %Aifighter.Users.Sessions.UserSession{user: {:ref, _, id}} ->
        case Aifighter.Repo.get(Aifighter.Schema.Users.User, id) do
          nil -> :error
          user -> {:ok, user}
        end

      %Aifighter.Users.Sessions.UserSession{user: %{id: id}} ->
        case Aifighter.Repo.get(Aifighter.Schema.Users.User, id) do
          nil -> :error
          user -> {:ok, user}
        end
