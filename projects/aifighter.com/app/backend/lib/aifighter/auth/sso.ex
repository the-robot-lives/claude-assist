defmodule Aifighter.Auth.SSO do
  alias Aifighter.Schema.Users.User, as: UserSchema
  alias Aifighter.Schema.Users.Credentials.UserCredential, as: CredentialSchema
  alias Aifighter.Schema.Versioned.Names.Name
  import Ecto.Query, only: [from: 2]

  @provider_map %{
    oidc: &Aifighter.Auth.Providers.oidc/0,
    saml: &Aifighter.Auth.Providers.saml/0,
    google: &Aifighter.Auth.Providers.google/0,
    facebook: &Aifighter.Auth.Providers.facebook/0,
    github: &Aifighter.Auth.Providers.github/0,
    linkedin: &Aifighter.Auth.Providers.linkedin/0
  }
  ...
  def authenticate_sso(provider_type, %{email: email} = attrs) do
    ...
    {:ok, provider_id} = Aifighter.Auth.Providers.Provider.id(provider_ref)
    ...
    case find_user_by_email(email) do
      ...
      :not_found ->
        if Application.get_env(:aifighter, :sso_require_invite, false) do
  ...
  defp ensure_sso_credential(user, provider_ref, provider_id, provider_type, attrs, context) do
    ...
    case Aifighter.Repo.one(q) do
      nil ->
        %Aifighter.Users.Credentials.UserCredential{
          user: Aifighter.Users.User.ref(user.id),
  ...
  defp create_sso_session(user, provider_type, context) do
    ...
    %Aifighter.Users.Sessions.UserSession{
      user: user_ref,
      status: :active,
  ...
  defp auto_provision_user(email, attrs, provider_ref, provider_id, provider_type, context) do
    ...
    {:ok, name} =
      Aifighter.EntityRepo.create(
        %Aifighter.Versioned.Names.Name{first: first, last: last, time_stamp: Noizu.Entity.TimeStamp.now()},
        context
      )
    ...
    {:ok, user} = Aifighter.Repo.insert(user_schema, on_conflict: :nothing, conflict_target: :email)
