defmodule Aifighter.Media do
  alias Aifighter.Media.Asset, as: Entity
  alias Aifighter.Schema.Media.Asset, as: Schema
  use Noizu.Repo
  ...
  def get_cached_variant(media_id, canonical_params) do
    import Ecto.Query
    Aifighter.Repo.one(
      from v in Aifighter.Schema.Media.Variant,
        where: v.media_id == ^media_id and v.params == ^canonical_params
    )
  end

  def cache_variant(attrs) do
    %Aifighter.Schema.Media.Variant{}
    |> Aifighter.Schema.Media.Variant.changeset(attrs)
    |> Aifighter.Repo.insert(on_conflict: :nothing)
  end

  def fetch_from_s3(key) do
    config = Application.get_env(:aifighter, Aifighter.Storage, [])
  ...
  def get_or_create_variant(media, transform_params) do
    canonical = Aifighter.Media.Transform.canonical_params(transform_params)
      ...
      nil ->
        with {:ok, original_binary} <- fetch_from_s3(media.file),
             {:ok, transformed, content_type} <-
               Aifighter.Media.Transform.transform(original_binary, transform_params) do
          variant_key = Aifighter.Media.Transform.variant_s3_key(media.file, transform_params)
