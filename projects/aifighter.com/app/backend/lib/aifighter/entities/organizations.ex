defmodule Aifighter.Organizations do
  @moduledoc """
  Context for Aifighter.Organizations
  """
  alias Aifighter.Organizations.Organization, as: Entity
  alias Aifighter.Schema.Organizations.Organization, as: Schema
  alias Aifighter.Schema.Organizations.InviteToken, as: InviteTokenSchema
  alias Aifighter.Schema.Authz.ScopedMembership, as: ScopedMembershipSchema
  use Noizu.Repo
  ...
  def create_organization_with_owner(attrs, user_id) do
    Aifighter.Repo.transaction(fn ->
      with {:ok, org} <- %Schema{} |> Schema.changeset(attrs) |> Aifighter.Repo.insert(),
           {:ok, _membership} <- Aifighter.Authz.ScopedMemberships.add_member("organization", org.id, user_id, "owner") do
        org
      else
  ...
  def list_user_organizations(user_id) do
    from(sm in ScopedMembershipSchema,
      ...
      join: g in Aifighter.Schema.Authz.Group, on: g.id == sm.group_id,
  ...
  def authorize(user_id, organization_id, required_role) do
    Aifighter.Authz.authorize(user_id, "organization", organization_id, required_role)
  end

  def list_members(organization_id) do
    Aifighter.Authz.ScopedMemberships.list_for_resource("organization", organization_id)
  end
  ...
  def increment_invite_uses(invite_token) do
    ...
    |> Aifighter.Repo.update_all(inc: [uses: 1])
  end
end
