defmodule Aifighter.Users.Credentials do
  @moduledoc """
  Context for Aifighter.Users.Credentials
  """
  alias Aifighter.Users.Credentials.UserCredential, as: Entity
  alias Aifighter.Schema.Users.Credentials.UserCredential, as: Schema
  use Noizu.Repo
  import Ecto.Query, only: [from: 2]

  def_repo(entity: Aifighter.Users.Credentials.UserCredential)
  ...
  def register(user, {:login, {email, password}}, context, _options) do
    {:ok, auth_provider} = Aifighter.Auth.Providers.login()

    {:ok, description} =
      %Aifighter.Versioned.Strings.String{
        content: "",
  ...
  def authenticate({:login, {email, password}}, context, options) do
    {:ok, auth_provider} = Aifighter.Auth.Providers.login()
    {:ok, auth_provider_id} = Aifighter.Auth.Providers.Provider.id(auth_provider)
        ...
        [credential] ->
          if Bcrypt.verify_pass(password, credential.settings["password"]) do
            with {:ok, credential_entity} <-
                   Aifighter.Users.Credentials.UserCredential.entity(credential.id, context),
              ...
              |> Aifighter.EntityRepo.create(context, options)
            end
          else
