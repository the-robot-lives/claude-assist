defmodule Aifighter.Authz.Policies do
  alias Aifighter.Authz.Policies.Policy, as: Entity
  alias Aifighter.Schema.Authz.Policy, as: Schema
  alias Aifighter.Schema.Authz.UserPolicy, as: UserPolicySchema
  ...
  def list_active(opts \\ []) do
    ...
    Aifighter.Repo.all(query)
  end

  def get_by_name(name) do
    Aifighter.Repo.one(from p in Schema, where: p.name == ^name and p.is_active == true)
  end
    ...
    case Aifighter.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          policy |> Schema.changeset(attrs) |> Aifighter.Repo.update()
        end
    end
  end
  ...
  def detach_from_user(user_id, policy_id) do
    case Aifighter.Repo.one(
      from up in UserPolicySchema,
        where: up.user_id == ^user_id and up.policy_id == ^policy_id
    ) do
      nil -> {:error, :not_found}
      user_policy -> Aifighter.Repo.delete(user_policy)
    end
  end
end
