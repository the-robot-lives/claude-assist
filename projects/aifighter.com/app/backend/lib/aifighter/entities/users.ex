defmodule Aifighter.Users do
  @moduledoc """
  Context for Aifighter.Users
  """
  alias Aifighter.Users.User, as: Entity
  alias Aifighter.Schema.Users.User, as: Schema
  use Noizu.Repo
  def_repo(entity: Aifighter.Users.User)

  def list(context, options \\ []) do
    settings = Noizu.Entity.Meta.persistence(Entity) |> hd

    Aifighter.Repo.all(Schema)
    |> Enum.map(fn record ->
  ...
  def register(details, {:login, {email, password}} = auth, context, options) do
    ...
    email = Aifighter.Users.Credentials.standardize_email(email)
    ...
    with :valid <- valid_user_name?(details.user_name),
      ...
      with {:ok, name_entity} <-
             %Aifighter.Versioned.Names.Name{
               first: name.first,
               middle: name.middle,
               last: name.last,
               time_stamp: Noizu.Entity.TimeStamp.now()
             }
             |> Aifighter.EntityRepo.create(context),
           {:ok, name_ref} <- Noizu.EntityReference.Protocol.ref(name_entity),
           {:ok, description} <-
             %Aifighter.Versioned.Descriptions.Description{
               title: "Description",
           ...
           {:ok, credential} <-
             Aifighter.Users.Credentials.register(user_ref, auth, context, options) do
        {:ok, {user, credential}}
      end
    end
  end
  ...
  def authenticate(auth = {:login, {_email, _password}}, context, options \\ nil) do
    with {:ok, session} <- Aifighter.Users.Credentials.authenticate(auth, context, options) do
      {:ok, session}
    end
  end

  def by_handle(handle, context, options \\ []) do
    with record = %Schema{} <- Aifighter.Repo.get_by(Schema, %{handle: handle}) do
      settings = Noizu.Entity.Meta.persistence(Entity) |> hd
      {:ok, entity} = Entity.from_record(record, settings, context, options)
  ...
  def user_name_available?(user_name, _context, _options \\ nil) do
    with %{} <- Aifighter.Repo.get_by(Schema, %{user_name: user_name}) do
        ...
        fn
          ...
          {"name", value} ->
            case user.name do
              %Ecto.Changeset{} ->
                value =
                  Aifighter.Versioned.Names.change_versioned_name(user.name.data, value)

                {:name, value}

              _ ->
                value =
                  Aifighter.Versioned.Names.change_versioned_name(
                    user.name || %Aifighter.Versioned.Names.Name{},
                    value
                  )
      ...
      |> put_in(
        ...
        {:embed,
         %{
           ...
           on_cast: :update,
           on_replace: :update,
           related: Aifighter.Versioned.Names.Name
         }}
      )
  ...
  def valid_login?(email, password) do
    Aifighter.Users.Credentials.valid_login?(email, password)
  end

  def login_available?(email, context, options \\ nil) do
    Aifighter.Users.Credentials.login_available?(email, context, options)
  end
  ...
  defp unique_handle(handle, context, options) do
    case Aifighter.Users.by_handle(handle, context, options) do
      nil ->
        {:ok, handle}
      ...
      {:ok, _} ->
        Enum.reduce_while(0..999, handle, fn suffix, _acc ->
          ...
          case Aifighter.Users.by_handle(with_suffix, context, options) do
            nil ->
              {:halt, {:ok, with_suffix}}
