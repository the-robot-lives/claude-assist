...
| # | Proposal | What it kills | Primary win | Brain analogy | planning.md anchor |
|---|---|---|---|---|---|
| [001](001-sundered-backprop-local-learning.md) | **Sundered Backprop** | The global backward pass | Training wall-clock via module-parallel local learning | Regional plasticity; no global error broadcast | §D cleave points as training contracts; §C feedback as teacher |
...
| [003](003-multi-timescale-state-backbone.md) | **Multi-Timescale State Backbone** | Attention-over-history & the KV cache | O(L) training, O(1)-per-token decoding, flat latency at any context | Nested neural timescales; executive vs background | §B promoted to backbone; §A absorbed as the glacial bank |
...
| [005](005-frozen-core-accretion.md) | **Frozen-Core Accretion** | Retraining the monolith | 10³–10⁴× cheaper marginal capability; zero forgetting | Neurogenesis, habit formation, sleep consolidation | §7 recipe promoted to the whole training economy |
| [006](006-neuron-lifecycle-senescence-and-neurogenesis.md) | **The Neuron Lifecycle** | Fixed topology (static width) | Capacity that tracks the task; continual learning without forgetting | Neurogenesis, synaptic pruning, cellular senescence, sleep | 005 at neuron granularity + continuous; §A gates birth rate, §B carries age/vitality |

## Coverage of the brief

- **Faster training:** 001 (wall-clock parallelism), 003 (O(L) sequence scaling), 005 (marginal-cost collapse), 006 (adapter-cost births, right-sized nets)
...
- **Growth & mortality (node senescence/creation):** 006 (neuron lifecycle), with 005 as its coarse module-level counterpart
...
003 multi-timescale SSM core          ← the substrate (time arrow built in)
  ...
  lifecycle via 005 frozen-core accretion ← capabilities accrete; sleep consolidates
    at neuron grain, 006 lifecycle     ← units are born, senesce, and retire within modules
```
...
- **006 + 002:** event-driven execution is what makes over-capacity affordable — idle/senescent masked slots don't fire, so they cost ~nothing until reclaimed. Load-bearing for each other.
- **006 + 005:** same offline "sleep" cycle runs both — memory consolidation, shim distillation, *and* the neuron census (birth/retire/recompact) in one pass. 006 is 005's binary freeze-and-graft made continuous and fine-grained.
...
| M3 neuromodulator | **002** pilot (feed-forward blocks only) once **m** exists |
...
| M7 memory | Add **005**'s consolidation daemon behind the salience gate; fold **006**'s census into the same "sleep" pass (offline-only turnover first) |
