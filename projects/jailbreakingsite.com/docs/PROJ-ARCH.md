...
The architecture is intentionally minimal: a single-page static export with no backend, no database, and no authentication. The only external integration is the foryou signup service (foryou.therobotlives.com), which the waitlist form POSTs to directly from the browser for double opt-in email signups.
...
graph LR
    ...
    subgraph External
        FY[foryou signup service<br/>foryou.therobotlives.com]
    end

    User -->|HTTPS| CF
    CF -->|origin pull| NG
    NG --> SA
    User -.->|POST /api/v1/public/lists/jailbreaking-waitlist/signups| FY
```
...
| `web/src/app/waitlist-form.tsx` | Client component — email signup via foryou signup service |
...
|-------------|----------|---------|
| foryou | HTTPS POST (public API) | Waitlist email subscription (double opt-in) |
...
- **Why foryou over SaaS**: Self-hosted, portfolio-wide signup service on existing infrastructure. No vendor lock-in, no per-subscriber costs, full data ownership, and shared double opt-in handling across all Noizu properties.
- **Why no API backend**: Pre-launch phase — the only dynamic behavior is client-side form submission to the external foryou signup service. Backend services will be added when Catalog/Defender/Academy features are built.
