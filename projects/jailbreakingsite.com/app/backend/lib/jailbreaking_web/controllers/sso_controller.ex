defmodule JailbreakingWeb.SSOController do
  ...
  def exchange(conn, %{"code" => code}) do
    with {:ok, session_id} <- Jailbreaking.Auth.SSOCode.exchange(code),
         {:ok, session} <- Jailbreaking.Users.Sessions.get(session_id, Noizu.Context.system()),
  ...
  defp handle_sso_callback(conn, provider_type, attrs) do
    ...
    case Jailbreaking.Auth.SSO.authenticate_sso(provider_type, attrs) do
      {:ok, session} ->
        {:ok, code} = Jailbreaking.Auth.SSOCode.create(session.id)
  ...
  defp maybe_add(list, flag, name) do
    if Application.get_env(:jailbreaking, flag), do: [name | list], else: list
  end
end
