defmodule JailbreakingWeb.UserController do
  use JailbreakingWeb, :controller

  alias Jailbreaking.Guardian
  alias Jailbreaking.Users
  ...
  defp validate_update_params(user, params) do
    if params["new_password"] && params["current_password"] do
      ...
      q =
        from c in Jailbreaking.Schema.Users.Credentials.UserCredential,
  ...
  defp apply_updates(user, params, _context) do
    ...
    if map_size(updates) > 0 do
      ...
      |> Jailbreaking.Repo.update_all(set: Enum.to_list(updates))
    end

    if params["new_password"] do
      Jailbreaking.Users.Credentials.update_password(user, params["new_password"], Noizu.Context.system())
    end
