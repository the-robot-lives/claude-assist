defmodule JailbreakingWeb.AdminController do
  use JailbreakingWeb, :controller

  alias Jailbreaking.Schema.Users.User, as: UserSchema
  alias Jailbreaking.Schema.Organizations.Organization, as: OrgSchema
  import Ecto.Query
  ...
  def list_organizations(conn, params) do
    ...
    total = Jailbreaking.Repo.aggregate(OrgSchema, :count, :id)

    conn |> put_status(:ok) |> json(%{organizations: orgs, total: total, page: page, per_page: per_page})
  end

  def show_organization(conn, %{"id" => id}) do
    case Jailbreaking.Repo.get(OrgSchema, id) do
      nil ->
        conn |> put_status(:not_found) |> json(%{error: "Organization not found"})

      org ->
        members = Jailbreaking.Organizations.list_members(org.id)
