defmodule JailbreakingWeb.Router do
  use JailbreakingWeb, :router
  ...
  pipeline :authenticated do
    plug JailbreakingWeb.AuthPipeline
  end
  ...
  pipeline :rate_limited_auth do
    plug JailbreakingWeb.Plugs.RateLimit, action: :auth
  end

  pipeline :rate_limited_sensitive do
    plug JailbreakingWeb.Plugs.RateLimit, action: :auth_sensitive
  end

  pipeline :org_viewer do
    plug JailbreakingWeb.Plugs.RequireRole, role: "viewer"
  end

  pipeline :org_editor do
    plug JailbreakingWeb.Plugs.RequireRole, role: "editor"
  end

  pipeline :org_admin do
    plug JailbreakingWeb.Plugs.RequireRole, role: "admin"
  end

  pipeline :org_owner do
    plug JailbreakingWeb.Plugs.RequireRole, role: "owner"
  end

  pipeline :admin do
    plug JailbreakingWeb.Plugs.RequireAdmin
  end

  scope "/", JailbreakingWeb do
    pipe_through :api
    get "/health", HealthController, :index
  end

  scope "/api/v1", JailbreakingWeb do
  ...
  scope "/media", JailbreakingWeb do
    pipe_through [:api]
  ...
  if Application.compile_env(:jailbreaking, :saml_enabled) do
    scope "/sso/saml" do
  ...
  # OIDC redirect flow
  scope "/auth/oidc", JailbreakingWeb do
    pipe_through [:sso_session]
  ...
  if Application.compile_env(:jailbreaking, :dev_routes) do
    import Phoenix.LiveDashboard.Router

    scope "/dev" do
      pipe_through [:fetch_session, :protect_from_forgery]
      live_dashboard "/dashboard", metrics: JailbreakingWeb.Telemetry
    end
  end
end
