defmodule Jailbreaking.Auth.SSO do
  alias Jailbreaking.Schema.Users.User, as: UserSchema
  alias Jailbreaking.Schema.Users.Credentials.UserCredential, as: CredentialSchema
  alias Jailbreaking.Schema.Versioned.Names.Name
  import Ecto.Query, only: [from: 2]

  @provider_map %{
    oidc: &Jailbreaking.Auth.Providers.oidc/0,
    saml: &Jailbreaking.Auth.Providers.saml/0,
    google: &Jailbreaking.Auth.Providers.google/0,
    facebook: &Jailbreaking.Auth.Providers.facebook/0,
    github: &Jailbreaking.Auth.Providers.github/0,
    linkedin: &Jailbreaking.Auth.Providers.linkedin/0
  }
  ...
  def authenticate_sso(provider_type, %{email: email} = attrs) do
    ...
    {:ok, provider_id} = Jailbreaking.Auth.Providers.Provider.id(provider_ref)
    ...
    case find_user_by_email(email) do
      ...
      :not_found ->
        if Application.get_env(:jailbreaking, :sso_require_invite, false) do
  ...
  defp find_user_by_email(email) do
    ...
    case Jailbreaking.Repo.one(q) do
      nil -> :not_found
      user -> {:ok, user}
    end
  end
  ...
  defp ensure_sso_credential(user, provider_ref, provider_id, provider_type, attrs, context) do
    ...
    case Jailbreaking.Repo.one(q) do
      nil ->
        %Jailbreaking.Users.Credentials.UserCredential{
          user: Jailbreaking.Users.User.ref(user.id),
  ...
  defp create_sso_session(user, provider_type, context) do
    ...
    %Jailbreaking.Users.Sessions.UserSession{
      user: user_ref,
      status: :active,
  ...
  defp auto_provision_user(email, attrs, provider_ref, provider_id, provider_type, context) do
    ...
    {:ok, name} =
      Jailbreaking.EntityRepo.create(
        %Jailbreaking.Versioned.Names.Name{first: first, last: last, time_stamp: Noizu.Entity.TimeStamp.now()},
        context
      )
    ...
    {:ok, user} = Jailbreaking.Repo.insert(user_schema, on_conflict: :nothing, conflict_target: :email)
