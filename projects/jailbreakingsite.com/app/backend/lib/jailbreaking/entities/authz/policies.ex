defmodule Jailbreaking.Authz.Policies do
  alias Jailbreaking.Authz.Policies.Policy, as: Entity
  alias Jailbreaking.Schema.Authz.Policy, as: Schema
  alias Jailbreaking.Schema.Authz.UserPolicy, as: UserPolicySchema
  ...
  def list_active(opts \\ []) do
    ...
    Jailbreaking.Repo.all(query)
  end

  def get_by_name(name) do
    Jailbreaking.Repo.one(from p in Schema, where: p.name == ^name and p.is_active == true)
  end
    ...
    case Jailbreaking.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          policy |> Schema.changeset(attrs) |> Jailbreaking.Repo.update()
        end
    end
  end
    ...
    case Jailbreaking.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          Jailbreaking.Repo.delete(policy)
        end
    end
  end
  ...
  def detach_from_user(user_id, policy_id) do
    case Jailbreaking.Repo.one(
      from up in UserPolicySchema,
        where: up.user_id == ^user_id and up.policy_id == ^policy_id
    ) do
      nil -> {:error, :not_found}
      user_policy -> Jailbreaking.Repo.delete(user_policy)
    end
  end
end
