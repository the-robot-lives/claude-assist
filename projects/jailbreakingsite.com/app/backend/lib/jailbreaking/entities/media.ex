defmodule Jailbreaking.Media do
  alias Jailbreaking.Media.Asset, as: Entity
  alias Jailbreaking.Schema.Media.Asset, as: Schema
  use Noizu.Repo
  ...
  def get_cached_variant(media_id, canonical_params) do
    import Ecto.Query
    Jailbreaking.Repo.one(
      from v in Jailbreaking.Schema.Media.Variant,
        where: v.media_id == ^media_id and v.params == ^canonical_params
    )
  end

  def cache_variant(attrs) do
    %Jailbreaking.Schema.Media.Variant{}
    |> Jailbreaking.Schema.Media.Variant.changeset(attrs)
    |> Jailbreaking.Repo.insert(on_conflict: :nothing)
  end

  def fetch_from_s3(key) do
    config = Application.get_env(:jailbreaking, Jailbreaking.Storage, [])
  ...
  def get_or_create_variant(media, transform_params) do
    canonical = Jailbreaking.Media.Transform.canonical_params(transform_params)
      ...
      nil ->
        with {:ok, original_binary} <- fetch_from_s3(media.file),
             {:ok, transformed, content_type} <-
               Jailbreaking.Media.Transform.transform(original_binary, transform_params) do
          variant_key = Jailbreaking.Media.Transform.variant_s3_key(media.file, transform_params)
