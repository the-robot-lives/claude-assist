defmodule Jailbreaking.Users.Credentials do
  @moduledoc """
  Context for Jailbreaking.Users.Credentials
  """
  alias Jailbreaking.Users.Credentials.UserCredential, as: Entity
  alias Jailbreaking.Schema.Users.Credentials.UserCredential, as: Schema
  use Noizu.Repo
  import Ecto.Query, only: [from: 2]

  def_repo(entity: Jailbreaking.Users.Credentials.UserCredential)
  ...
  def register(user, {:login, {email, password}}, context, _options) do
    {:ok, auth_provider} = Jailbreaking.Auth.Providers.login()

    {:ok, description} =
      %Jailbreaking.Versioned.Strings.String{
        content: "",
  ...
  def authenticate({:login, {email, password}}, context, options) do
    {:ok, auth_provider} = Jailbreaking.Auth.Providers.login()
    {:ok, auth_provider_id} = Jailbreaking.Auth.Providers.Provider.id(auth_provider)
    ...
    with :valid <- valid_login?(email, password) do
      ...
      case Jailbreaking.Repo.all(q) do
        [] ->
        ...
        [credential] ->
          if Bcrypt.verify_pass(password, credential.settings["password"]) do
            with {:ok, credential_entity} <-
                   Jailbreaking.Users.Credentials.UserCredential.entity(credential.id, context),
              ...
              |> Jailbreaking.EntityRepo.create(context, options)
            end
          else
