defmodule Jailbreaking.Organizations do
  @moduledoc """
  Context for Jailbreaking.Organizations
  """
  alias Jailbreaking.Organizations.Organization, as: Entity
  alias Jailbreaking.Schema.Organizations.Organization, as: Schema
  alias Jailbreaking.Schema.Organizations.InviteToken, as: InviteTokenSchema
  alias Jailbreaking.Schema.Authz.ScopedMembership, as: ScopedMembershipSchema
  use Noizu.Repo
  ...
  def create_organization_with_owner(attrs, user_id) do
    Jailbreaking.Repo.transaction(fn ->
      with {:ok, org} <- %Schema{} |> Schema.changeset(attrs) |> Jailbreaking.Repo.insert(),
           {:ok, _membership} <- Jailbreaking.Authz.ScopedMemberships.add_member("organization", org.id, user_id, "owner") do
        org
      else
        {:error, reason} -> Jailbreaking.Repo.rollback(reason)
      end
    end)
  end
  ...
  def list_user_organizations(user_id) do
    from(sm in ScopedMembershipSchema,
      ...
      join: g in Jailbreaking.Schema.Authz.Group, on: g.id == sm.group_id,
    ...
    |> Jailbreaking.Repo.all()
  end

  def authorize(user_id, organization_id, required_role) do
    Jailbreaking.Authz.authorize(user_id, "organization", organization_id, required_role)
  end

  def list_members(organization_id) do
    Jailbreaking.Authz.ScopedMemberships.list_for_resource("organization", organization_id)
  end
  ...
  def increment_invite_uses(invite_token) do
    ...
    |> Jailbreaking.Repo.update_all(inc: [uses: 1])
  end
end
