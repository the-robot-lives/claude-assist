...
if System.get_env("PHX_SERVER") do
  config :timely, TimelyWeb.Endpoint, server: true
end

config :timely, :redis,
  uri: System.get_env("REDIS_URL") || "redis://localhost:6379/0",
  key_prefix: System.get_env("REDIS_KEY_PREFIX", "timely:")
...
if otel_endpoint = System.get_env("OTEL_EXPORTER_OTLP_ENDPOINT") do
  ...
  config :opentelemetry,
    ...
    resource: %{
      "service.name" => System.get_env("OTEL_SERVICE_NAME") || "timely-backend",
...
if config_env() == :prod do
  ...
  config :timely, :dns_cluster_query, System.get_env("DNS_CLUSTER_QUERY")
  ...
  config :timely, :frontend_url, System.get_env("FRONTEND_URL")
  ...
  config :timely,
         :mail_from,
         {System.get_env("MAIL_FROM_NAME", "Timely"),
          System.get_env("MAIL_FROM_ADDRESS", "noreply@timely.local")}

  # ── Storage (S3/MinIO) ──────────────────────────────────────────
  if s3_bucket = System.get_env("S3_BUCKET") do
    config :timely, Timely.Storage,
      bucket: s3_bucket,
  ...
  # ── Verified deep links (App Links / Universal Links) ──────────
  #
  # Served at /.well-known/assetlinks.json and
  # /.well-known/apple-app-site-association. These carry SIGNING CERTIFICATE
  # fingerprints, which are ops values from the release keystore - they rotate
  # independently of the app and differ between debug and release builds, so
  # they come from the environment rather than being baked into the image.
  #
  #   ANDROID_APP_PACKAGE=com.noizu.timely
  #   ANDROID_APP_FINGERPRINTS=AA:BB:...:FF          (comma-separated SHA-256)
  #   IOS_APP_IDS=TEAMID.com.noizu.timely            (comma-separated)
  #
  # Unset means the endpoint answers 404, which leaves verification unattempted
  # and lets an App Link fall back to the disambiguation dialog - safe to stage
  # before the fingerprint exists.
  config :timely, :android_app_package, System.get_env("ANDROID_APP_PACKAGE")

  config :timely,
         :android_app_fingerprints,
         (System.get_env("ANDROID_APP_FINGERPRINTS") || "")
         |> String.split(",", trim: true)
         |> Enum.map(&String.trim/1)

  config :timely,
         :ios_app_ids,
         (System.get_env("IOS_APP_IDS") || "")
         |> String.split(",", trim: true)
         |> Enum.map(&String.trim/1)

  # ── SSO: native redirect allow-list ────────────────────────────
  #
  # Where an SSO flow is permitted to hand back its one-time code. The code is
  # traded for an access + refresh token pair, so an unlisted target is a
  # token-exfiltration vector, not merely an open redirect - hence an
  # allow-list rather than a reflected `redirect_uri`.
  #
  # Comma-separated. The web target (/auth/sso-callback) is always allowed and
  # does not need listing. Native entries are exact strings, e.g.
  #   SSO_REDIRECT_ALLOWLIST=com.noizu.timely://auth/callback,https://timely.noizu.com/auth/native-callback
  config :timely,
         :sso_redirect_allowlist,
         (System.get_env("SSO_REDIRECT_ALLOWLIST") || "")
         |> String.split(",", trim: true)
         |> Enum.map(&String.trim/1)
  ...
  if oidc_client_id = System.get_env("OIDC_CLIENT_ID") do
    ...
    config :timely, :oidc_enabled, true
  end
    ...
    config :samly, Samly.Provider,
      ...
      identity_providers: [
        %{
          ...
          pre_session_create_pipeline: TimelyWeb.SAMLHandler
        }
        |> Map.merge(Map.new(saml_metadata))
      ]

    config :timely, :saml_enabled, true
  end

  # ── SSO: Social OAuth (each enabled when *_CLIENT_ID is set) ──
  config :timely, :sso_require_invite, System.get_env("SSO_REQUIRE_INVITE") == "true"
  ...
  config :timely, :sso_domains, sso_domains
  config :timely, :sso_auto_approve_domains, sso_auto_approve_domains
  ...
  oauth_providers =
    if google_id = System.get_env("GOOGLE_CLIENT_ID") do
      ...
      config :timely, :google_enabled, true
  ...
  oauth_providers =
    if fb_id = System.get_env("FACEBOOK_CLIENT_ID") do
      ...
      config :timely, :facebook_enabled, true
  ...
  oauth_providers =
    if gh_id = System.get_env("GITHUB_CLIENT_ID") do
      ...
      config :timely, :github_enabled, true
  ...
  #     config :timely, :linkedin_enabled, true
