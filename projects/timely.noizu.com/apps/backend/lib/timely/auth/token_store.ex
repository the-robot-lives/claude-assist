defmodule Timely.Auth.TokenStore do
  ...
  def store_refresh_jti(jti) when is_binary(jti) do
    Timely.Redis.set("refresh_jti:#{jti}", "1", ex: @refresh_ttl)
  end
  ...
  def valid_refresh_jti?(jti) when is_binary(jti) do
    case Timely.Redis.get("refresh_jti:#{jti}") do
      {:ok, "1"} -> true
      _ -> false
    end
  end
  ...
  def revoke_refresh_jti(jti) when is_binary(jti) do
    Timely.Redis.del("refresh_jti:#{jti}")
  end
end
