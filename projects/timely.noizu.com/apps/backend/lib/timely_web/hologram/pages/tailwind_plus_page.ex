defmodule TimelyWeb.Hologram.Pages.TailwindPlusPage do
  @moduledoc """
  Browse Tailwind Plus demos (iframe → static HTML under priv/static/twp/demos).

  Catalog is JSON + HTML only — no TypeScript. Prefers `Timely.StyleGuide.TwpCatalog`
  ...
  defp catalog_sections do
    if catalog_available?() do
      Timely.StyleGuide.TwpCatalog.sections()
    else
  ...
  defp catalog_stats do
    if catalog_available?() and function_exported?(Timely.StyleGuide.TwpCatalog, :stats, 0) do
      Timely.StyleGuide.TwpCatalog.stats()
    else
  ...
  defp catalog_first_entry do
    if catalog_available?() and function_exported?(Timely.StyleGuide.TwpCatalog, :first_entry, 0) do
      Timely.StyleGuide.TwpCatalog.first_entry()
    else
      nil
    end
  end
  ...
  defp catalog_available? do
    Code.ensure_loaded?(Timely.StyleGuide.TwpCatalog) and
      function_exported?(Timely.StyleGuide.TwpCatalog, :sections, 0)
  end
