defmodule TimelyWeb.ConsentController do
  use TimelyWeb, :controller

  alias Timely.Schema.CookieConsent
  import Ecto.Query
  ...
  # ⟦𓌿𓂳𓋮𓉃⟧ show :: auto-generated pointer for public function show
  ...
  # ⟦𓍂𓈢𓎛𓆞⟧ update :: auto-generated pointer for public function update
  def update(conn, %{"consent" => params}) do
    ...
    if is_nil(user_id) && is_nil(browser_session_id) do
      ...
      case Timely.Repo.insert_or_update(changeset) do
        {:ok, consent} ->
          json(conn, %{
  ...
  defp current_user_id(conn) do
    with ["Bearer " <> token] <- Plug.Conn.get_req_header(conn, "authorization"),
         {:ok, claims} <- Timely.Guardian.decode_and_verify(token),
         {:ok, session} <- Timely.Guardian.resource_from_claims(claims) do
  ...
  defp find_consent(user_id, browser_session_id) when not is_nil(user_id) do
    Timely.Repo.one(from c in CookieConsent, where: c.user_id == ^user_id, limit: 1) ||
