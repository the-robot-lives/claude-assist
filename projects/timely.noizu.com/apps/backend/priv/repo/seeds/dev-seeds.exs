...
alias Timely.Schema.Organizations.Organization
alias Timely.Schema.Organizations.Membership
alias Timely.Schema.Organizations.InviteToken
alias Timely.Schema.Users.Credentials.UserCredential
alias Timely.Schema.Versioned.Names.Name
alias Timely.Schema.Versioned.Descriptions.Description

admin_id = UUID.uuid5(:oid, "Timely.Dev.Admin")
dev_org_id = UUID.uuid5(:oid, "Timely.Dev.Organization")
login_provider_id = UUID.uuid5(:oid, "Timely.Schema.Auth.Providers.Provider@Login")

seed "dev:admin-name" do
  Timely.Repo.insert!(
    %Name{id: UUID.uuid5(:oid, "Timely.Dev.Admin.Name"), first: "Admin", last: "User"},
    on_conflict: :nothing,
    conflict_target: :id
  )
end
...
seed "dev:admin-description" do
  Timely.Repo.insert!(
    %Description{
      id: UUID.uuid5(:oid, "Timely.Dev.Admin.Description"),
      title: "Admin",
...
seed "dev:admin-user" do
  Timely.Repo.insert!(
    %User{
      ...
      user_name: "admin",
      handle: "admin",
      name_id: UUID.uuid5(:oid, "Timely.Dev.Admin.Name"),
      description_id: UUID.uuid5(:oid, "Timely.Dev.Admin.Description"),
...
seed "dev:admin-credential" do
  Timely.Repo.insert!(
    %UserCredential{
      id: UUID.uuid5(:oid, "Timely.Dev.Admin.Credential"),
      user_id: admin_id,
...
seed "dev:admin-membership" do
  Timely.Repo.insert!(
    %Membership{
      id: UUID.uuid5(:oid, "Timely.Dev.Admin.Membership"),
...
seed "dev:bootstrap-invite" do
  ...
  Timely.Repo.insert!(
    %InviteToken{
      id: UUID.uuid5(:oid, "Timely.Dev.BootstrapInvite"),
      organization_id: dev_org_id,
...
seed "dev:admin-magic-link-token" do
  admin_ref = Timely.Users.User.ref(admin_id)
  ...
  Token: #{encoded_key}
  URL:   http://localhost:8080/auth/verify?token=#{encoded_key}
  """)
end
