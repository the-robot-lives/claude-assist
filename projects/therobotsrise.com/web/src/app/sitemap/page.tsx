...
export default function SitemapPage() {
  ...
  const sectionList = pageSections.flatMap((group) =>
    group.sections.map((s) => ({ group: group.label, ...s }))
  );
    ...
    PAGE --> THEMES["Themes\nactive indicator"]`}</pre>
        ...
        <section className="sg-sitemap-section">
          ...
          {pageSections.map((group) => (
            <div key={group.label} className="sg-sitemap-group">
              <h3 className="sg-sitemap-group-title">{group.label}</h3>
      ...
      <Script id="mermaid-init" strategy="lazyOnload">{`mermaid.initialize({startOnLoad:true,theme:'neutral'});mermaid.run();`}</Script>
    </div>
  );
}
