defmodule Therobotplans.Users.Credentials do
  @moduledoc """
  Context for Therobotplans.Users.Credentials
  """
  alias Therobotplans.Users.Credentials.UserCredential, as: Entity
  alias Therobotplans.Schema.Users.Credentials.UserCredential, as: Schema
  use Noizu.Repo
  import Ecto.Query, only: [from: 2]

  def_repo(entity: Therobotplans.Users.Credentials.UserCredential)
  ...
  def register(user, {:login, {email, password}}, context, _options) do
    ...
    {:ok, description} =
      %Therobotplans.Versioned.Strings.String{
        content: "",
  ...
  def authenticate({:login, {email, password}}, context, options) do
    ...
    with :valid <- valid_login?(email, password) do
      ...
      case Therobotplans.Repo.all(q) do
        [] ->
        ...
        [credential] ->
          if Bcrypt.verify_pass(password, credential.settings["password"]) do
            with {:ok, credential_entity} <-
                   Therobotplans.Users.Credentials.UserCredential.entity(credential.id, context),
              ...
              |> Therobotplans.EntityRepo.create(context, options)
            end
          else
