defmodule TherobotlivesWeb.Router do
  use TherobotlivesWeb, :router
  ...
  pipeline :authenticated do
    plug TherobotlivesWeb.AuthPipeline
  end

  pipeline :sso_session do
    plug Plug.Session,
      store: :cookie,
      key: "_therobotlives_sso",
  ...
  pipeline :rate_limited_auth do
    plug TherobotlivesWeb.Plugs.RateLimit, action: :auth
  end

  pipeline :rate_limited_sensitive do
    plug TherobotlivesWeb.Plugs.RateLimit, action: :auth_sensitive
  end

  pipeline :org_viewer do
    plug TherobotlivesWeb.Plugs.RequireRole, role: "viewer"
  end

  pipeline :org_editor do
    plug TherobotlivesWeb.Plugs.RequireRole, role: "editor"
  end

  pipeline :org_admin do
    plug TherobotlivesWeb.Plugs.RequireRole, role: "admin"
  end

  pipeline :org_owner do
    plug TherobotlivesWeb.Plugs.RequireRole, role: "owner"
  end

  pipeline :admin do
    plug TherobotlivesWeb.Plugs.RequireAdmin
  end

  scope "/", TherobotlivesWeb do
    pipe_through :api
    get "/health", HealthController, :index
  end

  scope "/api/v1", TherobotlivesWeb do
  ...
  scope "/media", TherobotlivesWeb do
    pipe_through [:api]
  ...
  if Application.compile_env(:therobotlives, :saml_enabled) do
    scope "/sso/saml" do
  ...
  # OIDC redirect flow
  scope "/auth/oidc", TherobotlivesWeb do
    pipe_through [:sso_session]
  ...
  scope "/auth", TherobotlivesWeb do
    pipe_through [:sso_session]
  ...
  if Application.compile_env(:therobotlives, :dev_routes) do
    import Phoenix.LiveDashboard.Router

    scope "/dev" do
      pipe_through [:fetch_session, :protect_from_forgery]
      live_dashboard "/dashboard", metrics: TherobotlivesWeb.Telemetry
    end
  end
end
