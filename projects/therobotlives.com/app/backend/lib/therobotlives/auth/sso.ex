defmodule Therobotlives.Auth.SSO do
  alias Therobotlives.Schema.Users.User, as: UserSchema
  alias Therobotlives.Schema.Users.Credentials.UserCredential, as: CredentialSchema
  alias Therobotlives.Schema.Versioned.Names.Name
  import Ecto.Query, only: [from: 2]

  @provider_map %{
    oidc: &Therobotlives.Auth.Providers.oidc/0,
    saml: &Therobotlives.Auth.Providers.saml/0,
    google: &Therobotlives.Auth.Providers.google/0,
    facebook: &Therobotlives.Auth.Providers.facebook/0,
    github: &Therobotlives.Auth.Providers.github/0,
    linkedin: &Therobotlives.Auth.Providers.linkedin/0
  }
  ...
  def authenticate_sso(provider_type, %{email: email} = attrs) do
    ...
    {:ok, provider_id} = Therobotlives.Auth.Providers.Provider.id(provider_ref)
    ...
    case find_user_by_email(email) do
      ...
      :not_found ->
        if Application.get_env(:therobotlives, :sso_require_invite, false) do
  ...
  defp find_user_by_email(email) do
    ...
    case Therobotlives.Repo.one(q) do
      nil -> :not_found
      user -> {:ok, user}
    end
  end
  ...
  defp ensure_sso_credential(user, provider_ref, provider_id, provider_type, attrs, context) do
    ...
    case Therobotlives.Repo.one(q) do
      nil ->
        %Therobotlives.Users.Credentials.UserCredential{
          user: Therobotlives.Users.User.ref(user.id),
  ...
  defp create_sso_session(user, provider_type, context) do
    ...
    %Therobotlives.Users.Sessions.UserSession{
      user: user_ref,
      status: :active,
  ...
  defp auto_provision_user(email, attrs, provider_ref, provider_id, provider_type, context) do
    ...
    {:ok, name} =
      Therobotlives.EntityRepo.create(
        %Therobotlives.Versioned.Names.Name{first: first, last: last, time_stamp: Noizu.Entity.TimeStamp.now()},
        context
      )
    ...
    {:ok, user} = Therobotlives.Repo.insert(user_schema, on_conflict: :nothing, conflict_target: :email)
