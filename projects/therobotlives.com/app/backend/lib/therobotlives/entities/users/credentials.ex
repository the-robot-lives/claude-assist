defmodule Therobotlives.Users.Credentials do
  @moduledoc """
  Context for Therobotlives.Users.Credentials
  """
  alias Therobotlives.Users.Credentials.UserCredential, as: Entity
  alias Therobotlives.Schema.Users.Credentials.UserCredential, as: Schema
  use Noizu.Repo
  import Ecto.Query, only: [from: 2]

  def_repo(entity: Therobotlives.Users.Credentials.UserCredential)
  ...
  def register(user, {:login, {email, password}}, context, _options) do
    {:ok, auth_provider} = Therobotlives.Auth.Providers.login()

    {:ok, description} =
      %Therobotlives.Versioned.Strings.String{
        content: "",
  ...
  def authenticate({:login, {email, password}}, context, options) do
    {:ok, auth_provider} = Therobotlives.Auth.Providers.login()
    {:ok, auth_provider_id} = Therobotlives.Auth.Providers.Provider.id(auth_provider)
    ...
    with :valid <- valid_login?(email, password) do
      ...
      case Therobotlives.Repo.all(q) do
        [] ->
        ...
        [credential] ->
          if Bcrypt.verify_pass(password, credential.settings["password"]) do
            with {:ok, credential_entity} <-
                   Therobotlives.Users.Credentials.UserCredential.entity(credential.id, context),
              ...
              |> Therobotlives.EntityRepo.create(context, options)
            end
          else
