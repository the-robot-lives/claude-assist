defmodule Therobotlives.Organizations do
  @moduledoc """
  Context for Therobotlives.Organizations
  """
  alias Therobotlives.Organizations.Organization, as: Entity
  alias Therobotlives.Schema.Organizations.Organization, as: Schema
  alias Therobotlives.Schema.Organizations.InviteToken, as: InviteTokenSchema
  alias Therobotlives.Schema.Authz.ScopedMembership, as: ScopedMembershipSchema
  use Noizu.Repo
  ...
  def create_organization_with_owner(attrs, user_id) do
    Therobotlives.Repo.transaction(fn ->
      with {:ok, org} <- %Schema{} |> Schema.changeset(attrs) |> Therobotlives.Repo.insert(),
           {:ok, _membership} <- Therobotlives.Authz.ScopedMemberships.add_member("organization", org.id, user_id, "owner") do
        org
      else
  ...
  def list_user_organizations(user_id) do
    from(sm in ScopedMembershipSchema,
      ...
      join: g in Therobotlives.Schema.Authz.Group, on: g.id == sm.group_id,
    ...
    |> Therobotlives.Repo.all()
  end

  def authorize(user_id, organization_id, required_role) do
    Therobotlives.Authz.authorize(user_id, "organization", organization_id, required_role)
  end

  def list_members(organization_id) do
    Therobotlives.Authz.ScopedMemberships.list_for_resource("organization", organization_id)
  end
  ...
  def increment_invite_uses(invite_token) do
    ...
    |> Therobotlives.Repo.update_all(inc: [uses: 1])
  end
end
