defmodule Therobotlives.Media do
  alias Therobotlives.Media.Asset, as: Entity
  alias Therobotlives.Schema.Media.Asset, as: Schema
  use Noizu.Repo
  ...
  def get_cached_variant(media_id, canonical_params) do
    import Ecto.Query
    Therobotlives.Repo.one(
      from v in Therobotlives.Schema.Media.Variant,
        where: v.media_id == ^media_id and v.params == ^canonical_params
    )
  end

  def cache_variant(attrs) do
    %Therobotlives.Schema.Media.Variant{}
    |> Therobotlives.Schema.Media.Variant.changeset(attrs)
    |> Therobotlives.Repo.insert(on_conflict: :nothing)
  end

  def fetch_from_s3(key) do
    config = Application.get_env(:therobotlives, Therobotlives.Storage, [])
  ...
  def get_or_create_variant(media, transform_params) do
    canonical = Therobotlives.Media.Transform.canonical_params(transform_params)
      ...
      nil ->
        with {:ok, original_binary} <- fetch_from_s3(media.file),
             {:ok, transformed, content_type} <-
               Therobotlives.Media.Transform.transform(original_binary, transform_params) do
          variant_key = Therobotlives.Media.Transform.variant_s3_key(media.file, transform_params)
