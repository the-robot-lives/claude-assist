defmodule Therobotlives.Authz.Policies do
  alias Therobotlives.Authz.Policies.Policy, as: Entity
  alias Therobotlives.Schema.Authz.Policy, as: Schema
  alias Therobotlives.Schema.Authz.UserPolicy, as: UserPolicySchema
  ...
  def list_active(opts \\ []) do
    ...
    Therobotlives.Repo.all(query)
  end

  def get_by_name(name) do
    Therobotlives.Repo.one(from p in Schema, where: p.name == ^name and p.is_active == true)
  end
    ...
    case Therobotlives.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          policy |> Schema.changeset(attrs) |> Therobotlives.Repo.update()
        end
    end
  end
    ...
    case Therobotlives.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          Therobotlives.Repo.delete(policy)
        end
    end
  end
  ...
  def detach_from_user(user_id, policy_id) do
    case Therobotlives.Repo.one(
      from up in UserPolicySchema,
        where: up.user_id == ^user_id and up.policy_id == ^policy_id
    ) do
      nil -> {:error, :not_found}
      user_policy -> Therobotlives.Repo.delete(user_policy)
    end
  end
end
