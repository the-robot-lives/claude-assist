...
// Reusable signups table with URL-driven search / status-filter / sort +
// CSV export + slide-over detail. Rendered by BOTH the admin console
// (/app/admin/services/:projectId/lists/:listId) and the org dashboard
// (/app/:orgId/services/:projectId/lists/:listId). The only page-specific
// inputs are the list id and the "back" link, passed as props.
...
export interface SignupsViewProps {
  listId: string;
  backHref: string;
  backLabel?: string;
}
...
function SignupsViewInner({ listId, backHref, backLabel = "Back to lists" }: SignupsViewProps) {
  ...
  const [list, setList] = useState<OrgList | null>(null);
  const [rows, setRows] = useState<OrgSignup[]>([]);
  const [total, setTotal] = useState(0);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [errorStatus, setErrorStatus] = useState<number | null>(null);
  const [exporting, setExporting] = useState(false);
  const [selected, setSelected] = useState<OrgSignup | null>(null);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        if (serverDefault) {
          const res = await listSignups(listId, {
          ...
          const all = await fetchAllSignups(listId, { status: status || undefined });
          if (cancelled) return;
        ...
        if (cancelled) return;
        const status2 = e instanceof OrgApiError ? e.status : null;
  ...
  if (error && errorStatus === 404) {
    return (
      <div>
        <BackLink href={backHref} label={backLabel} />
        <EmptyState title="Not found" message="This list no longer exists." cta={{ href: backHref, label: backLabel }} />
      </div>
    );
  }
...
/** Signups table with its own Suspense boundary (required for useSearchParams). */
export function SignupsView(props: SignupsViewProps) {
  return (
    <Suspense fallback={<p className="sg-admin-loading">Loading…</p>}>
      <SignupsViewInner {...props} />
    </Suspense>
  );
}
