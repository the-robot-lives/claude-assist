...
if System.get_env("PHX_SERVER") do
  config :foryou, ForyouWeb.Endpoint, server: true
end
...
if config_env() == :prod do
  ...
  config :foryou, :dns_cluster_query, System.get_env("DNS_CLUSTER_QUERY")
  ...
  config :foryou, :frontend_url, System.get_env("FRONTEND_URL")
  ...
  if oidc_client_id = System.get_env("OIDC_CLIENT_ID") do
    ...
    config :foryou, :oidc_enabled, true
  end
  ...
  if saml_metadata = System.get_env("SAML_IDP_METADATA_URL") do
    ...
    config :foryou, :saml_enabled, true
  end

  # ── SSO: Social OAuth (each enabled when *_CLIENT_ID is set) ──
  config :foryou, :sso_require_invite, System.get_env("SSO_REQUIRE_INVITE") == "true"
  ...
  oauth_providers =
    if google_id = System.get_env("GOOGLE_CLIENT_ID") do
      ...
      config :foryou, :google_enabled, true
  ...
  oauth_providers =
    if fb_id = System.get_env("FACEBOOK_CLIENT_ID") do
      ...
      config :foryou, :facebook_enabled, true
  ...
  oauth_providers =
    if gh_id = System.get_env("GITHUB_CLIENT_ID") do
      ...
      config :foryou, :github_enabled, true
  ...
  #     config :foryou, :linkedin_enabled, true
