defmodule ForyouWeb.ConnCase do
  ...
  using do
    quote do
      ...
      import ForyouWeb.ConnCase

      @endpoint ForyouWeb.Endpoint
    end
  end
  ...
  def setup_user_and_token(_context \\ %{}) do
    ...
    Foryou.Repo.insert!(
      %Foryou.Schema.Auth.Providers.Provider{
    ...
    {:ok, user} = Foryou.Repo.insert(user)

    credential = %Foryou.Schema.Users.Credentials.UserCredential{
      id: Ecto.UUID.generate(),
      user_id: user.id,
    ...
    {:ok, _credential} = Foryou.Repo.insert(credential)

    session = %Foryou.Schema.Users.Sessions.UserSession{
      id: Ecto.UUID.generate(),
      user_id: user.id,
      status: :active,
      details: %{}
    ...
    {:ok, session} = Foryou.Repo.insert(session)

    # Build entity-layer session struct for Guardian signing
    session_entity = %Foryou.Users.Sessions.UserSession{
      id: session.id,
      user: {:ref, Foryou.Users.User, user.id},
      status: :active,
      details: %{}
    }

    {:ok, access_token, _claims} =
      Foryou.Guardian.encode_and_sign(session_entity, %{}, token_type: "access", ttl: {1, :hour})

    {:ok, refresh_token, %{"jti" => jti}} =
      Foryou.Guardian.encode_and_sign(session_entity, %{}, token_type: "refresh", ttl: {7, :day})

    Foryou.Auth.TokenStore.store_refresh_jti(jti)
