defmodule Foryou.Organizations do
  @moduledoc """
  Context for Foryou.Organizations
  """
  alias Foryou.Organizations.Organization, as: Entity
  alias Foryou.Schema.Organizations.Organization, as: Schema
  alias Foryou.Schema.Organizations.InviteToken, as: InviteTokenSchema
  alias Foryou.Schema.Authz.ScopedMembership, as: ScopedMembershipSchema
  use Noizu.Repo
  ...
  def create_organization_with_owner(attrs, user_id) do
    Foryou.Repo.transaction(fn ->
      with {:ok, org} <- %Schema{} |> Schema.changeset(attrs) |> Foryou.Repo.insert(),
           {:ok, _membership} <- Foryou.Authz.ScopedMemberships.add_member("organization", org.id, user_id, "owner") do
        org
      else
  ...
  def list_user_organizations(user_id) do
    from(sm in ScopedMembershipSchema,
      ...
      join: g in Foryou.Schema.Authz.Group, on: g.id == sm.group_id,
  ...
  def authorize(user_id, organization_id, required_role) do
    Foryou.Authz.authorize(user_id, "organization", organization_id, required_role)
  end

  def list_members(organization_id) do
    Foryou.Authz.ScopedMemberships.list_for_resource("organization", organization_id)
  end
  ...
  def increment_invite_uses(invite_token) do
    ...
    |> Foryou.Repo.update_all(inc: [uses: 1])
  end
end
