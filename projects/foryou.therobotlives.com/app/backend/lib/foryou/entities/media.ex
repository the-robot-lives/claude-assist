defmodule Foryou.Media do
  alias Foryou.Media.Asset, as: Entity
  alias Foryou.Schema.Media.Asset, as: Schema
  use Noizu.Repo
  ...
  def get_cached_variant(media_id, canonical_params) do
    import Ecto.Query
    Foryou.Repo.one(
      from v in Foryou.Schema.Media.Variant,
        where: v.media_id == ^media_id and v.params == ^canonical_params
    )
  end

  def cache_variant(attrs) do
    %Foryou.Schema.Media.Variant{}
    |> Foryou.Schema.Media.Variant.changeset(attrs)
    |> Foryou.Repo.insert(on_conflict: :nothing)
  end

  def fetch_from_s3(key) do
    config = Application.get_env(:foryou, Foryou.Storage, [])
  ...
  def get_or_create_variant(media, transform_params) do
    canonical = Foryou.Media.Transform.canonical_params(transform_params)
      ...
      nil ->
        with {:ok, original_binary} <- fetch_from_s3(media.file),
             {:ok, transformed, content_type} <-
               Foryou.Media.Transform.transform(original_binary, transform_params) do
          variant_key = Foryou.Media.Transform.variant_s3_key(media.file, transform_params)
