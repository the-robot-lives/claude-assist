defmodule Foryou.Authz.Policies do
  alias Foryou.Authz.Policies.Policy, as: Entity
  alias Foryou.Schema.Authz.Policy, as: Schema
  alias Foryou.Schema.Authz.UserPolicy, as: UserPolicySchema
  ...
  def list_active(opts \\ []) do
    ...
    Foryou.Repo.all(query)
  end

  def get_by_name(name) do
    Foryou.Repo.one(from p in Schema, where: p.name == ^name and p.is_active == true)
  end
    ...
    case Foryou.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          policy |> Schema.changeset(attrs) |> Foryou.Repo.update()
        end
    end
  end
  ...
  def detach_from_user(user_id, policy_id) do
    case Foryou.Repo.one(
      from up in UserPolicySchema,
        where: up.user_id == ^user_id and up.policy_id == ^policy_id
    ) do
      nil -> {:error, :not_found}
      user_policy -> Foryou.Repo.delete(user_policy)
    end
  end
end
