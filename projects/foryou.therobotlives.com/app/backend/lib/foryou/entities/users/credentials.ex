defmodule Foryou.Users.Credentials do
  @moduledoc """
  Context for Foryou.Users.Credentials
  """
  alias Foryou.Users.Credentials.UserCredential, as: Entity
  alias Foryou.Schema.Users.Credentials.UserCredential, as: Schema
  use Noizu.Repo
  import Ecto.Query, only: [from: 2]

  def_repo(entity: Foryou.Users.Credentials.UserCredential)
  ...
  def register(user, {:login, {email, password}}, context, _options) do
    {:ok, auth_provider} = Foryou.Auth.Providers.login()

    {:ok, description} =
      %Foryou.Versioned.Strings.String{
        content: "",
  ...
  def authenticate({:login, {email, password}}, context, options) do
    {:ok, auth_provider} = Foryou.Auth.Providers.login()
    {:ok, auth_provider_id} = Foryou.Auth.Providers.Provider.id(auth_provider)
        ...
        [credential] ->
          if Bcrypt.verify_pass(password, credential.settings["password"]) do
            with {:ok, credential_entity} <-
                   Foryou.Users.Credentials.UserCredential.entity(credential.id, context),
              ...
              |> Foryou.EntityRepo.create(context, options)
            end
          else
