defmodule ForyouWeb.Plugs.RequireAdmin do
  @behaviour Plug
  import Plug.Conn
  ...
  defp get_user(conn) do
    case Foryou.Guardian.Plug.current_resource(conn) do
      %Foryou.Users.Sessions.UserSession{user: {:ref, _, id}} ->
        case Foryou.Repo.get(Foryou.Schema.Users.User, id) do
          nil -> :error
          user -> {:ok, user}
        end

      %Foryou.Users.Sessions.UserSession{user: %{id: id}} ->
        case Foryou.Repo.get(Foryou.Schema.Users.User, id) do
          nil -> :error
          user -> {:ok, user}
        end
