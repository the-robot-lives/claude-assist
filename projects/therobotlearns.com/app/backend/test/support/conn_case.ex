defmodule TheRobotLearnsWeb.ConnCase do
  ...
  using do
    quote do
      ...
      import TheRobotLearnsWeb.ConnCase

      @endpoint TheRobotLearnsWeb.Endpoint
    end
  end
  ...
  def setup_user_and_token(_context \\ %{}) do
    ...
    user = %TheRobotLearns.Schema.Users.User{
    ...
    {:ok, user} = TheRobotLearns.Repo.insert(user)

    credential = %TheRobotLearns.Schema.Users.Credentials.UserCredential{
      id: Ecto.UUID.generate(),
      user_id: user.id,
    ...
    {:ok, _credential} = TheRobotLearns.Repo.insert(credential)

    session = %TheRobotLearns.Schema.Users.Sessions.UserSession{
      id: Ecto.UUID.generate(),
      user_id: user.id,
      status: :active,
      details: %{}
    ...
    {:ok, session} = TheRobotLearns.Repo.insert(session)
    ...
    {:ok, access_token, _claims} =
      TheRobotLearns.Guardian.encode_and_sign(session_entity, %{}, token_type: "access", ttl: {1, :hour})

    {:ok, refresh_token, %{"jti" => jti}} =
      TheRobotLearns.Guardian.encode_and_sign(session_entity, %{}, token_type: "refresh", ttl: {7, :day})

    TheRobotLearns.Auth.TokenStore.store_refresh_jti(jti)
