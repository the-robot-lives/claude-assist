defmodule TheRobotLearns.Auth.SSOTest do
  use TheRobotLearns.DataCase

  alias TheRobotLearns.Schema.Auth.Providers.Provider
  alias TheRobotLearns.Schema.Users.User

  setup do
    previous_domains = Application.get_env(:the_robot_learns, :sso_domains)
    previous_auto_approve = Application.get_env(:the_robot_learns, :sso_auto_approve_domains)
    previous_policies = Application.get_env(:the_robot_learns, :sso_domain_policies)

    Application.put_env(:the_robot_learns, :sso_domains, %{})
    Application.put_env(:the_robot_learns, :sso_auto_approve_domains, [])
    Application.put_env(:the_robot_learns, :sso_domain_policies, %{})

    on_exit(fn ->
      Application.put_env(:the_robot_learns, :sso_domains, previous_domains)
      Application.put_env(:the_robot_learns, :sso_auto_approve_domains, previous_auto_approve)
      Application.put_env(:the_robot_learns, :sso_domain_policies, previous_policies)
    end)
  end
  ...
  test "auto-provisioned SSO users are pending until profile completion" do
    ...
    assert {:ok, session} =
             TheRobotLearns.Auth.SSO.authenticate_sso(:google, %{
               email: email,
               name: %{first: "Ada", last: "Lovelace"},
               sub: "google-#{System.unique_integer([:positive])}"
             })

    assert {:ref, TheRobotLearns.Users.User, user_id} = session.user

    user = TheRobotLearns.Repo.get!(User, user_id)
  ...
  defp ensure_provider!(title, description) do
    id = UUID.uuid5(:oid, "TheRobotLearns.Schema.Auth.Providers.Provider@#{title}")
