defmodule TheRobotLearnsWeb.AuthControllerTest do
  use TheRobotLearnsWeb.ConnCase
  import Ecto.Query

  alias TheRobotLearns.Schema.Organizations.InviteToken
  alias TheRobotLearns.Schema.Organizations.InviteTokenRedemption
  ...
  describe "POST /api/v1/auth/register" do
    test "creates complete password registrations as pending without invite", %{conn: conn} do
      ...
      user = TheRobotLearns.Repo.get_by!(User, email: email)
    ...
    test "valid invite activates the user and records redemption", %{conn: conn} do
      ...
      user = TheRobotLearns.Repo.get_by!(User, email: email)
      invite = TheRobotLearns.Repo.get!(InviteToken, invite.id)
    ...
    test "missing invite is rejected when invites are required", %{conn: conn} do
      Application.put_env(:the_robot_learns, :require_invite_for_signup, true)
      on_exit(fn -> Application.put_env(:the_robot_learns, :require_invite_for_signup, false) end)

      conn = post(conn, "/api/v1/auth/register", %{user: registration_params(unique_email())})

      assert json_response(conn, 422)["error"] =~ "invite"
    end

    test "valid invite still registers when invites are required", %{conn: conn} do
      Application.put_env(:the_robot_learns, :require_invite_for_signup, true)
      on_exit(fn -> Application.put_env(:the_robot_learns, :require_invite_for_signup, false) end)

      email = unique_email()
      {:ok, _invite, raw_token} = create_invite(email, max_uses: 1)
      ...
      assert json_response(conn, 201)["user"]["status"] == "active"
    end
  end
  ...
  defp create_invite(email, attrs) do
    ...
    TheRobotLearns.Organizations.create_invite_token(Map.merge(defaults, Map.new(attrs)))
  end
end
