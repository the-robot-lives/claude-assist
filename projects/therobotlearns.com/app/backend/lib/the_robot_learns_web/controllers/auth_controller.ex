defmodule TheRobotLearnsWeb.AuthController do
  ...
  def register(conn, %{"user" => user_params} = params) do
    ...
    with {:ok, invite} <- resolve_invite(raw_token, email),
      ...
      {:error, :invite_required} ->
        ...
        |> json(%{error: "A valid invite token is required to sign up."})
  ...
  def login(conn, %{"email" => email, "password" => password}) do
    case TheRobotLearns.Users.Credentials.authenticate(
  ...
  def request_magic_link(conn, %{"email" => email}) do
    frontend_url = Application.get_env(:the_robot_learns, :frontend_url, "http://localhost:3000")

    case TheRobotLearns.Auth.SmartTokenAuth.request_magic_link(email) do
      {:ok, %{encoded_key: encoded_key, user: _user}} ->
        magic_link = "#{frontend_url}/auth/verify?token=#{encoded_key}"
        TheRobotLearns.Auth.SmartTokenEmail.send_magic_link(email, magic_link)

        response = %{message: "If an account exists with that email, a magic link has been sent."}

        response =
          if Application.get_env(:the_robot_learns, :dev_routes) do
  ...
  def verify_magic_link(conn, %{"token" => token_key}) do
    case TheRobotLearns.Auth.SmartTokenAuth.verify_magic_link(token_key, conn) do
      {:ok, session} ->
        {:ok, access_token, _} =
          Guardian.encode_and_sign(session, %{}, token_type: "access", ttl: {1, :hour})
  ...
  def request_otp_login(conn, %{"email" => email}) do
    case TheRobotLearns.Auth.SmartTokenAuth.request_otp_login(email) do
      {:ok, %{otp_code: otp_code, user: _user}} ->
        TheRobotLearns.Auth.SmartTokenEmail.send_otp_login(email, otp_code)
  ...
  def verify_otp_login(conn, %{"email" => email, "code" => code}) do
    case TheRobotLearns.Auth.SmartTokenAuth.verify_otp_login(email, code, conn) do
      {:ok, session} ->
        {:ok, access_token, _} =
          Guardian.encode_and_sign(session, %{}, token_type: "access", ttl: {1, :hour})
  ...
  def request_password_reset(conn, %{"email" => email}) do
    case TheRobotLearns.Auth.SmartTokenAuth.request_password_reset(email) do
      {:ok, %{otp_code: otp_code, user: _user}} ->
        TheRobotLearns.Auth.SmartTokenEmail.send_password_reset(email, otp_code)
  ...
  def verify_password_reset(conn, %{
    ...
    case TheRobotLearns.Auth.SmartTokenAuth.verify_password_reset(email, code, new_password, conn) do
      {:ok, _credential} ->
        conn
        |> put_status(:ok)
  ...
  def refresh(conn, %{"refresh_token" => refresh_token}) do
    alias TheRobotLearns.Auth.TokenStore
  ...
  def send_verification(conn, _params) do
    ...
    case TheRobotLearns.Auth.SmartTokenAuth.request_email_verification(user.email) do
      {:ok, %{encoded_key: encoded_key}} ->
        verification_link = "#{frontend_url}/auth/verify-email?token=#{encoded_key}"
        TheRobotLearns.Auth.SmartTokenEmail.send_verification_email(user.email, verification_link)
  ...
  def verify_email(conn, %{"token" => token_key}) do
    case TheRobotLearns.Auth.SmartTokenAuth.verify_email_token(token_key) do
      {:ok, user} ->
        TheRobotLearns.Events.dispatch(:user_verified, %{user_id: user.id})
  ...
  defp create_session_for_user(user) do
    user_ref = {:ref, TheRobotLearns.Users.User, user.id}

    session_entity = %TheRobotLearns.Users.Sessions.UserSession{
      user: user_ref,
      status: :active,
      details: %{}
    }

    TheRobotLearns.Users.Sessions.create(session_entity, Noizu.Context.system())
  end

  defp resolve_user_from_session(%TheRobotLearns.Users.Sessions.UserSession{} = session) do
    case session.user do
      {:ref, _, id} ->
        {:ok, user} = TheRobotLearns.Users.get_user(id, Noizu.Context.system())
        user

      %TheRobotLearns.Users.User{} = user ->
        user
    end
  end
  ...
  # Invite gating: when signup requires an invite, a missing / invalid / expired /
  # consumed token fails closed (422). SSO/OIDC sign-in does not pass through here
  # and is therefore unaffected.
  defp resolve_invite(nil, _email) do
    if require_invite_for_signup?(), do: {:error, :invite_required}, else: {:ok, nil}
  end

  defp resolve_invite(raw_token, email),
    do: Organizations.find_active_invite_by_raw_token(raw_token, email)

  defp require_invite_for_signup? do
    Application.get_env(:the_robot_learns, :require_invite_for_signup, false)
  end
