defmodule TheRobotLearnsWeb.CustomRoleController do
  use TheRobotLearnsWeb, :controller

  alias TheRobotLearns.Schema.Organizations.CustomRole, as: RoleSchema
  alias TheRobotLearns.Schema.Organizations.CustomRolePermission, as: PermSchema
  ...
  def create(conn, %{"org_id" => org_id, "role" => role_params}) do
    ...
    if Authz.check_permission(user_id, "organization", org_id, "organization:manage_settings") do
      ...
      case %RoleSchema{} |> RoleSchema.changeset(attrs) |> TheRobotLearns.Repo.insert() do
        {:ok, role} ->
  ...
  def update(conn, %{"org_id" => org_id, "id" => role_id, "role" => role_params}) do
    ...
    if Authz.check_permission(user_id, "organization", org_id, "organization:manage_settings") do
      case TheRobotLearns.Repo.one(
        ...
        role ->
          case role |> RoleSchema.changeset(role_params) |> TheRobotLearns.Repo.update() do
            {:ok, updated} ->
  ...
  def delete(conn, %{"org_id" => org_id, "id" => role_id}) do
    ...
    if Authz.check_permission(user_id, "organization", org_id, "organization:manage_settings") do
      case TheRobotLearns.Repo.one(
        ...
        role ->
          case role |> Ecto.Changeset.change(is_active: false) |> TheRobotLearns.Repo.update() do
            {:ok, _} ->
  ...
  def remove_permission(conn, %{
    ...
    if Authz.check_permission(user_id, "organization", org_id, "organization:manage_settings") do
      case TheRobotLearns.Repo.one(
        ...
        perm ->
          {:ok, _} = TheRobotLearns.Repo.delete(perm)
