defmodule TheRobotLearnsWeb.ConsentController do
  use TheRobotLearnsWeb, :controller

  alias TheRobotLearns.Schema.CookieConsent
  import Ecto.Query
  ...
  def update(conn, %{"consent" => params}) do
    ...
    if is_nil(user_id) && is_nil(browser_session_id) do
      ...
      case TheRobotLearns.Repo.insert_or_update(changeset) do
        {:ok, consent} ->
          json(conn, %{
  ...
  defp current_user_id(conn) do
    with ["Bearer " <> token] <- Plug.Conn.get_req_header(conn, "authorization"),
         {:ok, claims} <- TheRobotLearns.Guardian.decode_and_verify(token),
         {:ok, session} <- TheRobotLearns.Guardian.resource_from_claims(claims) do
  ...
  defp find_consent(user_id, browser_session_id) when not is_nil(user_id) do
    TheRobotLearns.Repo.one(from c in CookieConsent, where: c.user_id == ^user_id, limit: 1) ||
