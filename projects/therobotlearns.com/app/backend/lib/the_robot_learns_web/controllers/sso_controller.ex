defmodule TheRobotLearnsWeb.SSOController do
  ...
  def providers(conn, _params) do
    ...
    json(conn, %{
      providers: providers,
      domains: TheRobotLearns.Auth.SSODomains.providers_map(),
      domain_policies: TheRobotLearns.Auth.SSODomains.public_policies()
    })
  end
  ...
  def exchange(conn, %{"code" => code}) do
    with {:ok, session_id} <- TheRobotLearns.Auth.SSOCode.exchange(code),
         {:ok, session} <- TheRobotLearns.Users.Sessions.get(session_id, Noizu.Context.system(), []),
         {:ok, access_token, _} <-
  ...
  defp handle_sso_callback(conn, provider_type, attrs) do
    ...
    case TheRobotLearns.Auth.SSO.authenticate_sso(provider_type, attrs) do
      {:ok, session} ->
        {:ok, code} = TheRobotLearns.Auth.SSOCode.create(session.id)
  ...
  defp maybe_add(list, flag, name) do
    if Application.get_env(:the_robot_learns, flag), do: [name | list], else: list
  end
end
