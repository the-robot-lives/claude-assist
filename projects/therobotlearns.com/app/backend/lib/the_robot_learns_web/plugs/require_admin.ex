defmodule TheRobotLearnsWeb.Plugs.RequireAdmin do
  @behaviour Plug
  import Plug.Conn
  ...
  defp get_user(conn) do
    case TheRobotLearns.Guardian.Plug.current_resource(conn) do
      %TheRobotLearns.Users.Sessions.UserSession{user: {:ref, _, id}} ->
        case TheRobotLearns.Repo.get(TheRobotLearns.Schema.Users.User, id) do
          nil -> :error
          user -> {:ok, user}
        end

      %TheRobotLearns.Users.Sessions.UserSession{user: %{id: id}} ->
        case TheRobotLearns.Repo.get(TheRobotLearns.Schema.Users.User, id) do
          nil -> :error
          user -> {:ok, user}
        end
