defmodule TheRobotLearns.Auth.SSO do
  ...
  @provider_map %{
    oidc: &TheRobotLearns.Auth.Providers.oidc/0,
    saml: &TheRobotLearns.Auth.Providers.saml/0,
    google: &TheRobotLearns.Auth.Providers.google/0,
    facebook: &TheRobotLearns.Auth.Providers.facebook/0,
    github: &TheRobotLearns.Auth.Providers.github/0,
    linkedin: &TheRobotLearns.Auth.Providers.linkedin/0
  }
  ...
  defp ensure_sso_credential(user, provider_ref, provider_id, provider_type, attrs, context) do
    ...
    case TheRobotLearns.Repo.one(q) do
      nil ->
        %TheRobotLearns.Users.Credentials.UserCredential{
          user: user_ref,
  ...
  defp create_sso_session(user, provider_type, context) do
    ...
    %TheRobotLearns.Users.Sessions.UserSession{
      user: user_ref,
      status: :active,
  ...
  defp auto_provision_user(email, attrs, provider_ref, _provider_id, provider_type, context) do
    ...
    {:ok, name} =
      %TheRobotLearns.Schema.Versioned.Names.Name{
        first: first,
        last: last
      }
    ...
    user_schema = %UserSchema{
      ...
      name_id: name.id,
      email: email,
      status: TheRobotLearns.Auth.SSODomains.registration_status(email, provider_type),
      verified: true,
      flagged: false,
      approved_at: auto_approved_at(email, provider_type)
    }

    {:ok, user} = TheRobotLearns.Repo.insert(user_schema, on_conflict: :nothing, conflict_target: :email)

    %TheRobotLearns.Users.Credentials.UserCredential{
      user: {:ref, TheRobotLearns.Users.User, user.id},
      auth_provider: provider_ref,
      status: :active,
      settings: sso_settings(provider_type, attrs),
      state: %{},
  ...
  defp auto_approved_at(email, provider_type) do
    if TheRobotLearns.Auth.SSODomains.auto_approve?(email, provider_type), do: DateTime.utc_now()
  end
