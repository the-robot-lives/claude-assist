defmodule TheRobotLearns.Users.Credentials do
  @moduledoc """
  Context for TheRobotLearns.Users.Credentials
  """
  alias TheRobotLearns.Users.Credentials.UserCredential, as: Entity
  alias TheRobotLearns.Schema.Users.Credentials.UserCredential, as: Schema
  use Noizu.Repo
  import Ecto.Query, only: [from: 2]

  def_repo(entity: TheRobotLearns.Users.Credentials.UserCredential)
  ...
  def register(user, {:login, {email, password}}, context, _options) do
    ...
    {:ok, description} =
      %TheRobotLearns.Versioned.Strings.String{
        content: "",
  ...
  def authenticate({:login, {email, password}}, context, options) do
    ...
    with :valid <- valid_login?(email, password) do
      ...
      case TheRobotLearns.Repo.all(q) do
        [] ->
        ...
        [credential] ->
          if Bcrypt.verify_pass(password, credential.settings["password"]) do
            with {:ok, credential_entity} <-
                   TheRobotLearns.Users.Credentials.UserCredential.entity(credential.id, context),
                 {:ok, user} <-
              ...
              |> TheRobotLearns.EntityRepo.create(context, options)
            end
          else
