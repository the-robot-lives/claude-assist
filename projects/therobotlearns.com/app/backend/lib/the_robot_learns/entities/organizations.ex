defmodule TheRobotLearns.Organizations do
  @moduledoc """
  Context for TheRobotLearns.Organizations
  """
  alias TheRobotLearns.Organizations.Organization, as: Entity
  alias TheRobotLearns.Schema.Organizations.Organization, as: Schema
  alias TheRobotLearns.Schema.Organizations.InviteToken, as: InviteTokenSchema
  alias TheRobotLearns.Schema.Organizations.InviteTokenRedemption, as: InviteTokenRedemptionSchema
  alias TheRobotLearns.Schema.Authz.ScopedMembership, as: ScopedMembershipSchema
  use Noizu.Repo
  ...
  def create_organization_with_owner(attrs, user_id) do
    TheRobotLearns.Repo.transaction(fn ->
      with {:ok, org} <- %Schema{} |> Schema.changeset(attrs) |> TheRobotLearns.Repo.insert(),
           {:ok, _membership} <-
             TheRobotLearns.Authz.ScopedMemberships.add_member("organization", org.id, user_id, "owner") do
        org
      else
  ...
  def list_user_organizations(user_id) do
    from(sm in ScopedMembershipSchema,
      ...
      join: g in TheRobotLearns.Schema.Authz.Group,
      on: g.id == sm.group_id,
      where:
  ...
  def authorize(user_id, organization_id, required_role) do
    TheRobotLearns.Authz.authorize(user_id, "organization", organization_id, required_role)
  end

  def list_members(organization_id) do
    TheRobotLearns.Authz.ScopedMemberships.list_for_resource("organization", organization_id)
  end
  ...
  def find_active_invite_by_raw_token(raw_token, email \\ nil) when is_binary(raw_token) do
    ...
    email = email && TheRobotLearns.Users.Credentials.standardize_email(email)
    ...
    |> Enum.find(fn token ->
      email_matches? =
        is_nil(token.email) ||
          is_nil(email) ||
          TheRobotLearns.Users.Credentials.standardize_email(token.email) == email
  ...
  def increment_invite_uses(invite_token) do
    ...
    |> TheRobotLearns.Repo.update_all(inc: [uses: 1, redemption_count: 1])
  end
  ...
  def redeem_invite_for_user(invite_token, user, conn \\ nil) do
    ...
    TheRobotLearns.Repo.transaction(fn ->
      ...
      with {:ok, _redemption} <-
        ...
        {_, _} =
          from(t in InviteTokenSchema, where: t.id == ^invite_token.id)
          |> TheRobotLearns.Repo.update_all(
