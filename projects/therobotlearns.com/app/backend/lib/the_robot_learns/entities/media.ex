defmodule TheRobotLearns.Media do
  alias TheRobotLearns.Media.Asset, as: Entity
  alias TheRobotLearns.Schema.Media.Asset, as: Schema
  use Noizu.Repo
  ...
  def get_cached_variant(media_id, canonical_params) do
    ...
    TheRobotLearns.Repo.one(
      from v in TheRobotLearns.Schema.Media.Variant,
        where: v.media_id == ^media_id and v.params == ^canonical_params
    )
  end

  def cache_variant(attrs) do
    %TheRobotLearns.Schema.Media.Variant{}
    |> TheRobotLearns.Schema.Media.Variant.changeset(attrs)
    |> TheRobotLearns.Repo.insert(on_conflict: :nothing)
  end

  def fetch_from_s3(key) do
    config = Application.get_env(:the_robot_learns, TheRobotLearns.Storage, [])
  ...
  def get_or_create_variant(media, transform_params) do
    canonical = TheRobotLearns.Media.Transform.canonical_params(transform_params)
      ...
      nil ->
        with {:ok, original_binary} <- fetch_from_s3(media.file),
             {:ok, transformed, content_type} <-
               TheRobotLearns.Media.Transform.transform(original_binary, transform_params) do
          variant_key = TheRobotLearns.Media.Transform.variant_s3_key(media.file, transform_params)
