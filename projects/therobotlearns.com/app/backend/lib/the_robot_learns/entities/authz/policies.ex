defmodule TheRobotLearns.Authz.Policies do
  alias TheRobotLearns.Authz.Policies.Policy, as: Entity
  alias TheRobotLearns.Schema.Authz.Policy, as: Schema
  alias TheRobotLearns.Schema.Authz.UserPolicy, as: UserPolicySchema
  ...
  def list_active(opts \\ []) do
    ...
    TheRobotLearns.Repo.all(query)
  end

  def get_by_name(name) do
    TheRobotLearns.Repo.one(from p in Schema, where: p.name == ^name and p.is_active == true)
  end
    ...
    case TheRobotLearns.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          policy |> Schema.changeset(attrs) |> TheRobotLearns.Repo.update()
        end
    end
  end
    ...
    case TheRobotLearns.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          TheRobotLearns.Repo.delete(policy)
        end
    end
  end
  ...
  def detach_from_user(user_id, policy_id) do
    case TheRobotLearns.Repo.one(
      ...
      nil -> {:error, :not_found}
      user_policy -> TheRobotLearns.Repo.delete(user_policy)
    end
  end
end
