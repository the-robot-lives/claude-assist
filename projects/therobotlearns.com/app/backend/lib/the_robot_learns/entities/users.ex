defmodule TheRobotLearns.Users do
  @moduledoc """
  Context for TheRobotLearns.Users
  """
  alias TheRobotLearns.Users.User, as: Entity
  alias TheRobotLearns.Schema.Auth.Providers.Provider, as: ProviderSchema
  alias TheRobotLearns.Schema.Users.User, as: Schema
  alias TheRobotLearns.Schema.Users.Credentials.UserCredential, as: CredentialSchema
  alias TheRobotLearns.Schema.Versioned.Descriptions.Description, as: DescriptionSchema
  alias TheRobotLearns.Schema.Versioned.Names.Name, as: NameSchema
  use Noizu.Repo
  def_repo(entity: TheRobotLearns.Users.User)

  def list(context, options \\ []) do
    settings = Noizu.Entity.Meta.persistence(Entity) |> hd

    TheRobotLearns.Repo.all(Schema)
    |> Enum.map(fn record ->
  ...
  def register(details, {:login, {email, password}}, context, options) do
    ...
    email = TheRobotLearns.Users.Credentials.standardize_email(email)
    ...
    with :valid <- valid_user_name?(details.user_name),
      ...
      TheRobotLearns.Repo.transaction(fn ->
        with {:ok, name_record} <-
             ...
             auth_provider_id = UUID.uuid5(:oid, "TheRobotLearns.Schema.Auth.Providers.Provider@Login"),
             {:ok, _auth_provider} <-
               TheRobotLearns.Repo.insert(
                 %ProviderSchema{
                   id: auth_provider_id,
  ...
  def authenticate(auth = {:login, {_email, _password}}, context, options \\ nil) do
    with {:ok, session} <- TheRobotLearns.Users.Credentials.authenticate(auth, context, options) do
      {:ok, session}
    end
  end

  def by_handle(handle, context, options \\ []) do
    with record = %Schema{} <- TheRobotLearns.Repo.get_by(Schema, %{handle: handle}) do
      settings = Noizu.Entity.Meta.persistence(Entity) |> hd
      {:ok, entity} = Entity.from_record(record, settings, context, options)
  ...
  def user_name_available?(user_name, _context, _options \\ nil) do
    with %{} <- TheRobotLearns.Repo.get_by(Schema, %{user_name: user_name}) do
        ...
        fn
          ...
          {"name", value} ->
            case user.name do
              %Ecto.Changeset{} ->
                value =
                  TheRobotLearns.Versioned.Names.change(user.name.data, value)

                {:name, value}

              _ ->
                value =
                  TheRobotLearns.Versioned.Names.change(
                    user.name || %TheRobotLearns.Versioned.Names.Name{},
                    value
                  )
      ...
      |> put_in(
        ...
        {:embed,
         %{
           ...
           on_cast: :update,
           on_replace: :update,
           related: TheRobotLearns.Versioned.Names.Name
         }}
      )
  ...
  def valid_login?(email, password) do
    TheRobotLearns.Users.Credentials.valid_login?(email, password)
  end

  def login_available?(email, context, options \\ nil) do
    TheRobotLearns.Users.Credentials.login_available?(email, context, options)
  end
  ...
  defp unique_handle(handle, context, options) do
    case TheRobotLearns.Users.by_handle(handle, context, options) do
      nil ->
        {:ok, handle}
      ...
      {:ok, _} ->
        Enum.reduce_while(0..999, handle, fn suffix, _acc ->
          ...
          case TheRobotLearns.Users.by_handle(with_suffix, context, options) do
            nil ->
              {:halt, {:ok, with_suffix}}
