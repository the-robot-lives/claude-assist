...
if System.get_env("PHX_SERVER") do
  config :noizu_site, NoizuSiteWeb.Endpoint, server: true
end

config :noizu_site, :redis,
  uri: System.get_env("REDIS_URL") || "redis://localhost:6379/0",
  key_prefix: System.get_env("REDIS_KEY_PREFIX", "noizu_site:")
...
if otel_endpoint = System.get_env("OTEL_EXPORTER_OTLP_ENDPOINT") do
  ...
  config :opentelemetry,
    ...
    resource: %{
      "service.name" => System.get_env("OTEL_SERVICE_NAME") || "noizu_site-backend",
...
if config_env() == :prod do
  ...
  config :noizu_site, :dns_cluster_query, System.get_env("DNS_CLUSTER_QUERY")
  ...
  config :noizu_site, :frontend_url, System.get_env("FRONTEND_URL")
  ...
  config :noizu_site, :mail_from,
    {System.get_env("MAIL_FROM_NAME", "NoizuSite"),
     System.get_env("MAIL_FROM_ADDRESS", "noreply@noizu_site.local")}

  # ── Storage (S3/MinIO) ──────────────────────────────────────────
  if s3_bucket = System.get_env("S3_BUCKET") do
    config :noizu_site, NoizuSite.Storage,
      bucket: s3_bucket,
  ...
  if oidc_client_id = System.get_env("OIDC_CLIENT_ID") do
    ...
    config :noizu_site, :oidc_enabled, true
  end
  ...
  if saml_metadata = System.get_env("SAML_IDP_METADATA_URL") do
    ...
    config :noizu_site, :saml_enabled, true
  end

  # ── SSO: Social OAuth (each enabled when *_CLIENT_ID is set) ──
  config :noizu_site, :sso_require_invite, System.get_env("SSO_REQUIRE_INVITE") == "true"
  ...
  oauth_providers =
    if google_id = System.get_env("GOOGLE_CLIENT_ID") do
      ...
      config :noizu_site, :google_enabled, true
  ...
  oauth_providers =
    if fb_id = System.get_env("FACEBOOK_CLIENT_ID") do
      ...
      config :noizu_site, :facebook_enabled, true
  ...
  oauth_providers =
    if gh_id = System.get_env("GITHUB_CLIENT_ID") do
      ...
      config :noizu_site, :github_enabled, true
  ...
  #     config :noizu_site, :linkedin_enabled, true
