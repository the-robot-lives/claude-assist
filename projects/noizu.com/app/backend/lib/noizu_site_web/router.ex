defmodule NoizuSiteWeb.Router do
  use NoizuSiteWeb, :router
  ...
  pipeline :authenticated do
    plug NoizuSiteWeb.AuthPipeline
  end
  ...
  pipeline :rate_limited_auth do
    plug NoizuSiteWeb.Plugs.RateLimit, action: :auth
  end

  pipeline :rate_limited_sensitive do
    plug NoizuSiteWeb.Plugs.RateLimit, action: :auth_sensitive
  end

  pipeline :org_viewer do
    plug NoizuSiteWeb.Plugs.RequireRole, role: "viewer"
  end

  pipeline :org_editor do
    plug NoizuSiteWeb.Plugs.RequireRole, role: "editor"
  end

  pipeline :org_admin do
    plug NoizuSiteWeb.Plugs.RequireRole, role: "admin"
  end

  pipeline :org_owner do
    plug NoizuSiteWeb.Plugs.RequireRole, role: "owner"
  end

  pipeline :admin do
    plug NoizuSiteWeb.Plugs.RequireAdmin
  end

  scope "/", NoizuSiteWeb do
    pipe_through :api
    get "/health", HealthController, :index
  end

  scope "/api/v1", NoizuSiteWeb do
  ...
  if Application.compile_env(:noizu_site, :saml_enabled) do
    scope "/sso/saml" do
  ...
  # OIDC redirect flow
  scope "/auth/oidc", NoizuSiteWeb do
    pipe_through [:sso_session]
  ...
  if Application.compile_env(:noizu_site, :dev_routes) do
    import Phoenix.LiveDashboard.Router

    scope "/dev" do
      pipe_through [:fetch_session, :protect_from_forgery]
      live_dashboard "/dashboard", metrics: NoizuSiteWeb.Telemetry
    end
  end
end
