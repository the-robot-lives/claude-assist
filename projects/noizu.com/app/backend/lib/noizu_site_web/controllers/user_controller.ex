defmodule NoizuSiteWeb.UserController do
  use NoizuSiteWeb, :controller

  alias NoizuSite.Guardian
  alias NoizuSite.Users
  ...
  defp validate_update_params(user, params) do
    if params["new_password"] && params["current_password"] do
      ...
      q =
        from c in NoizuSite.Schema.Users.Credentials.UserCredential,
  ...
  defp apply_updates(user, params, _context) do
    ...
    if map_size(updates) > 0 do
      ...
      |> NoizuSite.Repo.update_all(set: Enum.to_list(updates))
    end

    if params["new_password"] do
      NoizuSite.Users.Credentials.update_password(user, params["new_password"], Noizu.Context.system())
    end
