defmodule NoizuSite.Organizations do
  @moduledoc """
  Context for NoizuSite.Organizations
  """
  alias NoizuSite.Organizations.Organization, as: Entity
  alias NoizuSite.Schema.Organizations.Organization, as: Schema
  alias NoizuSite.Schema.Organizations.InviteToken, as: InviteTokenSchema
  alias NoizuSite.Schema.Authz.ScopedMembership, as: ScopedMembershipSchema
  use Noizu.Repo
  ...
  def create_organization_with_owner(attrs, user_id) do
    NoizuSite.Repo.transaction(fn ->
      with {:ok, org} <- %Schema{} |> Schema.changeset(attrs) |> NoizuSite.Repo.insert(),
           {:ok, _membership} <- NoizuSite.Authz.ScopedMemberships.add_member("organization", org.id, user_id, "owner") do
        org
      else
  ...
  def list_user_organizations(user_id) do
    from(sm in ScopedMembershipSchema,
      ...
      join: g in NoizuSite.Schema.Authz.Group, on: g.id == sm.group_id,
  ...
  def authorize(user_id, organization_id, required_role) do
    NoizuSite.Authz.authorize(user_id, "organization", organization_id, required_role)
  end

  def list_members(organization_id) do
    NoizuSite.Authz.ScopedMemberships.list_for_resource("organization", organization_id)
  end
  ...
  def increment_invite_uses(invite_token) do
    ...
    |> NoizuSite.Repo.update_all(inc: [uses: 1])
  end
end
