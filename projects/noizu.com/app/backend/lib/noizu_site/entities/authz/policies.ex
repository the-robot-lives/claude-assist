defmodule NoizuSite.Authz.Policies do
  alias NoizuSite.Authz.Policies.Policy, as: Entity
  alias NoizuSite.Schema.Authz.Policy, as: Schema
  alias NoizuSite.Schema.Authz.UserPolicy, as: UserPolicySchema
  ...
  def list_active(opts \\ []) do
    ...
    NoizuSite.Repo.all(query)
  end

  def get_by_name(name) do
    NoizuSite.Repo.one(from p in Schema, where: p.name == ^name and p.is_active == true)
  end
    ...
    case NoizuSite.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          policy |> Schema.changeset(attrs) |> NoizuSite.Repo.update()
        end
    end
  end
  ...
  def detach_from_user(user_id, policy_id) do
    case NoizuSite.Repo.one(
      from up in UserPolicySchema,
        where: up.user_id == ^user_id and up.policy_id == ^policy_id
    ) do
      nil -> {:error, :not_found}
      user_policy -> NoizuSite.Repo.delete(user_policy)
    end
  end
end
