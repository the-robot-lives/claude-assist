defmodule NoizuSite.Media do
  alias NoizuSite.Media.Asset, as: Entity
  alias NoizuSite.Schema.Media.Asset, as: Schema
  use Noizu.Repo
  ...
  def get_cached_variant(media_id, canonical_params) do
    import Ecto.Query
    NoizuSite.Repo.one(
      from v in NoizuSite.Schema.Media.Variant,
        where: v.media_id == ^media_id and v.params == ^canonical_params
    )
  end

  def cache_variant(attrs) do
    %NoizuSite.Schema.Media.Variant{}
    |> NoizuSite.Schema.Media.Variant.changeset(attrs)
    |> NoizuSite.Repo.insert(on_conflict: :nothing)
  end

  def fetch_from_s3(key) do
    config = Application.get_env(:noizu_site, NoizuSite.Storage, [])
  ...
  def get_or_create_variant(media, transform_params) do
    canonical = NoizuSite.Media.Transform.canonical_params(transform_params)
      ...
      nil ->
        with {:ok, original_binary} <- fetch_from_s3(media.file),
             {:ok, transformed, content_type} <-
               NoizuSite.Media.Transform.transform(original_binary, transform_params) do
          variant_key = NoizuSite.Media.Transform.variant_s3_key(media.file, transform_params)
