defmodule NoizuSite.Users.Credentials do
  @moduledoc """
  Context for NoizuSite.Users.Credentials
  """
  alias NoizuSite.Users.Credentials.UserCredential, as: Entity
  alias NoizuSite.Schema.Users.Credentials.UserCredential, as: Schema
  use Noizu.Repo
  import Ecto.Query, only: [from: 2]

  def_repo(entity: NoizuSite.Users.Credentials.UserCredential)
  ...
  def register(user, {:login, {email, password}}, context, _options) do
    {:ok, auth_provider} = NoizuSite.Auth.Providers.login()

    {:ok, description} =
      %NoizuSite.Versioned.Strings.String{
        content: "",
  ...
  def authenticate({:login, {email, password}}, context, options) do
    {:ok, auth_provider} = NoizuSite.Auth.Providers.login()
    {:ok, auth_provider_id} = NoizuSite.Auth.Providers.Provider.id(auth_provider)
        ...
        [credential] ->
          if Bcrypt.verify_pass(password, credential.settings["password"]) do
            with {:ok, credential_entity} <-
                   NoizuSite.Users.Credentials.UserCredential.entity(credential.id, context),
              ...
              |> NoizuSite.EntityRepo.create(context, options)
            end
          else
