defmodule NoizuSite.Auth.SSO do
  alias NoizuSite.Schema.Users.User, as: UserSchema
  alias NoizuSite.Schema.Users.Credentials.UserCredential, as: CredentialSchema
  alias NoizuSite.Schema.Versioned.Names.Name
  import Ecto.Query, only: [from: 2]

  @provider_map %{
    oidc: &NoizuSite.Auth.Providers.oidc/0,
    saml: &NoizuSite.Auth.Providers.saml/0,
    google: &NoizuSite.Auth.Providers.google/0,
    facebook: &NoizuSite.Auth.Providers.facebook/0,
    github: &NoizuSite.Auth.Providers.github/0,
    linkedin: &NoizuSite.Auth.Providers.linkedin/0
  }
  ...
  def authenticate_sso(provider_type, %{email: email} = attrs) do
    ...
    {:ok, provider_id} = NoizuSite.Auth.Providers.Provider.id(provider_ref)
    ...
    case find_user_by_email(email) do
      ...
      :not_found ->
        if Application.get_env(:noizu_site, :sso_require_invite, false) do
  ...
  defp ensure_sso_credential(user, provider_ref, provider_id, provider_type, attrs, context) do
    ...
    case NoizuSite.Repo.one(q) do
      nil ->
        %NoizuSite.Users.Credentials.UserCredential{
          user: NoizuSite.Users.User.ref(user.id),
  ...
  defp create_sso_session(user, provider_type, context) do
    ...
    %NoizuSite.Users.Sessions.UserSession{
      user: user_ref,
      status: :active,
  ...
  defp auto_provision_user(email, attrs, provider_ref, provider_id, provider_type, context) do
    ...
    {:ok, name} =
      NoizuSite.EntityRepo.create(
        %NoizuSite.Versioned.Names.Name{first: first, last: last, time_stamp: Noizu.Entity.TimeStamp.now()},
        context
      )
    ...
    {:ok, user} = NoizuSite.Repo.insert(user_schema, on_conflict: :nothing, conflict_target: :email)
