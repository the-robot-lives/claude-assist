defmodule NoizuSiteWeb.ConnCase do
  ...
  using do
    quote do
      ...
      import NoizuSiteWeb.ConnCase

      @endpoint NoizuSiteWeb.Endpoint
    end
  end
  ...
  def setup_user_and_token(_context \\ %{}) do
    ...
    NoizuSite.Repo.insert!(
      %NoizuSite.Schema.Auth.Providers.Provider{
    ...
    user = %NoizuSite.Schema.Users.User{
    ...
    {:ok, user} = NoizuSite.Repo.insert(user)

    credential = %NoizuSite.Schema.Users.Credentials.UserCredential{
      id: Ecto.UUID.generate(),
      user_id: user.id,
    ...
    {:ok, _credential} = NoizuSite.Repo.insert(credential)

    session = %NoizuSite.Schema.Users.Sessions.UserSession{
      id: Ecto.UUID.generate(),
      user_id: user.id,
      status: :active,
      details: %{}
    ...
    {:ok, session} = NoizuSite.Repo.insert(session)

    # Build entity-layer session struct for Guardian signing
    session_entity = %NoizuSite.Users.Sessions.UserSession{
      id: session.id,
      user: {:ref, NoizuSite.Users.User, user.id},
      status: :active,
      details: %{}
    }

    {:ok, access_token, _claims} =
      NoizuSite.Guardian.encode_and_sign(session_entity, %{}, token_type: "access", ttl: {1, :hour})

    {:ok, refresh_token, %{"jti" => jti}} =
      NoizuSite.Guardian.encode_and_sign(session_entity, %{}, token_type: "refresh", ttl: {7, :day})

    NoizuSite.Auth.TokenStore.store_refresh_jti(jti)
