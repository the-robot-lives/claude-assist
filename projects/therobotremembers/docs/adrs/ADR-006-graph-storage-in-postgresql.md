...
> Amended by ADR-013 (Apache AGE Projection Layer): AGE is adopted as a projection of
> `association_edges`, not the migration target that replaces the recursive CTE — the CTE
> remains the recall path.
...
- ADR-013: Apache AGE Projection Layer — settles the "evaluate AGE first" migration path: AGE becomes a projection for Cypher/analytics while recall stays on the CTE
