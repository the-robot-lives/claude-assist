defmodule TheRobotRemembersWeb.ConnCase do
  ...
  using do
    quote do
      ...
      import TheRobotRemembersWeb.ConnCase

      @endpoint TheRobotRemembersWeb.Endpoint
    end
  end
  ...
  def setup_user_and_token(_context \\ %{}) do
    ...
    TheRobotRemembers.Repo.insert!(
      %TheRobotRemembers.Schema.Auth.Providers.Provider{
    ...
    user = %TheRobotRemembers.Schema.Users.User{
    ...
    {:ok, user} = TheRobotRemembers.Repo.insert(user)

    credential = %TheRobotRemembers.Schema.Users.Credentials.UserCredential{
      id: Ecto.UUID.generate(),
      user_id: user.id,
    ...
    {:ok, _credential} = TheRobotRemembers.Repo.insert(credential)

    session = %TheRobotRemembers.Schema.Users.Sessions.UserSession{
      id: Ecto.UUID.generate(),
      user_id: user.id,
      status: :active,
      details: %{}
    ...
    {:ok, session} = TheRobotRemembers.Repo.insert(session)

    # Build entity-layer session struct for Guardian signing
    session_entity = %TheRobotRemembers.Users.Sessions.UserSession{
      id: session.id,
      user: {:ref, TheRobotRemembers.Users.User, user.id},
      status: :active,
      details: %{}
    }

    {:ok, access_token, _claims} =
      TheRobotRemembers.Guardian.encode_and_sign(session_entity, %{}, token_type: "access", ttl: {1, :hour})

    {:ok, refresh_token, %{"jti" => jti}} =
      TheRobotRemembers.Guardian.encode_and_sign(session_entity, %{}, token_type: "refresh", ttl: {7, :day})

    TheRobotRemembers.Auth.TokenStore.store_refresh_jti(jti)
