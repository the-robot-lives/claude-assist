...
if System.get_env("PHX_SERVER") do
  config :the_robot_remembers, TheRobotRemembersWeb.Endpoint, server: true
end

config :the_robot_remembers, :redis,
...
if config_env() == :prod do
  ...
  config :the_robot_remembers, :dns_cluster_query, System.get_env("DNS_CLUSTER_QUERY")
  ...
  config :the_robot_remembers, :frontend_url, System.get_env("FRONTEND_URL")
  ...
  config :the_robot_remembers, :mail_from,
    {System.get_env("MAIL_FROM_NAME", "TheRobotRemembers"),
     System.get_env("MAIL_FROM_ADDRESS", "noreply@starter.local")}

  # ── Storage (S3/MinIO) ──────────────────────────────────────────
  if s3_bucket = System.get_env("S3_BUCKET") do
    config :the_robot_remembers, TheRobotRemembers.Storage,
      bucket: s3_bucket,
  ...
  if oidc_client_id = System.get_env("OIDC_CLIENT_ID") do
    ...
    config :the_robot_remembers, :oidc_enabled, true
  end
  ...
  if saml_metadata = System.get_env("SAML_IDP_METADATA_URL") do
    ...
    config :the_robot_remembers, :saml_enabled, true
  end

  # ── SSO: Social OAuth (each enabled when *_CLIENT_ID is set) ──
  config :the_robot_remembers, :sso_require_invite, System.get_env("SSO_REQUIRE_INVITE") == "true"
  ...
  oauth_providers =
    if google_id = System.get_env("GOOGLE_CLIENT_ID") do
      ...
      config :the_robot_remembers, :google_enabled, true
  ...
  oauth_providers =
    if fb_id = System.get_env("FACEBOOK_CLIENT_ID") do
      ...
      config :the_robot_remembers, :facebook_enabled, true
  ...
  oauth_providers =
    if gh_id = System.get_env("GITHUB_CLIENT_ID") do
      ...
      config :the_robot_remembers, :github_enabled, true
  ...
  #     config :the_robot_remembers, :linkedin_enabled, true
