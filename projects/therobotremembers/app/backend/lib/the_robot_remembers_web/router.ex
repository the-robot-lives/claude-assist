defmodule TheRobotRemembersWeb.Router do
  use TheRobotRemembersWeb, :router
  ...
  pipeline :authenticated do
    plug TheRobotRemembersWeb.AuthPipeline
  end
  ...
  pipeline :rate_limited_auth do
    plug TheRobotRemembersWeb.Plugs.RateLimit, action: :auth
  end

  pipeline :rate_limited_sensitive do
    plug TheRobotRemembersWeb.Plugs.RateLimit, action: :auth_sensitive
  end

  pipeline :org_viewer do
    plug TheRobotRemembersWeb.Plugs.RequireRole, role: "viewer"
  end

  pipeline :org_editor do
    plug TheRobotRemembersWeb.Plugs.RequireRole, role: "editor"
  end

  pipeline :org_admin do
    plug TheRobotRemembersWeb.Plugs.RequireRole, role: "admin"
  end

  pipeline :org_owner do
    plug TheRobotRemembersWeb.Plugs.RequireRole, role: "owner"
  end
  ...
  scope "/media", TheRobotRemembersWeb do
    pipe_through [:api]
  ...
  if Application.compile_env(:the_robot_remembers, :saml_enabled) do
    scope "/sso/saml" do
  ...
  # OIDC redirect flow
  scope "/auth/oidc", TheRobotRemembersWeb do
    pipe_through [:sso_session]
  ...
  scope "/auth", TheRobotRemembersWeb do
    pipe_through [:sso_session]
  ...
  if Application.compile_env(:the_robot_remembers, :dev_routes) do
    import Phoenix.LiveDashboard.Router

    scope "/dev" do
      pipe_through [:fetch_session, :protect_from_forgery]
      live_dashboard "/dashboard", metrics: TheRobotRemembersWeb.Telemetry
    end
  end
end
