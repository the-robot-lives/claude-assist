defmodule TheRobotRemembersWeb.UserController do
  use TheRobotRemembersWeb, :controller

  alias TheRobotRemembers.Guardian
  alias TheRobotRemembers.Users
  ...
  defp get_current_user(conn) do
    ...
    case session.user do
      ...
      %TheRobotRemembers.Users.User{} = user ->
        user
    end
  end
  ...
  defp validate_update_params(user, params) do
    if params["new_password"] && params["current_password"] do
      ...
      q =
        from c in TheRobotRemembers.Schema.Users.Credentials.UserCredential,
  ...
  defp apply_updates(user, params, _context) do
    ...
    if map_size(updates) > 0 do
      ...
      |> TheRobotRemembers.Repo.update_all(set: Enum.to_list(updates))
    end

    if params["new_password"] do
      TheRobotRemembers.Users.Credentials.update_password(user, params["new_password"], Noizu.Context.system())
    end
