defmodule TheRobotRemembersWeb.MediaServeController do
  use TheRobotRemembersWeb, :controller

  alias TheRobotRemembers.Media
  alias TheRobotRemembers.Media.Transform
  ...
  defp serve_original(conn, media) do
    if media.visibility == "public" do
      url = TheRobotRemembers.Storage.presigned_download_url(media.file)
  ...
  defp serve_transformed(conn, media, transform_params) do
    case Media.get_or_create_variant(media, transform_params) do
      {:ok, variant_key, content_type} ->
        if media.visibility == "public" do
          url = TheRobotRemembers.Storage.presigned_download_url(variant_key)
      ...
      case Plug.Conn.get_req_header(conn, "authorization") do
        ["Bearer " <> token] ->
          case TheRobotRemembers.Guardian.decode_and_verify(token) do
            {:ok, claims} ->
              case TheRobotRemembers.Guardian.resource_from_claims(claims) do
                {:ok, %TheRobotRemembers.Users.Sessions.UserSession{user: {:ref, _, id}}} -> id
                {:ok, %TheRobotRemembers.Users.Sessions.UserSession{user: %{id: id}}} -> id
                _ -> nil
              end
