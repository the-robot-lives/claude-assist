defmodule TheRobotRemembersWeb.SSOController do
  ...
  def exchange(conn, %{"code" => code}) do
    with {:ok, session_id} <- TheRobotRemembers.Auth.SSOCode.exchange(code),
         {:ok, session} <- TheRobotRemembers.Users.Sessions.get(session_id, Noizu.Context.system()),
  ...
  defp handle_sso_callback(conn, provider_type, attrs) do
    ...
    case TheRobotRemembers.Auth.SSO.authenticate_sso(provider_type, attrs) do
      {:ok, session} ->
        {:ok, code} = TheRobotRemembers.Auth.SSOCode.create(session.id)
  ...
  defp maybe_add(list, flag, name) do
    if Application.get_env(:the_robot_remembers, flag), do: [name | list], else: list
  end
end
