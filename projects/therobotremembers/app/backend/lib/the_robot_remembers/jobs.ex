defmodule TheRobotRemembers.Jobs do
  @moduledoc """
  ...
  job volume (observed as a `DBConnection.OwnershipError` that poisons a later, unrelated query
  in the same test). Running `perform/1` directly in the test process keeps every DB write on the
  test's own checked-out connection. Args are JSON round-tripped first so the worker sees the same
  ...
  def mode, do: Application.get_env(:the_robot_remembers, :jobs_mode, :oban)
  ...
  defp perform_sync(worker, args) do
    ...
    e ->
      Logger.warning("[Jobs] sync #{inspect(worker)} failed: #{inspect(e)}")
      {:error, e}
  catch
