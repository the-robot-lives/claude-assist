defmodule TheRobotRemembers.Auth.SSO do
  alias TheRobotRemembers.Schema.Users.User, as: UserSchema
  alias TheRobotRemembers.Schema.Users.Credentials.UserCredential, as: CredentialSchema
  alias TheRobotRemembers.Schema.Versioned.Names.Name
  import Ecto.Query, only: [from: 2]

  @provider_map %{
    oidc: &TheRobotRemembers.Auth.Providers.oidc/0,
    saml: &TheRobotRemembers.Auth.Providers.saml/0,
    google: &TheRobotRemembers.Auth.Providers.google/0,
    facebook: &TheRobotRemembers.Auth.Providers.facebook/0,
    github: &TheRobotRemembers.Auth.Providers.github/0,
    linkedin: &TheRobotRemembers.Auth.Providers.linkedin/0
  }
  ...
  def authenticate_sso(provider_type, %{email: email} = attrs) do
    ...
    {:ok, provider_id} = TheRobotRemembers.Auth.Providers.Provider.id(provider_ref)
    ...
    case find_user_by_email(email) do
      ...
      :not_found ->
        if Application.get_env(:the_robot_remembers, :sso_require_invite, false) do
  ...
  defp find_user_by_email(email) do
    ...
    case TheRobotRemembers.Repo.one(q) do
      nil -> :not_found
      user -> {:ok, user}
    end
  end
  ...
  defp ensure_sso_credential(user, provider_ref, provider_id, provider_type, attrs, context) do
    ...
    case TheRobotRemembers.Repo.one(q) do
      nil ->
        %TheRobotRemembers.Users.Credentials.UserCredential{
          user: TheRobotRemembers.Users.User.ref(user.id),
  ...
  defp create_sso_session(user, provider_type, context) do
    ...
    %TheRobotRemembers.Users.Sessions.UserSession{
      user: user_ref,
      status: :active,
  ...
  defp auto_provision_user(email, attrs, provider_ref, provider_id, provider_type, context) do
    ...
    {:ok, name} =
      TheRobotRemembers.EntityRepo.create(
        %TheRobotRemembers.Versioned.Names.Name{first: first, last: last, time_stamp: Noizu.Entity.TimeStamp.now()},
        context
      )
    ...
    {:ok, user} = TheRobotRemembers.Repo.insert(user_schema, on_conflict: :nothing, conflict_target: :email)
