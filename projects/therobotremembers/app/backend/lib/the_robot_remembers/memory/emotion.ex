defmodule TheRobotRemembers.Memory.Emotion do
  @moduledoc """
  Emotional-vector math. The 7-d resonance vector is `agent VAD ++ Monitor hormones`,
  pre-weighted so L2 distance reflects the VAD-vs-hormone split. Valence is normalized
  from [-1,1] to [0,1] for the vector; the raw components are kept on the row for
  explainability (see `Schema.Memory.Memory`).
  """

  @order ~w(valence arousal dominance cortisol dopamine oxytocin serotonin)a

  def config, do: Application.get_env(:the_robot_remembers, :emotion, [])
  ...
  @doc "Coarse VAD-only hot-index bucket: 4 valence × 3 arousal × 3 dominance = 36 buckets."
