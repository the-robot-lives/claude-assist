defmodule TheRobotRemembers.Users.Credentials do
  @moduledoc """
  Context for TheRobotRemembers.Users.Credentials
  """
  alias TheRobotRemembers.Users.Credentials.UserCredential, as: Entity
  alias TheRobotRemembers.Schema.Users.Credentials.UserCredential, as: Schema
  use Noizu.Repo
  import Ecto.Query, only: [from: 2]

  def_repo(entity: TheRobotRemembers.Users.Credentials.UserCredential)
  ...
  def register(user, {:login, {email, password}}, context, _options) do
    {:ok, auth_provider} = TheRobotRemembers.Auth.Providers.login()

    {:ok, description} =
      %TheRobotRemembers.Versioned.Strings.String{
        content: "",
  ...
  def authenticate({:login, {email, password}}, context, options) do
    {:ok, auth_provider} = TheRobotRemembers.Auth.Providers.login()
    {:ok, auth_provider_id} = TheRobotRemembers.Auth.Providers.Provider.id(auth_provider)
    ...
    with :valid <- valid_login?(email, password) do
      ...
      case TheRobotRemembers.Repo.all(q) do
        [] ->
        ...
        [credential] ->
          if Bcrypt.verify_pass(password, credential.settings["password"]) do
            with {:ok, credential_entity} <-
                   TheRobotRemembers.Users.Credentials.UserCredential.entity(credential.id, context),
              ...
              |> TheRobotRemembers.EntityRepo.create(context, options)
            end
          else
