defmodule TheRobotRemembers.Authz.Policies do
  alias TheRobotRemembers.Authz.Policies.Policy, as: Entity
  alias TheRobotRemembers.Schema.Authz.Policy, as: Schema
  alias TheRobotRemembers.Schema.Authz.UserPolicy, as: UserPolicySchema
  ...
  def list_active(opts \\ []) do
    ...
    TheRobotRemembers.Repo.all(query)
  end

  def get_by_name(name) do
    TheRobotRemembers.Repo.one(from p in Schema, where: p.name == ^name and p.is_active == true)
  end
    ...
    case TheRobotRemembers.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          policy |> Schema.changeset(attrs) |> TheRobotRemembers.Repo.update()
        end
    end
  end
    ...
    case TheRobotRemembers.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          TheRobotRemembers.Repo.delete(policy)
        end
    end
  end
  ...
  def detach_from_user(user_id, policy_id) do
    case TheRobotRemembers.Repo.one(
      from up in UserPolicySchema,
        where: up.user_id == ^user_id and up.policy_id == ^policy_id
    ) do
      nil -> {:error, :not_found}
      user_policy -> TheRobotRemembers.Repo.delete(user_policy)
    end
  end
end
