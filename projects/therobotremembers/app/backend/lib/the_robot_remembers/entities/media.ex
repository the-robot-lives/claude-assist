defmodule TheRobotRemembers.Media do
  alias TheRobotRemembers.Media.Asset, as: Entity
  alias TheRobotRemembers.Schema.Media.Asset, as: Schema
  use Noizu.Repo
  ...
  def get_cached_variant(media_id, canonical_params) do
    import Ecto.Query
    TheRobotRemembers.Repo.one(
      from v in TheRobotRemembers.Schema.Media.Variant,
        where: v.media_id == ^media_id and v.params == ^canonical_params
    )
  end

  def cache_variant(attrs) do
    %TheRobotRemembers.Schema.Media.Variant{}
    |> TheRobotRemembers.Schema.Media.Variant.changeset(attrs)
    |> TheRobotRemembers.Repo.insert(on_conflict: :nothing)
  end

  def fetch_from_s3(key) do
    config = Application.get_env(:the_robot_remembers, TheRobotRemembers.Storage, [])
  ...
  def get_or_create_variant(media, transform_params) do
    canonical = TheRobotRemembers.Media.Transform.canonical_params(transform_params)
      ...
      nil ->
        with {:ok, original_binary} <- fetch_from_s3(media.file),
             {:ok, transformed, content_type} <-
               TheRobotRemembers.Media.Transform.transform(original_binary, transform_params) do
          variant_key = TheRobotRemembers.Media.Transform.variant_s3_key(media.file, transform_params)
