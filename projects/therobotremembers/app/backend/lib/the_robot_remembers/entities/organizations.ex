defmodule TheRobotRemembers.Organizations do
  @moduledoc """
  Context for TheRobotRemembers.Organizations
  """
  alias TheRobotRemembers.Organizations.Organization, as: Entity
  alias TheRobotRemembers.Schema.Organizations.Organization, as: Schema
  alias TheRobotRemembers.Schema.Organizations.InviteToken, as: InviteTokenSchema
  alias TheRobotRemembers.Schema.Authz.ScopedMembership, as: ScopedMembershipSchema
  use Noizu.Repo
  ...
  def create_organization_with_owner(attrs, user_id) do
    TheRobotRemembers.Repo.transaction(fn ->
      with {:ok, org} <- %Schema{} |> Schema.changeset(attrs) |> TheRobotRemembers.Repo.insert(),
           {:ok, _membership} <- TheRobotRemembers.Authz.ScopedMemberships.add_member("organization", org.id, user_id, "owner") do
        org
      else
        {:error, reason} -> TheRobotRemembers.Repo.rollback(reason)
      end
    end)
  end
  ...
  def list_user_organizations(user_id) do
    from(sm in ScopedMembershipSchema,
      ...
      join: g in TheRobotRemembers.Schema.Authz.Group, on: g.id == sm.group_id,
  ...
  def authorize(user_id, organization_id, required_role) do
    TheRobotRemembers.Authz.authorize(user_id, "organization", organization_id, required_role)
  end

  def list_members(organization_id) do
    TheRobotRemembers.Authz.ScopedMemberships.list_for_resource("organization", organization_id)
  end
  ...
  def increment_invite_uses(invite_token) do
    ...
    |> TheRobotRemembers.Repo.update_all(inc: [uses: 1])
  end
end
