defmodule HoloGraphWeb.CustomRoleController do
  use HoloGraphWeb, :controller

  alias HoloGraph.Schema.Organizations.CustomRole, as: RoleSchema
  alias HoloGraph.Schema.Organizations.CustomRolePermission, as: PermSchema
  ...
  def create(conn, %{"org_id" => org_id, "role" => role_params}) do
    ...
    if Authz.check_permission(user_id, "organization", org_id, "organization:manage_settings") do
      ...
      case %RoleSchema{} |> RoleSchema.changeset(attrs) |> HoloGraph.Repo.insert() do
        {:ok, role} ->
  ...
  def update(conn, %{"org_id" => org_id, "id" => role_id, "role" => role_params}) do
    ...
    if Authz.check_permission(user_id, "organization", org_id, "organization:manage_settings") do
      case HoloGraph.Repo.one(
        ...
        role ->
          case role |> RoleSchema.changeset(role_params) |> HoloGraph.Repo.update() do
            {:ok, updated} ->
  ...
  def delete(conn, %{"org_id" => org_id, "id" => role_id}) do
    ...
    if Authz.check_permission(user_id, "organization", org_id, "organization:manage_settings") do
      case HoloGraph.Repo.one(
        ...
        role ->
          case role |> Ecto.Changeset.change(is_active: false) |> HoloGraph.Repo.update() do
            {:ok, _} ->
  ...
  def remove_permission(conn, %{
    ...
    if Authz.check_permission(user_id, "organization", org_id, "organization:manage_settings") do
      case HoloGraph.Repo.one(
        ...
        perm ->
          {:ok, _} = HoloGraph.Repo.delete(perm)
