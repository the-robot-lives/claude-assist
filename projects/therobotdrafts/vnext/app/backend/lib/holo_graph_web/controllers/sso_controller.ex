defmodule HoloGraphWeb.SSOController do
  ...
  def providers(conn, _params) do
    ...
    json(conn, %{
      providers: providers,
      domains: HoloGraph.Auth.SSODomains.providers_map(),
      domain_policies: HoloGraph.Auth.SSODomains.public_policies()
    })
  end
  ...
  def exchange(conn, %{"code" => code}) do
    with {:ok, session_id} <- HoloGraph.Auth.SSOCode.exchange(code),
         {:ok, session} <- HoloGraph.Users.Sessions.get(session_id, Noizu.Context.system(), []),
         {:ok, access_token, _} <-
  ...
  defp handle_sso_callback(conn, provider_type, attrs) do
    ...
    case HoloGraph.Auth.SSO.authenticate_sso(provider_type, attrs) do
      {:ok, session} ->
        {:ok, code} = HoloGraph.Auth.SSOCode.create(session.id)
  ...
  defp maybe_add(list, flag, name) do
    if Application.get_env(:holo_graph, flag), do: [name | list], else: list
  end
end
