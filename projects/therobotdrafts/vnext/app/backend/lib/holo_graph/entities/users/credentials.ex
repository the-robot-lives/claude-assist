defmodule HoloGraph.Users.Credentials do
  @moduledoc """
  Context for HoloGraph.Users.Credentials
  """
  alias HoloGraph.Users.Credentials.UserCredential, as: Entity
  alias HoloGraph.Schema.Users.Credentials.UserCredential, as: Schema
  use Noizu.Repo
  import Ecto.Query, only: [from: 2]

  def_repo(entity: HoloGraph.Users.Credentials.UserCredential)
  ...
  def register(user, {:login, {email, password}}, context, _options) do
    {:ok, auth_provider} = HoloGraph.Auth.Providers.login()

    {:ok, description} =
      %HoloGraph.Versioned.Strings.String{
        content: "",
  ...
  def authenticate({:login, {email, password}}, context, options) do
    {:ok, auth_provider} = HoloGraph.Auth.Providers.login()
    {:ok, auth_provider_id} = HoloGraph.Auth.Providers.Provider.id(auth_provider)
        ...
        [credential] ->
          if Bcrypt.verify_pass(password, credential.settings["password"]) do
            with {:ok, credential_entity} <-
                   HoloGraph.Users.Credentials.UserCredential.entity(credential.id, context),
                 {:ok, user} <-
              ...
              |> HoloGraph.EntityRepo.create(context, options)
            end
          else
