defmodule HoloGraph.Organizations do
  @moduledoc """
  Context for HoloGraph.Organizations
  """
  alias HoloGraph.Organizations.Organization, as: Entity
  alias HoloGraph.Schema.Organizations.Organization, as: Schema
  alias HoloGraph.Schema.Organizations.InviteToken, as: InviteTokenSchema
  alias HoloGraph.Schema.Organizations.InviteTokenRedemption, as: InviteTokenRedemptionSchema
  alias HoloGraph.Schema.Authz.ScopedMembership, as: ScopedMembershipSchema
  use Noizu.Repo
  ...
  def list_user_organizations(user_id) do
    from(sm in ScopedMembershipSchema,
      ...
      join: g in HoloGraph.Schema.Authz.Group,
      on: g.id == sm.group_id,
      where:
  ...
  def authorize(user_id, organization_id, required_role) do
    HoloGraph.Authz.authorize(user_id, "organization", organization_id, required_role)
  end

  def list_members(organization_id) do
    HoloGraph.Authz.ScopedMemberships.list_for_resource("organization", organization_id)
  end
  ...
  def find_active_invite_by_raw_token(raw_token, email \\ nil) when is_binary(raw_token) do
    ...
    email = email && HoloGraph.Users.Credentials.standardize_email(email)
    ...
    |> Enum.find(fn token ->
      email_matches? =
        is_nil(token.email) ||
          is_nil(email) ||
          HoloGraph.Users.Credentials.standardize_email(token.email) == email
  ...
  def increment_invite_uses(invite_token) do
    ...
    |> HoloGraph.Repo.update_all(inc: [uses: 1, redemption_count: 1])
  end
