defmodule HoloGraph.Authz.Policies do
  alias HoloGraph.Authz.Policies.Policy, as: Entity
  alias HoloGraph.Schema.Authz.Policy, as: Schema
  alias HoloGraph.Schema.Authz.UserPolicy, as: UserPolicySchema
  ...
  def list_active(opts \\ []) do
    ...
    HoloGraph.Repo.all(query)
  end

  def get_by_name(name) do
    HoloGraph.Repo.one(from p in Schema, where: p.name == ^name and p.is_active == true)
  end
    ...
    case HoloGraph.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          policy |> Schema.changeset(attrs) |> HoloGraph.Repo.update()
        end
    end
  end
  ...
  def detach_from_user(user_id, policy_id) do
    case HoloGraph.Repo.one(
      ...
      nil -> {:error, :not_found}
      user_policy -> HoloGraph.Repo.delete(user_policy)
    end
  end
end
