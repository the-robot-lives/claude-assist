defmodule HoloGraph.Media do
  alias HoloGraph.Media.Asset, as: Entity
  alias HoloGraph.Schema.Media.Asset, as: Schema
  use Noizu.Repo
  ...
  def get_cached_variant(media_id, canonical_params) do
    ...
    HoloGraph.Repo.one(
      from v in HoloGraph.Schema.Media.Variant,
        where: v.media_id == ^media_id and v.params == ^canonical_params
    )
  end

  def cache_variant(attrs) do
    %HoloGraph.Schema.Media.Variant{}
    |> HoloGraph.Schema.Media.Variant.changeset(attrs)
    |> HoloGraph.Repo.insert(on_conflict: :nothing)
  end

  def fetch_from_s3(key) do
    config = Application.get_env(:holo_graph, HoloGraph.Storage, [])
  ...
  def get_or_create_variant(media, transform_params) do
    canonical = HoloGraph.Media.Transform.canonical_params(transform_params)
      ...
      nil ->
        with {:ok, original_binary} <- fetch_from_s3(media.file),
             {:ok, transformed, content_type} <-
               HoloGraph.Media.Transform.transform(original_binary, transform_params) do
          variant_key = HoloGraph.Media.Transform.variant_s3_key(media.file, transform_params)
