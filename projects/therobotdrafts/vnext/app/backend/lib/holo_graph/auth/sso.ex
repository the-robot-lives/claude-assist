defmodule HoloGraph.Auth.SSO do
  ...
  @provider_map %{
    oidc: &HoloGraph.Auth.Providers.oidc/0,
    saml: &HoloGraph.Auth.Providers.saml/0,
    google: &HoloGraph.Auth.Providers.google/0,
    facebook: &HoloGraph.Auth.Providers.facebook/0,
    github: &HoloGraph.Auth.Providers.github/0,
    linkedin: &HoloGraph.Auth.Providers.linkedin/0
  }
  ...
  def authenticate_sso(provider_type, %{email: email} = attrs) do
    ...
    with provider_type when not is_nil(provider_type) <- provider_type,
         true <- HoloGraph.Auth.SSODomains.sso_available?(email, provider_type) do
      provider_ref = provider_ref(provider_type)
      {:ok, provider_id} = HoloGraph.Auth.Providers.Provider.id(provider_ref)
  ...
  defp ensure_sso_credential(user, provider_ref, provider_id, provider_type, attrs, context) do
    ...
    case HoloGraph.Repo.one(q) do
      nil ->
        ...
        |> HoloGraph.EntityRepo.create(context)
  ...
  defp create_sso_session(user, provider_type, context) do
    ...
    %HoloGraph.Users.Sessions.UserSession{
      user: user_ref,
      status: :active,
  ...
  defp auto_provision_user(email, attrs, provider_ref, _provider_id, provider_type, context) do
    ...
    {:ok, name} =
      %HoloGraph.Schema.Versioned.Names.Name{
        first: first,
        last: last
      }
    ...
    user_schema = %UserSchema{
      ...
      name_id: name.id,
      email: email,
      status: HoloGraph.Auth.SSODomains.registration_status(email, provider_type),
      verified: true,
      flagged: false,
  ...
  defp auto_approved_at(email, provider_type) do
    if HoloGraph.Auth.SSODomains.auto_approve?(email, provider_type), do: DateTime.utc_now()
  end
