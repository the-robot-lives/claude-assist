defmodule HoloGraphWeb.AuthControllerTest do
  use HoloGraphWeb.ConnCase
  import Ecto.Query

  alias HoloGraph.Schema.Organizations.InviteToken
  alias HoloGraph.Schema.Organizations.InviteTokenRedemption
  ...
  describe "POST /api/v1/auth/register" do
    test "creates complete password registrations as pending without invite", %{conn: conn} do
      ...
      user = HoloGraph.Repo.get_by!(User, email: email)
    ...
    test "valid invite activates the user and records redemption", %{conn: conn} do
      ...
      user = HoloGraph.Repo.get_by!(User, email: email)
      invite = HoloGraph.Repo.get!(InviteToken, invite.id)
  ...
  defp create_invite(email, attrs) do
    ...
    HoloGraph.Organizations.create_invite_token(Map.merge(defaults, Map.new(attrs)))
  end
end
