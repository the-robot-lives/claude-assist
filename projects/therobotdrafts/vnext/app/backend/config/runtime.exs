...
if System.get_env("PHX_SERVER") do
  config :holo_graph, HoloGraphWeb.Endpoint, server: true
end

config :holo_graph, :redis,
...
if config_env() == :prod do
  ...
  config :holo_graph, :dns_cluster_query, System.get_env("DNS_CLUSTER_QUERY")
  ...
  config :holo_graph, :frontend_url, System.get_env("FRONTEND_URL")
  ...
  if oidc_client_id = System.get_env("OIDC_CLIENT_ID") do
    ...
    config :holo_graph, :oidc_enabled, true
  end
    ...
    config :samly, Samly.Provider,
      ...
      identity_providers: [
        %{
          ...
          pre_session_create_pipeline: HoloGraphWeb.SAMLHandler
        }
        |> Map.merge(Map.new(saml_metadata))
      ]

    config :holo_graph, :saml_enabled, true
  end

  # ── SSO: Social OAuth (each enabled when *_CLIENT_ID is set) ──
  config :holo_graph, :sso_require_invite, System.get_env("SSO_REQUIRE_INVITE") == "true"
  ...
  config :holo_graph, :sso_domains, sso_domains
  config :holo_graph, :sso_auto_approve_domains, sso_auto_approve_domains
  ...
  oauth_providers =
    if google_id = System.get_env("GOOGLE_CLIENT_ID") do
      ...
      config :holo_graph, :google_enabled, true
  ...
  oauth_providers =
    if fb_id = System.get_env("FACEBOOK_CLIENT_ID") do
      ...
      config :holo_graph, :facebook_enabled, true
  ...
  oauth_providers =
    if gh_id = System.get_env("GITHUB_CLIENT_ID") do
      ...
      config :holo_graph, :github_enabled, true
  ...
  #     config :holo_graph, :linkedin_enabled, true
