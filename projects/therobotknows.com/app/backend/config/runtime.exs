...
if System.get_env("PHX_SERVER") do
  config :therobotknows, TherobotknowsWeb.Endpoint, server: true
end

config :therobotknows, :redis,
  uri: System.get_env("REDIS_URL") || "redis://localhost:6379/0",
  key_prefix: System.get_env("REDIS_KEY_PREFIX", "therobotknows:")
...
if otel_endpoint = System.get_env("OTEL_EXPORTER_OTLP_ENDPOINT") do
  ...
  config :opentelemetry,
    ...
    resource: %{
      "service.name" => System.get_env("OTEL_SERVICE_NAME") || "therobotknows-backend",
...
if config_env() == :prod do
  ...
  config :therobotknows, :dns_cluster_query, System.get_env("DNS_CLUSTER_QUERY")
  ...
  config :therobotknows, :frontend_url, System.get_env("FRONTEND_URL")
  ...
  config :therobotknows, :mail_from,
    {System.get_env("MAIL_FROM_NAME", "Therobotknows"),
     System.get_env("MAIL_FROM_ADDRESS", "noreply@therobotknows.local")}

  # ── Storage (S3/MinIO) ──────────────────────────────────────────
  if s3_bucket = System.get_env("S3_BUCKET") do
    config :therobotknows, Therobotknows.Storage,
      bucket: s3_bucket,
  ...
  if oidc_client_id = System.get_env("OIDC_CLIENT_ID") do
    ...
    config :therobotknows, :oidc_enabled, true
  end
  ...
  if saml_metadata = System.get_env("SAML_IDP_METADATA_URL") do
    ...
    config :therobotknows, :saml_enabled, true
  end

  # ── SSO: Social OAuth (each enabled when *_CLIENT_ID is set) ──
  config :therobotknows, :sso_require_invite, System.get_env("SSO_REQUIRE_INVITE") == "true"
  ...
  oauth_providers =
    if google_id = System.get_env("GOOGLE_CLIENT_ID") do
      ...
      config :therobotknows, :google_enabled, true
  ...
  oauth_providers =
    if fb_id = System.get_env("FACEBOOK_CLIENT_ID") do
      ...
      config :therobotknows, :facebook_enabled, true
  ...
  oauth_providers =
    if gh_id = System.get_env("GITHUB_CLIENT_ID") do
      ...
      config :therobotknows, :github_enabled, true
  ...
  #     config :therobotknows, :linkedin_enabled, true
