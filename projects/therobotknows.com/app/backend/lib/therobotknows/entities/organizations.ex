defmodule Therobotknows.Organizations do
  @moduledoc """
  Context for Therobotknows.Organizations
  """
  alias Therobotknows.Organizations.Organization, as: Entity
  alias Therobotknows.Schema.Organizations.Organization, as: Schema
  alias Therobotknows.Schema.Organizations.InviteToken, as: InviteTokenSchema
  alias Therobotknows.Schema.Authz.ScopedMembership, as: ScopedMembershipSchema
  use Noizu.Repo
  ...
  def create_organization_with_owner(attrs, user_id) do
    Therobotknows.Repo.transaction(fn ->
      with {:ok, org} <- %Schema{} |> Schema.changeset(attrs) |> Therobotknows.Repo.insert(),
           {:ok, _membership} <- Therobotknows.Authz.ScopedMemberships.add_member("organization", org.id, user_id, "owner") do
        org
      else
  ...
  def list_user_organizations(user_id) do
    from(sm in ScopedMembershipSchema,
      ...
      join: g in Therobotknows.Schema.Authz.Group, on: g.id == sm.group_id,
  ...
  def authorize(user_id, organization_id, required_role) do
    Therobotknows.Authz.authorize(user_id, "organization", organization_id, required_role)
  end

  def list_members(organization_id) do
    Therobotknows.Authz.ScopedMemberships.list_for_resource("organization", organization_id)
  end
  ...
  def increment_invite_uses(invite_token) do
    ...
    |> Therobotknows.Repo.update_all(inc: [uses: 1])
  end
end
