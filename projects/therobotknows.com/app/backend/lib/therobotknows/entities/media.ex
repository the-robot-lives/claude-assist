defmodule Therobotknows.Media do
  alias Therobotknows.Media.Asset, as: Entity
  alias Therobotknows.Schema.Media.Asset, as: Schema
  use Noizu.Repo
  ...
  def get_cached_variant(media_id, canonical_params) do
    import Ecto.Query
    Therobotknows.Repo.one(
      from v in Therobotknows.Schema.Media.Variant,
        where: v.media_id == ^media_id and v.params == ^canonical_params
    )
  end

  def cache_variant(attrs) do
    %Therobotknows.Schema.Media.Variant{}
    |> Therobotknows.Schema.Media.Variant.changeset(attrs)
    |> Therobotknows.Repo.insert(on_conflict: :nothing)
  end

  def fetch_from_s3(key) do
    config = Application.get_env(:therobotknows, Therobotknows.Storage, [])
  ...
  def get_or_create_variant(media, transform_params) do
    canonical = Therobotknows.Media.Transform.canonical_params(transform_params)
      ...
      nil ->
        with {:ok, original_binary} <- fetch_from_s3(media.file),
             {:ok, transformed, content_type} <-
               Therobotknows.Media.Transform.transform(original_binary, transform_params) do
          variant_key = Therobotknows.Media.Transform.variant_s3_key(media.file, transform_params)
