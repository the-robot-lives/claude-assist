defmodule Therobotknows.Authz.Policies do
  alias Therobotknows.Authz.Policies.Policy, as: Entity
  alias Therobotknows.Schema.Authz.Policy, as: Schema
  alias Therobotknows.Schema.Authz.UserPolicy, as: UserPolicySchema
  ...
  def list_active(opts \\ []) do
    ...
    Therobotknows.Repo.all(query)
  end

  def get_by_name(name) do
    Therobotknows.Repo.one(from p in Schema, where: p.name == ^name and p.is_active == true)
  end
    ...
    case Therobotknows.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          policy |> Schema.changeset(attrs) |> Therobotknows.Repo.update()
        end
    end
  end
    ...
    case Therobotknows.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          Therobotknows.Repo.delete(policy)
        end
    end
  end
  ...
  def detach_from_user(user_id, policy_id) do
    case Therobotknows.Repo.one(
      from up in UserPolicySchema,
        where: up.user_id == ^user_id and up.policy_id == ^policy_id
    ) do
      nil -> {:error, :not_found}
      user_policy -> Therobotknows.Repo.delete(user_policy)
    end
  end
end
