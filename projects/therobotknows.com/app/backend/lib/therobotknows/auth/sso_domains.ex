defmodule Therobotknows.Auth.SSODomains do
  @moduledoc """
  ...
  A domain absent from the map is not permitted to sign in at all — the map is
  fail-closed, so `SSO_DOMAINS` must be set for any SSO login to succeed.
  """

  # Therobotknows' users.status enum has no :pending member; :waitlist is the
  # equivalent "provisioned but awaiting approval" state.
  @pending_status :waitlist

  def pending_status, do: @pending_status

  def policies do
    explicit = Application.get_env(:therobotknows, :sso_domain_policies, %{})
    legacy = Application.get_env(:therobotknows, :sso_domains, %{})
  ...
  def registration_status(email_or_domain, provider) do
    if auto_approve?(email_or_domain, provider), do: :active, else: @pending_status
  end
