defmodule TherobotknowsWeb.Router do
  use TherobotknowsWeb, :router
  ...
  pipeline :authenticated do
    plug TherobotknowsWeb.AuthPipeline
  end

  pipeline :sso_session do
    plug Plug.Session,
      store: :cookie,
      key: "_therobotknows_sso",
  ...
  pipeline :rate_limited_auth do
    plug TherobotknowsWeb.Plugs.RateLimit, action: :auth
  end

  pipeline :rate_limited_sensitive do
    plug TherobotknowsWeb.Plugs.RateLimit, action: :auth_sensitive
  end

  pipeline :org_viewer do
    plug TherobotknowsWeb.Plugs.RequireRole, role: "viewer"
  end

  pipeline :org_editor do
    plug TherobotknowsWeb.Plugs.RequireRole, role: "editor"
  end

  pipeline :org_admin do
    plug TherobotknowsWeb.Plugs.RequireRole, role: "admin"
  end

  pipeline :org_owner do
    plug TherobotknowsWeb.Plugs.RequireRole, role: "owner"
  end

  pipeline :admin do
    plug TherobotknowsWeb.Plugs.RequireAdmin
  end

  scope "/", TherobotknowsWeb do
    pipe_through :api
  ...
  scope "/api/v1/admin", TherobotknowsWeb do
  ...
  scope "/media", TherobotknowsWeb do
    pipe_through [:api]
  ...
  if Application.compile_env(:therobotknows, :saml_enabled) do
    scope "/sso/saml" do
  ...
  # OIDC redirect flow
  scope "/auth/oidc", TherobotknowsWeb do
    pipe_through [:sso_session]
  ...
  scope "/auth", TherobotknowsWeb do
    pipe_through [:sso_session]
  ...
  if Application.compile_env(:therobotknows, :dev_routes) do
    import Phoenix.LiveDashboard.Router

    scope "/dev" do
      pipe_through [:fetch_session, :protect_from_forgery]
      live_dashboard "/dashboard", metrics: TherobotknowsWeb.Telemetry
    end
  end
end
