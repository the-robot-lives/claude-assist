defmodule TherobotknowsWeb.SSOController do
  ...
  defp handle_sso_callback(conn, provider_type, attrs) do
    ...
    case Therobotknows.Auth.SSO.authenticate_sso(provider_type, attrs) do
      {:ok, session} ->
        {:ok, code} = Therobotknows.Auth.SSOCode.create(session.id)
  ...
  defp maybe_add(list, flag, name) do
    if Application.get_env(:therobotknows, flag), do: [name | list], else: list
  end
end
