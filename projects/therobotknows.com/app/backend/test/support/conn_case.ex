defmodule TherobotknowsWeb.ConnCase do
  ...
  using do
    quote do
      ...
      import TherobotknowsWeb.ConnCase

      @endpoint TherobotknowsWeb.Endpoint
    end
  end
  ...
  def setup_user_and_token(_context \\ %{}) do
    ...
    user = %Therobotknows.Schema.Users.User{
    ...
    {:ok, user} = Therobotknows.Repo.insert(user)

    credential = %Therobotknows.Schema.Users.Credentials.UserCredential{
      id: Ecto.UUID.generate(),
      user_id: user.id,
    ...
    {:ok, _credential} = Therobotknows.Repo.insert(credential)

    session = %Therobotknows.Schema.Users.Sessions.UserSession{
      id: Ecto.UUID.generate(),
      user_id: user.id,
      status: :active,
      details: %{}
    ...
    {:ok, session} = Therobotknows.Repo.insert(session)
    ...
    {:ok, access_token, _claims} =
      Therobotknows.Guardian.encode_and_sign(session_entity, %{}, token_type: "access", ttl: {1, :hour})

    {:ok, refresh_token, %{"jti" => jti}} =
      Therobotknows.Guardian.encode_and_sign(session_entity, %{}, token_type: "refresh", ttl: {7, :day})

    Therobotknows.Auth.TokenStore.store_refresh_jti(jti)
