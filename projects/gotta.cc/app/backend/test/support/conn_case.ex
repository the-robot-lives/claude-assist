defmodule GottaCcWeb.ConnCase do
  ...
  using do
    quote do
      ...
      import GottaCcWeb.ConnCase

      @endpoint GottaCcWeb.Endpoint
    end
  end
  ...
  def setup_user_and_token(_context \\ %{}) do
    ...
    GottaCc.Repo.insert!(
      %GottaCc.Schema.Auth.Providers.Provider{
    ...
    user = %GottaCc.Schema.Users.User{
    ...
    {:ok, user} = GottaCc.Repo.insert(user)

    credential = %GottaCc.Schema.Users.Credentials.UserCredential{
      id: Ecto.UUID.generate(),
      user_id: user.id,
    ...
    {:ok, _credential} = GottaCc.Repo.insert(credential)

    session = %GottaCc.Schema.Users.Sessions.UserSession{
      id: Ecto.UUID.generate(),
      user_id: user.id,
      status: :active,
      details: %{}
    ...
    {:ok, session} = GottaCc.Repo.insert(session)

    # Build entity-layer session struct for Guardian signing
    session_entity = %GottaCc.Users.Sessions.UserSession{
      id: session.id,
      user: {:ref, GottaCc.Users.User, user.id},
      status: :active,
      details: %{}
    }

    {:ok, access_token, _claims} =
      GottaCc.Guardian.encode_and_sign(session_entity, %{}, token_type: "access", ttl: {1, :hour})

    {:ok, refresh_token, %{"jti" => jti}} =
      GottaCc.Guardian.encode_and_sign(session_entity, %{}, token_type: "refresh", ttl: {7, :day})

    GottaCc.Auth.TokenStore.store_refresh_jti(jti)
