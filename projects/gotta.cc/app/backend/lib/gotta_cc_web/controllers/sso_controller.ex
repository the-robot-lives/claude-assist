defmodule GottaCcWeb.SSOController do
  ...
  def exchange(conn, %{"code" => code}) do
    with {:ok, session_id} <- GottaCc.Auth.SSOCode.exchange(code),
         {:ok, session} <- GottaCc.Users.Sessions.get(session_id, Noizu.Context.system()),
  ...
  defp handle_sso_callback(conn, provider_type, attrs) do
    ...
    case GottaCc.Auth.SSO.authenticate_sso(provider_type, attrs) do
      {:ok, session} ->
        {:ok, code} = GottaCc.Auth.SSOCode.create(session.id)
  ...
  defp maybe_add(list, flag, name) do
    if Application.get_env(:gotta_cc, flag), do: [name | list], else: list
  end
