defmodule GottaCc.Auth.SSO do
  alias GottaCc.Schema.Users.User, as: UserSchema
  alias GottaCc.Schema.Users.Credentials.UserCredential, as: CredentialSchema
  alias GottaCc.Schema.Versioned.Names.Name
  import Ecto.Query, only: [from: 2]

  @provider_map %{
    oidc: &GottaCc.Auth.Providers.oidc/0,
    saml: &GottaCc.Auth.Providers.saml/0,
    google: &GottaCc.Auth.Providers.google/0,
    facebook: &GottaCc.Auth.Providers.facebook/0,
    github: &GottaCc.Auth.Providers.github/0,
    linkedin: &GottaCc.Auth.Providers.linkedin/0
  }
  ...
  def authenticate_sso(provider_type, %{email: email} = attrs) do
    ...
    {:ok, provider_id} = GottaCc.Auth.Providers.Provider.id(provider_ref)
    ...
    case find_user_by_email(email) do
      ...
      :not_found ->
        if Application.get_env(:gotta_cc, :sso_require_invite, false) do
  ...
  defp ensure_sso_credential(user, provider_ref, provider_id, provider_type, attrs, context) do
    ...
    case GottaCc.Repo.one(q) do
      nil ->
        %GottaCc.Users.Credentials.UserCredential{
          user: GottaCc.Users.User.ref(user.id),
  ...
  defp create_sso_session(user, provider_type, context) do
    ...
    %GottaCc.Users.Sessions.UserSession{
      user: user_ref,
      status: :active,
  ...
  defp auto_provision_user(email, attrs, provider_ref, provider_id, provider_type, context) do
    ...
    {:ok, name} =
      GottaCc.EntityRepo.create(
        %GottaCc.Versioned.Names.Name{first: first, last: last, time_stamp: Noizu.Entity.TimeStamp.now()},
        context
      )
    ...
    {:ok, user} = GottaCc.Repo.insert(user_schema, on_conflict: :nothing, conflict_target: :email)
