defmodule GottaCc.Organizations do
  @moduledoc """
  Context for GottaCc.Organizations
  """
  alias GottaCc.Organizations.Organization, as: Entity
  alias GottaCc.Schema.Organizations.Organization, as: Schema
  alias GottaCc.Schema.Organizations.InviteToken, as: InviteTokenSchema
  alias GottaCc.Schema.Authz.ScopedMembership, as: ScopedMembershipSchema
  use Noizu.Repo
  ...
  def create_organization_with_owner(attrs, user_id) do
    GottaCc.Repo.transaction(fn ->
      with {:ok, org} <- %Schema{} |> Schema.changeset(attrs) |> GottaCc.Repo.insert(),
           {:ok, _membership} <- GottaCc.Authz.ScopedMemberships.add_member("organization", org.id, user_id, "owner") do
        org
      else
        {:error, reason} -> GottaCc.Repo.rollback(reason)
      end
    end)
  end
  ...
  def list_user_organizations(user_id) do
    from(sm in ScopedMembershipSchema,
      ...
      join: g in GottaCc.Schema.Authz.Group, on: g.id == sm.group_id,
  ...
  def authorize(user_id, organization_id, required_role) do
    GottaCc.Authz.authorize(user_id, "organization", organization_id, required_role)
  end

  def list_members(organization_id) do
    GottaCc.Authz.ScopedMemberships.list_for_resource("organization", organization_id)
  end
  ...
  def increment_invite_uses(invite_token) do
    ...
    |> GottaCc.Repo.update_all(inc: [uses: 1])
  end
end
