defmodule GottaCc.Media do
  alias GottaCc.Media.Asset, as: Entity
  alias GottaCc.Schema.Media.Asset, as: Schema
  use Noizu.Repo
  ...
  def get_cached_variant(media_id, canonical_params) do
    import Ecto.Query
    GottaCc.Repo.one(
      from v in GottaCc.Schema.Media.Variant,
        where: v.media_id == ^media_id and v.params == ^canonical_params
    )
  end

  def cache_variant(attrs) do
    %GottaCc.Schema.Media.Variant{}
    |> GottaCc.Schema.Media.Variant.changeset(attrs)
    |> GottaCc.Repo.insert(on_conflict: :nothing)
  end

  def fetch_from_s3(key) do
    config = Application.get_env(:gotta_cc, GottaCc.Storage, [])
  ...
  def get_or_create_variant(media, transform_params) do
    canonical = GottaCc.Media.Transform.canonical_params(transform_params)
      ...
      nil ->
        with {:ok, original_binary} <- fetch_from_s3(media.file),
             {:ok, transformed, content_type} <-
               GottaCc.Media.Transform.transform(original_binary, transform_params) do
          variant_key = GottaCc.Media.Transform.variant_s3_key(media.file, transform_params)
