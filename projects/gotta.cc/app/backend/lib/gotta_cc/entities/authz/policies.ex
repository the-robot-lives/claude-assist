defmodule GottaCc.Authz.Policies do
  alias GottaCc.Authz.Policies.Policy, as: Entity
  alias GottaCc.Schema.Authz.Policy, as: Schema
  alias GottaCc.Schema.Authz.UserPolicy, as: UserPolicySchema
  ...
  def get_by_name(name) do
    GottaCc.Repo.one(from p in Schema, where: p.name == ^name and p.is_active == true)
  end
    ...
    case GottaCc.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          policy |> Schema.changeset(attrs) |> GottaCc.Repo.update()
        end
    end
  end
  ...
  def detach_from_user(user_id, policy_id) do
    case GottaCc.Repo.one(
      from up in UserPolicySchema,
        where: up.user_id == ^user_id and up.policy_id == ^policy_id
    ) do
      nil -> {:error, :not_found}
      user_policy -> GottaCc.Repo.delete(user_policy)
    end
  end
end
