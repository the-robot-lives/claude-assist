defmodule GottaCc.Users do
  @moduledoc """
  Context for GottaCc.Users
  """
  alias GottaCc.Users.User, as: Entity
  alias GottaCc.Schema.Users.User, as: Schema
  use Noizu.Repo
  def_repo(entity: GottaCc.Users.User)
  ...
  def register(details, {:login, {email, password}} = auth, context, options) do
    ...
    email = GottaCc.Users.Credentials.standardize_email(email)
    ...
    with :valid <- valid_user_name?(details.user_name),
      ...
      with {:ok, name_entity} <-
             %GottaCc.Versioned.Names.Name{
               first: name.first,
               middle: name.middle,
               last: name.last,
               time_stamp: Noizu.Entity.TimeStamp.now()
             }
             |> GottaCc.EntityRepo.create(context),
           {:ok, name_ref} <- Noizu.EntityReference.Protocol.ref(name_entity),
           {:ok, description} <-
             %GottaCc.Versioned.Descriptions.Description{
               title: "Description",
           ...
           {:ok, credential} <-
             GottaCc.Users.Credentials.register(user_ref, auth, context, options) do
        {:ok, {user, credential}}
      end
    end
  end
  ...
  def authenticate(auth = {:login, {_email, _password}}, context, options \\ nil) do
    with {:ok, session} <- GottaCc.Users.Credentials.authenticate(auth, context, options) do
      {:ok, session}
    end
  end

  def by_handle(handle, context, options \\ []) do
    with record = %Schema{} <- GottaCc.Repo.get_by(Schema, %{handle: handle}) do
      settings = Noizu.Entity.Meta.persistence(Entity) |> hd
      {:ok, entity} = Entity.from_record(record, settings, context, options)
  ...
  def user_name_available?(user_name, _context, _options \\ nil) do
    with %{} <- GottaCc.Repo.get_by(Schema, %{user_name: user_name}) do
        ...
        fn
          ...
          {"name", value} ->
            case user.name do
              %Ecto.Changeset{} ->
                value =
                  GottaCc.Versioned.Names.change_versioned_name(user.name.data, value)

                {:name, value}

              _ ->
                value =
                  GottaCc.Versioned.Names.change_versioned_name(
                    user.name || %GottaCc.Versioned.Names.Name{},
                    value
                  )
      ...
      |> put_in(
        ...
        {:embed,
         %{
           ...
           on_cast: :update,
           on_replace: :update,
           related: GottaCc.Versioned.Names.Name
         }}
      )
  ...
  def valid_login?(email, password) do
    GottaCc.Users.Credentials.valid_login?(email, password)
  end

  def login_available?(email, context, options \\ nil) do
    GottaCc.Users.Credentials.login_available?(email, context, options)
  end
  ...
  defp unique_handle(handle, context, options) do
    case GottaCc.Users.by_handle(handle, context, options) do
      nil ->
        {:ok, handle}
      ...
      {:ok, _} ->
        Enum.reduce_while(0..999, handle, fn suffix, _acc ->
          ...
          case GottaCc.Users.by_handle(with_suffix, context, options) do
            nil ->
              {:halt, {:ok, with_suffix}}
