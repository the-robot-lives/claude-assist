defmodule GottaCc.Users.Credentials do
  @moduledoc """
  Context for GottaCc.Users.Credentials
  """
  alias GottaCc.Users.Credentials.UserCredential, as: Entity
  alias GottaCc.Schema.Users.Credentials.UserCredential, as: Schema
  use Noizu.Repo
  import Ecto.Query, only: [from: 2]

  def_repo(entity: GottaCc.Users.Credentials.UserCredential)
  ...
  def register(user, {:login, {email, password}}, context, _options) do
    {:ok, auth_provider} = GottaCc.Auth.Providers.login()

    {:ok, description} =
      %GottaCc.Versioned.Strings.String{
        content: "",
  ...
  def authenticate({:login, {email, password}}, context, options) do
    {:ok, auth_provider} = GottaCc.Auth.Providers.login()
    {:ok, auth_provider_id} = GottaCc.Auth.Providers.Provider.id(auth_provider)
        ...
        [credential] ->
          if Bcrypt.verify_pass(password, credential.settings["password"]) do
            with {:ok, credential_entity} <-
                   GottaCc.Users.Credentials.UserCredential.entity(credential.id, context),
              ...
              |> GottaCc.EntityRepo.create(context, options)
            end
          else
