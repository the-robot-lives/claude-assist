defmodule Therobotmakes.Auth.SSO do
  alias Therobotmakes.Schema.Users.User, as: UserSchema
  alias Therobotmakes.Schema.Users.Credentials.UserCredential, as: CredentialSchema
  alias Therobotmakes.Schema.Versioned.Names.Name
  import Ecto.Query, only: [from: 2]

  @provider_map %{
    oidc: &Therobotmakes.Auth.Providers.oidc/0,
    saml: &Therobotmakes.Auth.Providers.saml/0,
    google: &Therobotmakes.Auth.Providers.google/0,
    facebook: &Therobotmakes.Auth.Providers.facebook/0,
    github: &Therobotmakes.Auth.Providers.github/0,
    linkedin: &Therobotmakes.Auth.Providers.linkedin/0
  }
  ...
  def authenticate_sso(provider_type, %{email: email} = attrs) do
    ...
    {:ok, provider_id} = Therobotmakes.Auth.Providers.Provider.id(provider_ref)
    ...
    case find_user_by_email(email) do
      ...
      :not_found ->
        if Application.get_env(:therobotmakes, :sso_require_invite, false) do
  ...
  defp find_user_by_email(email) do
    ...
    case Therobotmakes.Repo.one(q) do
      nil -> :not_found
      user -> {:ok, user}
    end
  end
  ...
  defp ensure_sso_credential(user, provider_ref, provider_id, provider_type, attrs, context) do
    ...
    case Therobotmakes.Repo.one(q) do
      nil ->
        %Therobotmakes.Users.Credentials.UserCredential{
          user: Therobotmakes.Users.User.ref(user.id),
  ...
  defp create_sso_session(user, provider_type, context) do
    ...
    %Therobotmakes.Users.Sessions.UserSession{
      user: user_ref,
      status: :active,
  ...
  defp auto_provision_user(email, attrs, provider_ref, provider_id, provider_type, context) do
    ...
    {:ok, name} =
      Therobotmakes.EntityRepo.create(
        %Therobotmakes.Versioned.Names.Name{first: first, last: last, time_stamp: Noizu.Entity.TimeStamp.now()},
        context
      )
    ...
    {:ok, user} = Therobotmakes.Repo.insert(user_schema, on_conflict: :nothing, conflict_target: :email)
