defmodule Therobotmakes.Media do
  alias Therobotmakes.Media.Asset, as: Entity
  alias Therobotmakes.Schema.Media.Asset, as: Schema
  use Noizu.Repo
  ...
  def get_cached_variant(media_id, canonical_params) do
    import Ecto.Query
    Therobotmakes.Repo.one(
      from v in Therobotmakes.Schema.Media.Variant,
        where: v.media_id == ^media_id and v.params == ^canonical_params
    )
  end

  def cache_variant(attrs) do
    %Therobotmakes.Schema.Media.Variant{}
    |> Therobotmakes.Schema.Media.Variant.changeset(attrs)
    |> Therobotmakes.Repo.insert(on_conflict: :nothing)
  end

  def fetch_from_s3(key) do
    config = Application.get_env(:therobotmakes, Therobotmakes.Storage, [])
  ...
  def get_or_create_variant(media, transform_params) do
    canonical = Therobotmakes.Media.Transform.canonical_params(transform_params)
      ...
      nil ->
        with {:ok, original_binary} <- fetch_from_s3(media.file),
             {:ok, transformed, content_type} <-
               Therobotmakes.Media.Transform.transform(original_binary, transform_params) do
          variant_key = Therobotmakes.Media.Transform.variant_s3_key(media.file, transform_params)
