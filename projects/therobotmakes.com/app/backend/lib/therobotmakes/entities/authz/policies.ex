defmodule Therobotmakes.Authz.Policies do
  alias Therobotmakes.Authz.Policies.Policy, as: Entity
  alias Therobotmakes.Schema.Authz.Policy, as: Schema
  alias Therobotmakes.Schema.Authz.UserPolicy, as: UserPolicySchema
  ...
  def list_active(opts \\ []) do
    ...
    Therobotmakes.Repo.all(query)
  end

  def get_by_name(name) do
    Therobotmakes.Repo.one(from p in Schema, where: p.name == ^name and p.is_active == true)
  end
    ...
    case Therobotmakes.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          policy |> Schema.changeset(attrs) |> Therobotmakes.Repo.update()
        end
    end
  end
    ...
    case Therobotmakes.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          Therobotmakes.Repo.delete(policy)
        end
    end
  end
  ...
  def detach_from_user(user_id, policy_id) do
    case Therobotmakes.Repo.one(
      from up in UserPolicySchema,
        where: up.user_id == ^user_id and up.policy_id == ^policy_id
    ) do
      nil -> {:error, :not_found}
      user_policy -> Therobotmakes.Repo.delete(user_policy)
    end
  end
end
