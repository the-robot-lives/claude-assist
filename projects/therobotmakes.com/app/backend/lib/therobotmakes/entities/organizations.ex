defmodule Therobotmakes.Organizations do
  @moduledoc """
  Context for Therobotmakes.Organizations
  """
  alias Therobotmakes.Organizations.Organization, as: Entity
  alias Therobotmakes.Schema.Organizations.Organization, as: Schema
  alias Therobotmakes.Schema.Organizations.InviteToken, as: InviteTokenSchema
  alias Therobotmakes.Schema.Authz.ScopedMembership, as: ScopedMembershipSchema
  use Noizu.Repo
  ...
  def create_organization_with_owner(attrs, user_id) do
    Therobotmakes.Repo.transaction(fn ->
      with {:ok, org} <- %Schema{} |> Schema.changeset(attrs) |> Therobotmakes.Repo.insert(),
           {:ok, _membership} <- Therobotmakes.Authz.ScopedMemberships.add_member("organization", org.id, user_id, "owner") do
        org
      else
  ...
  def list_user_organizations(user_id) do
    from(sm in ScopedMembershipSchema,
      ...
      join: g in Therobotmakes.Schema.Authz.Group, on: g.id == sm.group_id,
    ...
    |> Therobotmakes.Repo.all()
  end

  def authorize(user_id, organization_id, required_role) do
    Therobotmakes.Authz.authorize(user_id, "organization", organization_id, required_role)
  end

  def list_members(organization_id) do
    Therobotmakes.Authz.ScopedMemberships.list_for_resource("organization", organization_id)
  end
  ...
  def increment_invite_uses(invite_token) do
    ...
    |> Therobotmakes.Repo.update_all(inc: [uses: 1])
  end
end
