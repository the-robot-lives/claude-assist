defmodule Therobotmakes.Users.Credentials do
  @moduledoc """
  Context for Therobotmakes.Users.Credentials
  """
  alias Therobotmakes.Users.Credentials.UserCredential, as: Entity
  alias Therobotmakes.Schema.Users.Credentials.UserCredential, as: Schema
  use Noizu.Repo
  import Ecto.Query, only: [from: 2]

  def_repo(entity: Therobotmakes.Users.Credentials.UserCredential)
  ...
  def register(user, {:login, {email, password}}, context, _options) do
    {:ok, auth_provider} = Therobotmakes.Auth.Providers.login()

    {:ok, description} =
      %Therobotmakes.Versioned.Strings.String{
        content: "",
  ...
  def authenticate({:login, {email, password}}, context, options) do
    {:ok, auth_provider} = Therobotmakes.Auth.Providers.login()
    {:ok, auth_provider_id} = Therobotmakes.Auth.Providers.Provider.id(auth_provider)
    ...
    with :valid <- valid_login?(email, password) do
      ...
      case Therobotmakes.Repo.all(q) do
        [] ->
        ...
        [credential] ->
          if Bcrypt.verify_pass(password, credential.settings["password"]) do
            with {:ok, credential_entity} <-
                   Therobotmakes.Users.Credentials.UserCredential.entity(credential.id, context),
              ...
              |> Therobotmakes.EntityRepo.create(context, options)
            end
          else
