defmodule TherobotmakesWeb.SSOController do
  ...
  def exchange(conn, %{"code" => code}) do
    with {:ok, session_id} <- Therobotmakes.Auth.SSOCode.exchange(code),
         {:ok, session} <- Therobotmakes.Users.Sessions.get(session_id, Noizu.Context.system()),
  ...
  defp handle_sso_callback(conn, provider_type, attrs) do
    ...
    case Therobotmakes.Auth.SSO.authenticate_sso(provider_type, attrs) do
      {:ok, session} ->
        {:ok, code} = Therobotmakes.Auth.SSOCode.create(session.id)
  ...
  defp maybe_add(list, flag, name) do
    if Application.get_env(:therobotmakes, flag), do: [name | list], else: list
  end
end
