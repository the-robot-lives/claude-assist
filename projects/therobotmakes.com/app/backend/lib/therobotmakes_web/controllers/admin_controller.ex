defmodule TherobotmakesWeb.AdminController do
  use TherobotmakesWeb, :controller

  alias Therobotmakes.Schema.Users.User, as: UserSchema
  alias Therobotmakes.Schema.Organizations.Organization, as: OrgSchema
  import Ecto.Query
  ...
  def list_organizations(conn, params) do
    ...
    total = Therobotmakes.Repo.aggregate(OrgSchema, :count, :id)

    conn |> put_status(:ok) |> json(%{organizations: orgs, total: total, page: page, per_page: per_page})
  end

  def show_organization(conn, %{"id" => id}) do
    case Therobotmakes.Repo.get(OrgSchema, id) do
      nil ->
        conn |> put_status(:not_found) |> json(%{error: "Organization not found"})

      org ->
        members = Therobotmakes.Organizations.list_members(org.id)
