defmodule TherobotmakesWeb.Router do
  use TherobotmakesWeb, :router
  ...
  pipeline :authenticated do
    plug TherobotmakesWeb.AuthPipeline
  end
  ...
  pipeline :rate_limited_auth do
    plug TherobotmakesWeb.Plugs.RateLimit, action: :auth
  end

  pipeline :rate_limited_sensitive do
    plug TherobotmakesWeb.Plugs.RateLimit, action: :auth_sensitive
  end

  pipeline :org_viewer do
    plug TherobotmakesWeb.Plugs.RequireRole, role: "viewer"
  end

  pipeline :org_editor do
    plug TherobotmakesWeb.Plugs.RequireRole, role: "editor"
  end

  pipeline :org_admin do
    plug TherobotmakesWeb.Plugs.RequireRole, role: "admin"
  end

  pipeline :org_owner do
    plug TherobotmakesWeb.Plugs.RequireRole, role: "owner"
  end

  pipeline :admin do
    plug TherobotmakesWeb.Plugs.RequireAdmin
  end

  scope "/", TherobotmakesWeb do
    pipe_through :api
    get "/health", HealthController, :index
  end

  scope "/api/v1", TherobotmakesWeb do
  ...
  scope "/media", TherobotmakesWeb do
    pipe_through [:api]
  ...
  if Application.compile_env(:therobotmakes, :saml_enabled) do
    scope "/sso/saml" do
  ...
  # OIDC redirect flow
  scope "/auth/oidc", TherobotmakesWeb do
    pipe_through [:sso_session]
  ...
  scope "/auth", TherobotmakesWeb do
    pipe_through [:sso_session]
  ...
  if Application.compile_env(:therobotmakes, :dev_routes) do
    import Phoenix.LiveDashboard.Router

    scope "/dev" do
      pipe_through [:fetch_session, :protect_from_forgery]
      live_dashboard "/dashboard", metrics: TherobotmakesWeb.Telemetry
    end
  end
end
