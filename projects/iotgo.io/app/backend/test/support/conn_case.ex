defmodule IotgoWeb.ConnCase do
  ...
  using do
    quote do
      ...
      import IotgoWeb.ConnCase

      @endpoint IotgoWeb.Endpoint
    end
  end
  ...
  def setup_user_and_token(_context \\ %{}) do
    ...
    Iotgo.Repo.insert!(
      %Iotgo.Schema.Auth.Providers.Provider{
    ...
    {:ok, user} = Iotgo.Repo.insert(user)

    credential = %Iotgo.Schema.Users.Credentials.UserCredential{
      id: Ecto.UUID.generate(),
      user_id: user.id,
    ...
    {:ok, _credential} = Iotgo.Repo.insert(credential)

    session = %Iotgo.Schema.Users.Sessions.UserSession{
      id: Ecto.UUID.generate(),
      user_id: user.id,
      status: :active,
      details: %{}
    ...
    {:ok, session} = Iotgo.Repo.insert(session)

    # Build entity-layer session struct for Guardian signing
    session_entity = %Iotgo.Users.Sessions.UserSession{
      id: session.id,
      user: {:ref, Iotgo.Users.User, user.id},
      status: :active,
      details: %{}
    }

    {:ok, access_token, _claims} =
      Iotgo.Guardian.encode_and_sign(session_entity, %{}, token_type: "access", ttl: {1, :hour})

    {:ok, refresh_token, %{"jti" => jti}} =
      Iotgo.Guardian.encode_and_sign(session_entity, %{}, token_type: "refresh", ttl: {7, :day})

    Iotgo.Auth.TokenStore.store_refresh_jti(jti)
