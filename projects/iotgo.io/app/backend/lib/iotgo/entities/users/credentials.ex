defmodule Iotgo.Users.Credentials do
  @moduledoc """
  Context for Iotgo.Users.Credentials
  """
  alias Iotgo.Users.Credentials.UserCredential, as: Entity
  alias Iotgo.Schema.Users.Credentials.UserCredential, as: Schema
  use Noizu.Repo
  import Ecto.Query, only: [from: 2]

  def_repo(entity: Iotgo.Users.Credentials.UserCredential)
  ...
  def register(user, {:login, {email, password}}, context, _options) do
    {:ok, auth_provider} = Iotgo.Auth.Providers.login()

    {:ok, description} =
      %Iotgo.Versioned.Strings.String{
        content: "",
  ...
  def authenticate({:login, {email, password}}, context, options) do
    {:ok, auth_provider} = Iotgo.Auth.Providers.login()
    {:ok, auth_provider_id} = Iotgo.Auth.Providers.Provider.id(auth_provider)
        ...
        [credential] ->
          if Bcrypt.verify_pass(password, credential.settings["password"]) do
            with {:ok, credential_entity} <-
                   Iotgo.Users.Credentials.UserCredential.entity(credential.id, context),
              ...
              |> Iotgo.EntityRepo.create(context, options)
            end
          else
