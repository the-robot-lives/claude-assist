defmodule Iotgo.Authz.Policies do
  alias Iotgo.Authz.Policies.Policy, as: Entity
  alias Iotgo.Schema.Authz.Policy, as: Schema
  alias Iotgo.Schema.Authz.UserPolicy, as: UserPolicySchema
  ...
  def list_active(opts \\ []) do
    ...
    Iotgo.Repo.all(query)
  end

  def get_by_name(name) do
    Iotgo.Repo.one(from p in Schema, where: p.name == ^name and p.is_active == true)
  end
    ...
    case Iotgo.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          policy |> Schema.changeset(attrs) |> Iotgo.Repo.update()
        end
    end
  end
  ...
  def detach_from_user(user_id, policy_id) do
    case Iotgo.Repo.one(
      ...
      nil -> {:error, :not_found}
      user_policy -> Iotgo.Repo.delete(user_policy)
    end
  end
end
