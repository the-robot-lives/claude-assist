defmodule Iotgo.Media do
  alias Iotgo.Media.Asset, as: Entity
  alias Iotgo.Schema.Media.Asset, as: Schema
  use Noizu.Repo
  ...
  def get_cached_variant(media_id, canonical_params) do
    import Ecto.Query
    Iotgo.Repo.one(
      from v in Iotgo.Schema.Media.Variant,
        where: v.media_id == ^media_id and v.params == ^canonical_params
    )
  end

  def cache_variant(attrs) do
    %Iotgo.Schema.Media.Variant{}
    |> Iotgo.Schema.Media.Variant.changeset(attrs)
    |> Iotgo.Repo.insert(on_conflict: :nothing)
  end

  def fetch_from_s3(key) do
    config = Application.get_env(:iotgo, Iotgo.Storage, [])
  ...
  def get_or_create_variant(media, transform_params) do
    canonical = Iotgo.Media.Transform.canonical_params(transform_params)
      ...
      nil ->
        with {:ok, original_binary} <- fetch_from_s3(media.file),
             {:ok, transformed, content_type} <-
               Iotgo.Media.Transform.transform(original_binary, transform_params) do
          variant_key = Iotgo.Media.Transform.variant_s3_key(media.file, transform_params)
