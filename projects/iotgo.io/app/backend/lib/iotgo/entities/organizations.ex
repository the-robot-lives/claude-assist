defmodule Iotgo.Organizations do
  @moduledoc """
  Context for Iotgo.Organizations
  """
  alias Iotgo.Organizations.Organization, as: Entity
  alias Iotgo.Schema.Organizations.Organization, as: Schema
  alias Iotgo.Schema.Organizations.InviteToken, as: InviteTokenSchema
  alias Iotgo.Schema.Authz.ScopedMembership, as: ScopedMembershipSchema
  use Noizu.Repo
  ...
  def create_organization_with_owner(attrs, user_id) do
    Iotgo.Repo.transaction(fn ->
      with {:ok, org} <- %Schema{} |> Schema.changeset(attrs) |> Iotgo.Repo.insert(),
           {:ok, _membership} <- Iotgo.Authz.ScopedMemberships.add_member("organization", org.id, user_id, "owner") do
        org
      else
  ...
  def list_user_organizations(user_id) do
    from(sm in ScopedMembershipSchema,
      ...
      join: g in Iotgo.Schema.Authz.Group, on: g.id == sm.group_id,
  ...
  def authorize(user_id, organization_id, required_role) do
    Iotgo.Authz.authorize(user_id, "organization", organization_id, required_role)
  end

  def list_members(organization_id) do
    Iotgo.Authz.ScopedMemberships.list_for_resource("organization", organization_id)
  end
  ...
  def increment_invite_uses(invite_token) do
    ...
    |> Iotgo.Repo.update_all(inc: [uses: 1])
  end
end
