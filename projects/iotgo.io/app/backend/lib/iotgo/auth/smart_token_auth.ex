defmodule Iotgo.Auth.SmartTokenAuth do
    ...
    case find_active_user_by_email(email) do
      {:ok, user} ->
        ...
        case find_and_authorize_otp(user_ref, otp_code, {:auth, :password_reset}, conn, context) do
          {:ok, _token} ->
            Iotgo.Users.Credentials.update_password(user, new_password, context)
    ...
    case SmartToken.authorize!(token_key, %{}, context) do
      {:ok, token} ->
        with {:ok, user} <- Noizu.EntityReference.Protocol.entity(token.resource, context) do
          ...
          |> Iotgo.Repo.update_all(set: [verified: true])
  ...
  defp find_active_user_by_email(email) do
    ...
    case Iotgo.Repo.one(q) do
      ...
      record ->
        settings = Noizu.Entity.Meta.persistence(Iotgo.Users.User) |> hd
        Iotgo.Users.User.from_record(record, settings, Noizu.Context.system(), [])
    end
  end
end
