defmodule TherobotsdayjobWeb.MediaServeController do
  use TherobotsdayjobWeb, :controller

  alias Therobotsdayjob.Media
  alias Therobotsdayjob.Media.Transform
  ...
  defp serve_original(conn, media) do
    if media.visibility == "public" do
      url = Therobotsdayjob.Storage.presigned_download_url(media.file)
  ...
  defp serve_transformed(conn, media, transform_params) do
    case Media.get_or_create_variant(media, transform_params) do
      {:ok, variant_key, content_type} ->
        if media.visibility == "public" do
          url = Therobotsdayjob.Storage.presigned_download_url(variant_key)
  ...
  defp get_user_id(conn) do
    try do
      case Plug.Conn.get_req_header(conn, "authorization") do
        ["Bearer " <> token] ->
          case Therobotsdayjob.Guardian.decode_and_verify(token) do
            {:ok, claims} ->
              case Therobotsdayjob.Guardian.resource_from_claims(claims) do
                {:ok, %Therobotsdayjob.Users.Sessions.UserSession{user: {:ref, _, id}}} -> id
                {:ok, %Therobotsdayjob.Users.Sessions.UserSession{user: %{id: id}}} -> id
                _ -> nil
              end
