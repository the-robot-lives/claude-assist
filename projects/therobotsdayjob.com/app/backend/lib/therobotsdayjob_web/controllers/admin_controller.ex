defmodule TherobotsdayjobWeb.AdminController do
  use TherobotsdayjobWeb, :controller

  alias Therobotsdayjob.Schema.Users.User, as: UserSchema
  alias Therobotsdayjob.Schema.Organizations.Organization, as: OrgSchema
  import Ecto.Query
  ...
  def list_users(conn, params) do
    ...
    total = Therobotsdayjob.Repo.aggregate(UserSchema, :count, :id)

    conn |> put_status(:ok) |> json(%{users: users, total: total, page: page, per_page: per_page})
  end

  def show_user(conn, %{"id" => id}) do
    case Therobotsdayjob.Repo.get(UserSchema, id) do
      nil ->
  ...
  def list_organizations(conn, params) do
    ...
    total = Therobotsdayjob.Repo.aggregate(OrgSchema, :count, :id)

    conn |> put_status(:ok) |> json(%{organizations: orgs, total: total, page: page, per_page: per_page})
  end

  def show_organization(conn, %{"id" => id}) do
    case Therobotsdayjob.Repo.get(OrgSchema, id) do
      nil ->
        conn |> put_status(:not_found) |> json(%{error: "Organization not found"})

      org ->
        members = Therobotsdayjob.Organizations.list_members(org.id)
