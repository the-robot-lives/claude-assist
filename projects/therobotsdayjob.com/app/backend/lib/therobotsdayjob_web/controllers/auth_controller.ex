defmodule TherobotsdayjobWeb.AuthController do
  ...
  def register(conn, %{"invite_token" => raw_token, "user" => user_params}) do
    with {:ok, invite} <- Organizations.find_active_invite_by_raw_token(raw_token),
         {:ok, {user, _credential}} <-
           Therobotsdayjob.Users.register(
             %{
      ...
      Therobotsdayjob.Auth.TokenStore.store_refresh_jti(refresh_jti)
      Therobotsdayjob.Events.dispatch(:user_registered, %{user_id: user.id, email: user.email})
  ...
  def login(conn, %{"email" => email, "password" => password}) do
    case Therobotsdayjob.Users.Credentials.authenticate({:login, {email, password}}, Noizu.Context.system(), []) do
      {:ok, session} ->
        {:ok, access_token, _} =
          Guardian.encode_and_sign(session, %{}, token_type: "access", ttl: {1, :hour})
  ...
  def request_magic_link(conn, %{"email" => email}) do
    ...
    case Therobotsdayjob.Auth.SmartTokenAuth.request_magic_link(email) do
      {:ok, %{encoded_key: encoded_key, user: _user}} ->
        magic_link = "#{frontend_url}/auth/verify?token=#{encoded_key}"
        Therobotsdayjob.Auth.SmartTokenEmail.send_magic_link(email, magic_link)

        response = %{message: "If an account exists with that email, a magic link has been sent."}

        response =
          if Application.get_env(:therobotsdayjob, :dev_routes) do
  ...
  def verify_magic_link(conn, %{"token" => token_key}) do
    case Therobotsdayjob.Auth.SmartTokenAuth.verify_magic_link(token_key, conn) do
      {:ok, session} ->
        {:ok, access_token, _} =
          Guardian.encode_and_sign(session, %{}, token_type: "access", ttl: {1, :hour})
  ...
  def request_otp_login(conn, %{"email" => email}) do
    case Therobotsdayjob.Auth.SmartTokenAuth.request_otp_login(email) do
      {:ok, %{otp_code: otp_code, user: _user}} ->
        Therobotsdayjob.Auth.SmartTokenEmail.send_otp_login(email, otp_code)
  ...
  def verify_otp_login(conn, %{"email" => email, "code" => code}) do
    case Therobotsdayjob.Auth.SmartTokenAuth.verify_otp_login(email, code, conn) do
      {:ok, session} ->
        {:ok, access_token, _} =
          Guardian.encode_and_sign(session, %{}, token_type: "access", ttl: {1, :hour})
  ...
  def request_password_reset(conn, %{"email" => email}) do
    case Therobotsdayjob.Auth.SmartTokenAuth.request_password_reset(email) do
      {:ok, %{otp_code: otp_code, user: _user}} ->
        Therobotsdayjob.Auth.SmartTokenEmail.send_password_reset(email, otp_code)
  ...
  def verify_password_reset(conn, %{"email" => email, "code" => code, "new_password" => new_password}) do
    case Therobotsdayjob.Auth.SmartTokenAuth.verify_password_reset(email, code, new_password, conn) do
      {:ok, _credential} ->
  ...
  def refresh(conn, %{"refresh_token" => refresh_token}) do
    alias Therobotsdayjob.Auth.TokenStore
  ...
  def send_verification(conn, _params) do
    ...
    case Therobotsdayjob.Auth.SmartTokenAuth.request_email_verification(user.email) do
      {:ok, %{encoded_key: encoded_key}} ->
        verification_link = "#{frontend_url}/auth/verify-email?token=#{encoded_key}"
        Therobotsdayjob.Auth.SmartTokenEmail.send_verification_email(user.email, verification_link)
  ...
  def verify_email(conn, %{"token" => token_key}) do
    case Therobotsdayjob.Auth.SmartTokenAuth.verify_email_token(token_key) do
      {:ok, user} ->
        Therobotsdayjob.Events.dispatch(:user_verified, %{user_id: user.id})
  ...
  defp create_session_for_user(user) do
    user_ref = Therobotsdayjob.Users.User.ref(user.id)
    session_entity = %Therobotsdayjob.Users.Sessions.UserSession{
      user: user_ref,
      status: :active,
      details: %{}
    }
    Therobotsdayjob.Users.Sessions.create(session_entity, Noizu.Context.system())
  end
