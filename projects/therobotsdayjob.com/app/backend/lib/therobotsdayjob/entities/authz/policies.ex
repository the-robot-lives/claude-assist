defmodule Therobotsdayjob.Authz.Policies do
  alias Therobotsdayjob.Authz.Policies.Policy, as: Entity
  alias Therobotsdayjob.Schema.Authz.Policy, as: Schema
  alias Therobotsdayjob.Schema.Authz.UserPolicy, as: UserPolicySchema
  ...
  def list_active(opts \\ []) do
    ...
    Therobotsdayjob.Repo.all(query)
  end

  def get_by_name(name) do
    Therobotsdayjob.Repo.one(from p in Schema, where: p.name == ^name and p.is_active == true)
  end
    ...
    case Therobotsdayjob.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          policy |> Schema.changeset(attrs) |> Therobotsdayjob.Repo.update()
        end
    end
  end
    ...
    case Therobotsdayjob.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          Therobotsdayjob.Repo.delete(policy)
        end
    end
  end
  ...
  def detach_from_user(user_id, policy_id) do
    case Therobotsdayjob.Repo.one(
      from up in UserPolicySchema,
        where: up.user_id == ^user_id and up.policy_id == ^policy_id
    ) do
      nil -> {:error, :not_found}
      user_policy -> Therobotsdayjob.Repo.delete(user_policy)
    end
  end
end
