defmodule Therobotsdayjob.Organizations do
  @moduledoc """
  Context for Therobotsdayjob.Organizations
  """
  alias Therobotsdayjob.Organizations.Organization, as: Entity
  alias Therobotsdayjob.Schema.Organizations.Organization, as: Schema
  alias Therobotsdayjob.Schema.Organizations.InviteToken, as: InviteTokenSchema
  alias Therobotsdayjob.Schema.Authz.ScopedMembership, as: ScopedMembershipSchema
  use Noizu.Repo
  ...
  def create_organization_with_owner(attrs, user_id) do
    Therobotsdayjob.Repo.transaction(fn ->
      with {:ok, org} <- %Schema{} |> Schema.changeset(attrs) |> Therobotsdayjob.Repo.insert(),
           {:ok, _membership} <- Therobotsdayjob.Authz.ScopedMemberships.add_member("organization", org.id, user_id, "owner") do
        org
      else
  ...
  def list_user_organizations(user_id) do
    from(sm in ScopedMembershipSchema,
      ...
      join: g in Therobotsdayjob.Schema.Authz.Group, on: g.id == sm.group_id,
    ...
    |> Therobotsdayjob.Repo.all()
  end

  def authorize(user_id, organization_id, required_role) do
    Therobotsdayjob.Authz.authorize(user_id, "organization", organization_id, required_role)
  end

  def list_members(organization_id) do
    Therobotsdayjob.Authz.ScopedMemberships.list_for_resource("organization", organization_id)
  end
  ...
  def increment_invite_uses(invite_token) do
    ...
    |> Therobotsdayjob.Repo.update_all(inc: [uses: 1])
  end
end
