defmodule Therobotsdayjob.Users.Credentials do
  @moduledoc """
  Context for Therobotsdayjob.Users.Credentials
  """
  alias Therobotsdayjob.Users.Credentials.UserCredential, as: Entity
  alias Therobotsdayjob.Schema.Users.Credentials.UserCredential, as: Schema
  use Noizu.Repo
  import Ecto.Query, only: [from: 2]

  def_repo(entity: Therobotsdayjob.Users.Credentials.UserCredential)
  ...
  def register(user, {:login, {email, password}}, context, _options) do
    {:ok, auth_provider} = Therobotsdayjob.Auth.Providers.login()

    {:ok, description} =
      %Therobotsdayjob.Versioned.Strings.String{
        content: "",
  ...
  def authenticate({:login, {email, password}}, context, options) do
    {:ok, auth_provider} = Therobotsdayjob.Auth.Providers.login()
    {:ok, auth_provider_id} = Therobotsdayjob.Auth.Providers.Provider.id(auth_provider)
    ...
    with :valid <- valid_login?(email, password) do
      ...
      case Therobotsdayjob.Repo.all(q) do
        [] ->
        ...
        [credential] ->
          if Bcrypt.verify_pass(password, credential.settings["password"]) do
            with {:ok, credential_entity} <-
                   Therobotsdayjob.Users.Credentials.UserCredential.entity(credential.id, context),
              ...
              |> Therobotsdayjob.EntityRepo.create(context, options)
            end
          else
