defmodule Therobotsdayjob.Users do
  @moduledoc """
  Context for Therobotsdayjob.Users
  """
  alias Therobotsdayjob.Users.User, as: Entity
  alias Therobotsdayjob.Schema.Users.User, as: Schema
  use Noizu.Repo
  def_repo(entity: Therobotsdayjob.Users.User)
  ...
  def register(details, {:login, {email, password}} = auth, context, options) do
    ...
    email = Therobotsdayjob.Users.Credentials.standardize_email(email)
    ...
    with :valid <- valid_user_name?(details.user_name),
      ...
      with {:ok, name_entity} <-
             %Therobotsdayjob.Versioned.Names.Name{
               first: name.first,
               middle: name.middle,
               last: name.last,
               time_stamp: Noizu.Entity.TimeStamp.now()
             }
             |> Therobotsdayjob.EntityRepo.create(context),
           {:ok, name_ref} <- Noizu.EntityReference.Protocol.ref(name_entity),
           {:ok, description} <-
             %Therobotsdayjob.Versioned.Descriptions.Description{
               title: "Description",
           ...
           {:ok, credential} <-
             Therobotsdayjob.Users.Credentials.register(user_ref, auth, context, options) do
        {:ok, {user, credential}}
      end
    end
  end
  ...
  def authenticate(auth = {:login, {_email, _password}}, context, options \\ nil) do
    with {:ok, session} <- Therobotsdayjob.Users.Credentials.authenticate(auth, context, options) do
      {:ok, session}
    end
  end

  def by_handle(handle, context, options \\ []) do
    with record = %Schema{} <- Therobotsdayjob.Repo.get_by(Schema, %{handle: handle}) do
      settings = Noizu.Entity.Meta.persistence(Entity) |> hd
      {:ok, entity} = Entity.from_record(record, settings, context, options)
  ...
  def user_name_available?(user_name, _context, _options \\ nil) do
    with %{} <- Therobotsdayjob.Repo.get_by(Schema, %{user_name: user_name}) do
        ...
        fn
          ...
          {"name", value} ->
            case user.name do
              %Ecto.Changeset{} ->
                value =
                  Therobotsdayjob.Versioned.Names.change_versioned_name(user.name.data, value)

                {:name, value}

              _ ->
                value =
                  Therobotsdayjob.Versioned.Names.change_versioned_name(
                    user.name || %Therobotsdayjob.Versioned.Names.Name{},
                    value
                  )
      ...
      |> put_in(
        ...
        {:embed,
         %{
           ...
           on_cast: :update,
           on_replace: :update,
           related: Therobotsdayjob.Versioned.Names.Name
         }}
      )
  ...
  def valid_login?(email, password) do
    Therobotsdayjob.Users.Credentials.valid_login?(email, password)
  end

  def login_available?(email, context, options \\ nil) do
    Therobotsdayjob.Users.Credentials.login_available?(email, context, options)
  end
  ...
  defp unique_handle(handle, context, options) do
    case Therobotsdayjob.Users.by_handle(handle, context, options) do
      nil ->
        {:ok, handle}
      ...
      {:ok, _} ->
        Enum.reduce_while(0..999, handle, fn suffix, _acc ->
          ...
          case Therobotsdayjob.Users.by_handle(with_suffix, context, options) do
            nil ->
              {:halt, {:ok, with_suffix}}
