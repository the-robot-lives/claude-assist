defmodule Therobotsdayjob.Media do
  alias Therobotsdayjob.Media.Asset, as: Entity
  alias Therobotsdayjob.Schema.Media.Asset, as: Schema
  use Noizu.Repo
  ...
  def get_cached_variant(media_id, canonical_params) do
    import Ecto.Query
    Therobotsdayjob.Repo.one(
      from v in Therobotsdayjob.Schema.Media.Variant,
        where: v.media_id == ^media_id and v.params == ^canonical_params
    )
  end

  def cache_variant(attrs) do
    %Therobotsdayjob.Schema.Media.Variant{}
    |> Therobotsdayjob.Schema.Media.Variant.changeset(attrs)
    |> Therobotsdayjob.Repo.insert(on_conflict: :nothing)
  end

  def fetch_from_s3(key) do
    config = Application.get_env(:therobotsdayjob, Therobotsdayjob.Storage, [])
  ...
  def get_or_create_variant(media, transform_params) do
    canonical = Therobotsdayjob.Media.Transform.canonical_params(transform_params)
      ...
      nil ->
        with {:ok, original_binary} <- fetch_from_s3(media.file),
             {:ok, transformed, content_type} <-
               Therobotsdayjob.Media.Transform.transform(original_binary, transform_params) do
          variant_key = Therobotsdayjob.Media.Transform.variant_s3_key(media.file, transform_params)
