defmodule Therobotsdayjob.Auth.SSO do
  alias Therobotsdayjob.Schema.Users.User, as: UserSchema
  alias Therobotsdayjob.Schema.Users.Credentials.UserCredential, as: CredentialSchema
  alias Therobotsdayjob.Schema.Versioned.Names.Name
  import Ecto.Query, only: [from: 2]

  @provider_map %{
    oidc: &Therobotsdayjob.Auth.Providers.oidc/0,
    saml: &Therobotsdayjob.Auth.Providers.saml/0,
    google: &Therobotsdayjob.Auth.Providers.google/0,
    facebook: &Therobotsdayjob.Auth.Providers.facebook/0,
    github: &Therobotsdayjob.Auth.Providers.github/0,
    linkedin: &Therobotsdayjob.Auth.Providers.linkedin/0
  }
  ...
  def authenticate_sso(provider_type, %{email: email} = attrs) do
    ...
    {:ok, provider_id} = Therobotsdayjob.Auth.Providers.Provider.id(provider_ref)
    ...
    case find_user_by_email(email) do
      ...
      :not_found ->
        if Application.get_env(:therobotsdayjob, :sso_require_invite, false) do
  ...
  defp find_user_by_email(email) do
    ...
    case Therobotsdayjob.Repo.one(q) do
      nil -> :not_found
      user -> {:ok, user}
    end
  end
  ...
  defp ensure_sso_credential(user, provider_ref, provider_id, provider_type, attrs, context) do
    ...
    case Therobotsdayjob.Repo.one(q) do
      nil ->
        %Therobotsdayjob.Users.Credentials.UserCredential{
          user: Therobotsdayjob.Users.User.ref(user.id),
  ...
  defp create_sso_session(user, provider_type, context) do
    ...
    %Therobotsdayjob.Users.Sessions.UserSession{
      user: user_ref,
      status: :active,
  ...
  defp auto_provision_user(email, attrs, provider_ref, provider_id, provider_type, context) do
    ...
    {:ok, name} =
      Therobotsdayjob.EntityRepo.create(
        %Therobotsdayjob.Versioned.Names.Name{first: first, last: last, time_stamp: Noizu.Entity.TimeStamp.now()},
        context
      )
    ...
    {:ok, user} = Therobotsdayjob.Repo.insert(user_schema, on_conflict: :nothing, conflict_target: :email)
