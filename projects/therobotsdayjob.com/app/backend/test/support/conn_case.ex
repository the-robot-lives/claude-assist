defmodule TherobotsdayjobWeb.ConnCase do
  ...
  using do
    quote do
      ...
      import TherobotsdayjobWeb.ConnCase

      @endpoint TherobotsdayjobWeb.Endpoint
    end
  end
  ...
  def setup_user_and_token(_context \\ %{}) do
    ...
    Therobotsdayjob.Repo.insert!(
      %Therobotsdayjob.Schema.Auth.Providers.Provider{
    ...
    user = %Therobotsdayjob.Schema.Users.User{
    ...
    {:ok, user} = Therobotsdayjob.Repo.insert(user)

    credential = %Therobotsdayjob.Schema.Users.Credentials.UserCredential{
      id: Ecto.UUID.generate(),
      user_id: user.id,
    ...
    {:ok, _credential} = Therobotsdayjob.Repo.insert(credential)

    session = %Therobotsdayjob.Schema.Users.Sessions.UserSession{
      id: Ecto.UUID.generate(),
      user_id: user.id,
      status: :active,
      details: %{}
    ...
    {:ok, session} = Therobotsdayjob.Repo.insert(session)

    # Build entity-layer session struct for Guardian signing
    session_entity = %Therobotsdayjob.Users.Sessions.UserSession{
      id: session.id,
      user: {:ref, Therobotsdayjob.Users.User, user.id},
      status: :active,
      details: %{}
    }

    {:ok, access_token, _claims} =
      Therobotsdayjob.Guardian.encode_and_sign(session_entity, %{}, token_type: "access", ttl: {1, :hour})

    {:ok, refresh_token, %{"jti" => jti}} =
      Therobotsdayjob.Guardian.encode_and_sign(session_entity, %{}, token_type: "refresh", ttl: {7, :day})

    Therobotsdayjob.Auth.TokenStore.store_refresh_jti(jti)
