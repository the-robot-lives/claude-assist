defmodule DesigningDerobotWeb.Router do
  use DesigningDerobotWeb, :router
  ...
  pipeline :authenticated do
    plug DesigningDerobotWeb.AuthPipeline
  end
  ...
  pipeline :rate_limited_auth do
    plug DesigningDerobotWeb.Plugs.RateLimit, action: :auth
  end

  pipeline :rate_limited_sensitive do
    plug DesigningDerobotWeb.Plugs.RateLimit, action: :auth_sensitive
  end

  pipeline :org_viewer do
    plug DesigningDerobotWeb.Plugs.RequireRole, role: "viewer"
  end

  pipeline :org_editor do
    plug DesigningDerobotWeb.Plugs.RequireRole, role: "editor"
  end

  pipeline :org_admin do
    plug DesigningDerobotWeb.Plugs.RequireRole, role: "admin"
  end

  pipeline :org_owner do
    plug DesigningDerobotWeb.Plugs.RequireRole, role: "owner"
  end

  pipeline :admin do
    plug DesigningDerobotWeb.Plugs.RequireAdmin
  end

  scope "/", DesigningDerobotWeb do
    pipe_through :api
    get "/health", HealthController, :index
  end

  scope "/api/v1", DesigningDerobotWeb do
  ...
  scope "/media", DesigningDerobotWeb do
    pipe_through [:api]
  ...
  if Application.compile_env(:designing_derobot, :saml_enabled) do
    scope "/sso/saml" do
  ...
  # OIDC redirect flow
  scope "/auth/oidc", DesigningDerobotWeb do
    pipe_through [:sso_session]
  ...
  scope "/auth", DesigningDerobotWeb do
    pipe_through [:sso_session]
  ...
  if Application.compile_env(:designing_derobot, :dev_routes) do
    import Phoenix.LiveDashboard.Router

    scope "/dev" do
      pipe_through [:fetch_session, :protect_from_forgery]
      live_dashboard "/dashboard", metrics: DesigningDerobotWeb.Telemetry
    end
  end
end
