defmodule DesigningDerobotWeb.SSOController do
  ...
  def exchange(conn, %{"code" => code}) do
    with {:ok, session_id} <- DesigningDerobot.Auth.SSOCode.exchange(code),
         {:ok, session} <- DesigningDerobot.Users.Sessions.get(session_id, Noizu.Context.system()),
  ...
  defp handle_sso_callback(conn, provider_type, attrs) do
    ...
    case DesigningDerobot.Auth.SSO.authenticate_sso(provider_type, attrs) do
      {:ok, session} ->
        {:ok, code} = DesigningDerobot.Auth.SSOCode.create(session.id)
  ...
  defp maybe_add(list, flag, name) do
    if Application.get_env(:designing_derobot, flag), do: [name | list], else: list
  end
end
