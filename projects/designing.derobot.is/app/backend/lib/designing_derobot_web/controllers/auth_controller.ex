defmodule DesigningDerobotWeb.AuthController do
  use DesigningDerobotWeb, :controller

  alias DesigningDerobot.Guardian
  alias DesigningDerobot.Organizations
  ...
  def register(conn, %{"invite_token" => raw_token, "user" => user_params}) do
    with {:ok, invite} <- Organizations.find_active_invite_by_raw_token(raw_token),
         {:ok, {user, _credential}} <-
           DesigningDerobot.Users.register(
             %{
      ...
      if invite.organization_id do
        DesigningDerobot.Authz.ScopedMemberships.add_member(
          "organization", invite.organization_id, user.id, "viewer"
        )
      end

      Organizations.increment_invite_uses(invite)
      DesigningDerobot.Auth.TokenStore.store_refresh_jti(refresh_jti)
      DesigningDerobot.Events.dispatch(:user_registered, %{user_id: user.id, email: user.email})
  ...
  def login(conn, %{"email" => email, "password" => password}) do
    case DesigningDerobot.Users.Credentials.authenticate({:login, {email, password}}, Noizu.Context.system(), []) do
      {:ok, session} ->
        {:ok, access_token, _} =
          Guardian.encode_and_sign(session, %{}, token_type: "access", ttl: {1, :hour})
  ...
  def request_magic_link(conn, %{"email" => email}) do
    frontend_url = Application.get_env(:designing_derobot, :frontend_url, "http://localhost:3000")

    case DesigningDerobot.Auth.SmartTokenAuth.request_magic_link(email) do
      {:ok, %{encoded_key: encoded_key, user: _user}} ->
        magic_link = "#{frontend_url}/auth/verify?token=#{encoded_key}"
        DesigningDerobot.Auth.SmartTokenEmail.send_magic_link(email, magic_link)

        response = %{message: "If an account exists with that email, a magic link has been sent."}

        response =
          if Application.get_env(:designing_derobot, :dev_routes) do
  ...
  def verify_magic_link(conn, %{"token" => token_key}) do
    case DesigningDerobot.Auth.SmartTokenAuth.verify_magic_link(token_key, conn) do
      {:ok, session} ->
        {:ok, access_token, _} =
          Guardian.encode_and_sign(session, %{}, token_type: "access", ttl: {1, :hour})
  ...
  def request_otp_login(conn, %{"email" => email}) do
    case DesigningDerobot.Auth.SmartTokenAuth.request_otp_login(email) do
      {:ok, %{otp_code: otp_code, user: _user}} ->
        DesigningDerobot.Auth.SmartTokenEmail.send_otp_login(email, otp_code)
  ...
  def verify_otp_login(conn, %{"email" => email, "code" => code}) do
    case DesigningDerobot.Auth.SmartTokenAuth.verify_otp_login(email, code, conn) do
      {:ok, session} ->
        {:ok, access_token, _} =
          Guardian.encode_and_sign(session, %{}, token_type: "access", ttl: {1, :hour})
  ...
  def request_password_reset(conn, %{"email" => email}) do
    case DesigningDerobot.Auth.SmartTokenAuth.request_password_reset(email) do
      {:ok, %{otp_code: otp_code, user: _user}} ->
        DesigningDerobot.Auth.SmartTokenEmail.send_password_reset(email, otp_code)
  ...
  def verify_password_reset(conn, %{"email" => email, "code" => code, "new_password" => new_password}) do
    case DesigningDerobot.Auth.SmartTokenAuth.verify_password_reset(email, code, new_password, conn) do
      {:ok, _credential} ->
  ...
  def refresh(conn, %{"refresh_token" => refresh_token}) do
    alias DesigningDerobot.Auth.TokenStore
  ...
  def send_verification(conn, _params) do
    ...
    case DesigningDerobot.Auth.SmartTokenAuth.request_email_verification(user.email) do
      {:ok, %{encoded_key: encoded_key}} ->
        verification_link = "#{frontend_url}/auth/verify-email?token=#{encoded_key}"
        DesigningDerobot.Auth.SmartTokenEmail.send_verification_email(user.email, verification_link)
  ...
  def verify_email(conn, %{"token" => token_key}) do
    case DesigningDerobot.Auth.SmartTokenAuth.verify_email_token(token_key) do
      {:ok, user} ->
        DesigningDerobot.Events.dispatch(:user_verified, %{user_id: user.id})
  ...
  defp create_session_for_user(user) do
    user_ref = DesigningDerobot.Users.User.ref(user.id)
    session_entity = %DesigningDerobot.Users.Sessions.UserSession{
      user: user_ref,
      status: :active,
      details: %{}
    }
    DesigningDerobot.Users.Sessions.create(session_entity, Noizu.Context.system())
  end

  defp resolve_user_from_session(%DesigningDerobot.Users.Sessions.UserSession{} = session) do
    case session.user do
      {:ref, _, id} ->
        {:ok, user} = DesigningDerobot.Users.get_user(id, Noizu.Context.system())
        user
      %DesigningDerobot.Users.User{} = user -> user
    end
  end
