defmodule DesigningDerobot.Auth.SSO do
  alias DesigningDerobot.Schema.Users.User, as: UserSchema
  alias DesigningDerobot.Schema.Users.Credentials.UserCredential, as: CredentialSchema
  alias DesigningDerobot.Schema.Versioned.Names.Name
  import Ecto.Query, only: [from: 2]

  @provider_map %{
    oidc: &DesigningDerobot.Auth.Providers.oidc/0,
    saml: &DesigningDerobot.Auth.Providers.saml/0,
    google: &DesigningDerobot.Auth.Providers.google/0,
    facebook: &DesigningDerobot.Auth.Providers.facebook/0,
    github: &DesigningDerobot.Auth.Providers.github/0,
    linkedin: &DesigningDerobot.Auth.Providers.linkedin/0
  }
  ...
  def authenticate_sso(provider_type, %{email: email} = attrs) do
    ...
    {:ok, provider_id} = DesigningDerobot.Auth.Providers.Provider.id(provider_ref)
    ...
    case find_user_by_email(email) do
      ...
      :not_found ->
        if Application.get_env(:designing_derobot, :sso_require_invite, false) do
  ...
  defp find_user_by_email(email) do
    ...
    case DesigningDerobot.Repo.one(q) do
      nil -> :not_found
      user -> {:ok, user}
    end
  end
  ...
  defp ensure_sso_credential(user, provider_ref, provider_id, provider_type, attrs, context) do
    ...
    case DesigningDerobot.Repo.one(q) do
      nil ->
        %DesigningDerobot.Users.Credentials.UserCredential{
          user: DesigningDerobot.Users.User.ref(user.id),
  ...
  defp create_sso_session(user, provider_type, context) do
    ...
    %DesigningDerobot.Users.Sessions.UserSession{
      user: user_ref,
      status: :active,
  ...
  defp auto_provision_user(email, attrs, provider_ref, provider_id, provider_type, context) do
    ...
    {:ok, name} =
      DesigningDerobot.EntityRepo.create(
        %DesigningDerobot.Versioned.Names.Name{first: first, last: last, time_stamp: Noizu.Entity.TimeStamp.now()},
        context
      )
    ...
    {:ok, user} = DesigningDerobot.Repo.insert(user_schema, on_conflict: :nothing, conflict_target: :email)
