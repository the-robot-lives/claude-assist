defmodule DesigningDerobot.Media do
  alias DesigningDerobot.Media.Asset, as: Entity
  alias DesigningDerobot.Schema.Media.Asset, as: Schema
  use Noizu.Repo
  ...
  def get_cached_variant(media_id, canonical_params) do
    import Ecto.Query
    DesigningDerobot.Repo.one(
      from v in DesigningDerobot.Schema.Media.Variant,
        where: v.media_id == ^media_id and v.params == ^canonical_params
    )
  end

  def cache_variant(attrs) do
    %DesigningDerobot.Schema.Media.Variant{}
    |> DesigningDerobot.Schema.Media.Variant.changeset(attrs)
    |> DesigningDerobot.Repo.insert(on_conflict: :nothing)
  end

  def fetch_from_s3(key) do
    config = Application.get_env(:designing_derobot, DesigningDerobot.Storage, [])
  ...
  def get_or_create_variant(media, transform_params) do
    canonical = DesigningDerobot.Media.Transform.canonical_params(transform_params)
      ...
      nil ->
        with {:ok, original_binary} <- fetch_from_s3(media.file),
             {:ok, transformed, content_type} <-
               DesigningDerobot.Media.Transform.transform(original_binary, transform_params) do
          variant_key = DesigningDerobot.Media.Transform.variant_s3_key(media.file, transform_params)
