defmodule DesigningDerobot.Users.Credentials do
  @moduledoc """
  Context for DesigningDerobot.Users.Credentials
  """
  alias DesigningDerobot.Users.Credentials.UserCredential, as: Entity
  alias DesigningDerobot.Schema.Users.Credentials.UserCredential, as: Schema
  use Noizu.Repo
  import Ecto.Query, only: [from: 2]

  def_repo(entity: DesigningDerobot.Users.Credentials.UserCredential)
  ...
  def register(user, {:login, {email, password}}, context, _options) do
    {:ok, auth_provider} = DesigningDerobot.Auth.Providers.login()

    {:ok, description} =
      %DesigningDerobot.Versioned.Strings.String{
        content: "",
  ...
  def authenticate({:login, {email, password}}, context, options) do
    {:ok, auth_provider} = DesigningDerobot.Auth.Providers.login()
    {:ok, auth_provider_id} = DesigningDerobot.Auth.Providers.Provider.id(auth_provider)
    ...
    with :valid <- valid_login?(email, password) do
      ...
      case DesigningDerobot.Repo.all(q) do
        [] ->
        ...
        [credential] ->
          if Bcrypt.verify_pass(password, credential.settings["password"]) do
            with {:ok, credential_entity} <-
                   DesigningDerobot.Users.Credentials.UserCredential.entity(credential.id, context),
              ...
              |> DesigningDerobot.EntityRepo.create(context, options)
            end
          else
