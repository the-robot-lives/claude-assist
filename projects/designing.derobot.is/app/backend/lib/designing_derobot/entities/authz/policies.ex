defmodule DesigningDerobot.Authz.Policies do
  alias DesigningDerobot.Authz.Policies.Policy, as: Entity
  alias DesigningDerobot.Schema.Authz.Policy, as: Schema
  alias DesigningDerobot.Schema.Authz.UserPolicy, as: UserPolicySchema
  ...
  def list_active(opts \\ []) do
    ...
    DesigningDerobot.Repo.all(query)
  end

  def get_by_name(name) do
    DesigningDerobot.Repo.one(from p in Schema, where: p.name == ^name and p.is_active == true)
  end
    ...
    case DesigningDerobot.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          policy |> Schema.changeset(attrs) |> DesigningDerobot.Repo.update()
        end
    end
  end
    ...
    case DesigningDerobot.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          DesigningDerobot.Repo.delete(policy)
        end
    end
  end
  ...
  def detach_from_user(user_id, policy_id) do
    case DesigningDerobot.Repo.one(
      from up in UserPolicySchema,
        where: up.user_id == ^user_id and up.policy_id == ^policy_id
    ) do
      nil -> {:error, :not_found}
      user_policy -> DesigningDerobot.Repo.delete(user_policy)
    end
  end
end
