defmodule DesigningDerobot.Organizations do
  @moduledoc """
  Context for DesigningDerobot.Organizations
  """
  alias DesigningDerobot.Organizations.Organization, as: Entity
  alias DesigningDerobot.Schema.Organizations.Organization, as: Schema
  alias DesigningDerobot.Schema.Organizations.InviteToken, as: InviteTokenSchema
  alias DesigningDerobot.Schema.Authz.ScopedMembership, as: ScopedMembershipSchema
  use Noizu.Repo
  ...
  def create_organization_with_owner(attrs, user_id) do
    DesigningDerobot.Repo.transaction(fn ->
      with {:ok, org} <- %Schema{} |> Schema.changeset(attrs) |> DesigningDerobot.Repo.insert(),
           {:ok, _membership} <- DesigningDerobot.Authz.ScopedMemberships.add_member("organization", org.id, user_id, "owner") do
        org
      else
  ...
  def list_user_organizations(user_id) do
    from(sm in ScopedMembershipSchema,
      ...
      join: g in DesigningDerobot.Schema.Authz.Group, on: g.id == sm.group_id,
    ...
    |> DesigningDerobot.Repo.all()
  end

  def authorize(user_id, organization_id, required_role) do
    DesigningDerobot.Authz.authorize(user_id, "organization", organization_id, required_role)
  end

  def list_members(organization_id) do
    DesigningDerobot.Authz.ScopedMemberships.list_for_resource("organization", organization_id)
  end
  ...
  def increment_invite_uses(invite_token) do
    ...
    |> DesigningDerobot.Repo.update_all(inc: [uses: 1])
  end
end
