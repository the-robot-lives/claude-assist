...
if System.get_env("PHX_SERVER") do
  config :designing_derobot, DesigningDerobotWeb.Endpoint, server: true
end

config :designing_derobot, :redis,
...
if config_env() == :prod do
  ...
  config :designing_derobot, :dns_cluster_query, System.get_env("DNS_CLUSTER_QUERY")
  ...
  config :designing_derobot, :frontend_url, System.get_env("FRONTEND_URL")
  ...
  config :designing_derobot, :mail_from,
    {System.get_env("MAIL_FROM_NAME", "DesigningDerobot"),
     System.get_env("MAIL_FROM_ADDRESS", "noreply@starter.local")}

  # ── Storage (S3/MinIO) ──────────────────────────────────────────
  if s3_bucket = System.get_env("S3_BUCKET") do
    config :designing_derobot, DesigningDerobot.Storage,
      bucket: s3_bucket,
  ...
  if oidc_client_id = System.get_env("OIDC_CLIENT_ID") do
    ...
    config :designing_derobot, :oidc_enabled, true
  end
  ...
  if saml_metadata = System.get_env("SAML_IDP_METADATA_URL") do
    ...
    config :designing_derobot, :saml_enabled, true
  end

  # ── SSO: Social OAuth (each enabled when *_CLIENT_ID is set) ──
  config :designing_derobot, :sso_require_invite, System.get_env("SSO_REQUIRE_INVITE") == "true"
  ...
  oauth_providers =
    if google_id = System.get_env("GOOGLE_CLIENT_ID") do
      ...
      config :designing_derobot, :google_enabled, true
  ...
  oauth_providers =
    if fb_id = System.get_env("FACEBOOK_CLIENT_ID") do
      ...
      config :designing_derobot, :facebook_enabled, true
  ...
  oauth_providers =
    if gh_id = System.get_env("GITHUB_CLIENT_ID") do
      ...
      config :designing_derobot, :github_enabled, true
  ...
  #     config :designing_derobot, :linkedin_enabled, true
