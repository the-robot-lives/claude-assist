defmodule Codefresh.Auth.SmartTokenAuth do
    ...
    case SmartToken.authorize!(token_key, %{}, context) do
      {:ok, token} ->
        with {:ok, user} <- Noizu.EntityReference.Protocol.entity(token.resource, context) do
          ...
          |> Codefresh.Repo.update_all(set: [verified: true])
  ...
  defp find_active_user_by_email(email) do
    ...
    case Codefresh.Repo.one(q) do
      ...
      record ->
        settings = Noizu.Entity.Meta.persistence(Codefresh.Users.User) |> hd
        Codefresh.Users.User.from_record(record, settings, Noizu.Context.system(), [])
    end
  end
end
