defmodule Codefresh.Auth.SSO do
  ...
  @provider_map %{
    oidc: &Codefresh.Auth.Providers.oidc/0,
    saml: &Codefresh.Auth.Providers.saml/0,
    google: &Codefresh.Auth.Providers.google/0,
    facebook: &Codefresh.Auth.Providers.facebook/0,
    github: &Codefresh.Auth.Providers.github/0,
    linkedin: &Codefresh.Auth.Providers.linkedin/0
  }
  ...
  def authenticate_sso(provider_type, %{email: email} = attrs) do
    ...
    {:ok, provider_id} = Codefresh.Auth.Providers.Provider.id(provider_ref)
    ...
    case find_user_by_email(email) do
      ...
      :not_found ->
        if Application.get_env(:codefresh, :sso_require_invite, false) do
