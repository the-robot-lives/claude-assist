defmodule Codefresh.Users.Credentials do
  @moduledoc """
  Context for Codefresh.Users.Credentials
  """
  alias Codefresh.Users.Credentials.UserCredential, as: Entity
  alias Codefresh.Schema.Users.Credentials.UserCredential, as: Schema
  use Noizu.Repo
  import Ecto.Query, only: [from: 2]

  def_repo(entity: Codefresh.Users.Credentials.UserCredential)
  ...
  def register(user, {:login, {email, password}}, context, _options) do
    ...
    {:ok, description} =
      %Codefresh.Versioned.Strings.String{
        content: "",
        ...
        [credential] ->
          if Bcrypt.verify_pass(password, credential.settings["password"]) do
            with {:ok, credential_entity} <-
              ...
              |> Codefresh.EntityRepo.create(context, options)
            end
          else
