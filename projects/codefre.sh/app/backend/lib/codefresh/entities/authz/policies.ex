defmodule Codefresh.Authz.Policies do
  alias Codefresh.Authz.Policies.Policy, as: Entity
  alias Codefresh.Schema.Authz.Policy, as: Schema
  alias Codefresh.Schema.Authz.UserPolicy, as: UserPolicySchema
  ...
  def list_active(opts \\ []) do
    ...
    Codefresh.Repo.all(query)
  end

  def get_by_name(name) do
    Codefresh.Repo.one(from p in Schema, where: p.name == ^name and p.is_active == true)
  end
    ...
    case Codefresh.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          policy |> Schema.changeset(attrs) |> Codefresh.Repo.update()
        end
    end
  end
  ...
  def detach_from_user(user_id, policy_id) do
    case Codefresh.Repo.one(
      ...
      nil -> {:error, :not_found}
      user_policy -> Codefresh.Repo.delete(user_policy)
    end
  end
end
