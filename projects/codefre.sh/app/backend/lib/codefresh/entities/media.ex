defmodule Codefresh.Media do
  alias Codefresh.Media.Asset, as: Entity
  alias Codefresh.Schema.Media.Asset, as: Schema
  use Noizu.Repo
  ...
  def cache_variant(attrs) do
    %Codefresh.Schema.Media.Variant{}
    |> Codefresh.Schema.Media.Variant.changeset(attrs)
    |> Codefresh.Repo.insert(on_conflict: :nothing)
  end
  ...
  def get_or_create_variant(media, transform_params) do
    canonical = Codefresh.Media.Transform.canonical_params(transform_params)
      ...
      nil ->
        with {:ok, original_binary} <- fetch_from_s3(media.file),
             {:ok, transformed, content_type} <-
               Codefresh.Media.Transform.transform(original_binary, transform_params) do
          variant_key = Codefresh.Media.Transform.variant_s3_key(media.file, transform_params)
