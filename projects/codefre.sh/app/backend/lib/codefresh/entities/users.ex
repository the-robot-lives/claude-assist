defmodule Codefresh.Users do
  @moduledoc """
  Context for Codefresh.Users
  """
  alias Codefresh.Users.User, as: Entity
  alias Codefresh.Schema.Users.User, as: Schema
  use Noizu.Repo
  def_repo(entity: Codefresh.Users.User)
  ...
  def register(details, {:login, {email, password}} = auth, context, options) do
    ...
    email = Codefresh.Users.Credentials.standardize_email(email)
    ...
    with :valid <- valid_user_name?(details.user_name),
      ...
      with {:ok, name_entity} <-
           ...
           {:ok, description} <-
             %Codefresh.Versioned.Descriptions.Description{
               title: "Description",
           ...
           {:ok, credential} <-
             Codefresh.Users.Credentials.register(user_ref, auth, context, options) do
        {:ok, {user, credential}}
      end
    end
  end
  ...
  def authenticate(auth = {:login, {_email, _password}}, context, options \\ nil) do
    with {:ok, session} <- Codefresh.Users.Credentials.authenticate(auth, context, options) do
      {:ok, session}
    end
  end

  def by_handle(handle, context, options \\ []) do
    with record = %Schema{} <- Codefresh.Repo.get_by(Schema, %{handle: handle}) do
      settings = Noizu.Entity.Meta.persistence(Entity) |> hd
      {:ok, entity} = Entity.from_record(record, settings, context, options)
  ...
  def user_name_available?(user_name, _context, _options \\ nil) do
    with %{} <- Codefresh.Repo.get_by(Schema, %{user_name: user_name}) do
      ...
      |> put_in(
        ...
        {:embed,
         %{
           ...
           on_cast: :update,
           on_replace: :update,
           related: Codefresh.Versioned.Names.Name
         }}
      )
  ...
  def valid_login?(email, password) do
    Codefresh.Users.Credentials.valid_login?(email, password)
  end

  def login_available?(email, context, options \\ nil) do
    Codefresh.Users.Credentials.login_available?(email, context, options)
  end
  ...
  defp unique_handle(handle, context, options) do
    case Codefresh.Users.by_handle(handle, context, options) do
      nil ->
        {:ok, handle}
      ...
      {:ok, _} ->
        Enum.reduce_while(0..999, handle, fn suffix, _acc ->
          ...
          case Codefresh.Users.by_handle(with_suffix, context, options) do
            nil ->
              {:halt, {:ok, with_suffix}}
